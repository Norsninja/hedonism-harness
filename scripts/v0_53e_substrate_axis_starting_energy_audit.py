"""v0.53e substrate-axis disambiguation audit (founder starting-energy rescue probe).

Pre-reg: [[docs/experiments/fear_hunger_v0.53e.md]]. Question: is v0.53c /
v0.53d's reachability-bound result on ``widened_gradient`` driven by founder
per-agent energy budget rather than ambient-pool dynamics? Specifically: does
raising ``BodyConfig.starting_energy`` from the V0_25 baseline of ``60.0`` to
``100.0`` (the maximal legal one-knob founder-energy-budget increase under the
existing ``BodyConfig.max_energy=100.0`` bound), with all other V0_25 substrate
parameters held identical, lift ``widened_gradient`` reachability above the
locked 25% threshold at tick-200 AND admit a measurable v0.48 sensor_radius
spatial / foraging bridge?

Corpus (locked, identical shape to v0.53 / v0.53b / v0.53c / v0.53d): A_null
arm only across v0.42 / v0.43R / v0.44 / v0.45, hazards {0, 8}, seeds 41..72.
64 runs per arm; 3 arms (asymmetric BodyConfig); 192 runs total.
...
Arms (locked, 3 — asymmetric BodyConfig):
  ...
  C_widened_starting_energy_100:    widened_gradient_layout() +
                                    body_config=BodyConfig(starting_energy=100.0)

A and B preserve V0_25 baseline (default ``BodyConfig()``: starting_energy=60.0).
C differs from B by exactly one knob (``BodyConfig.starting_energy``: 60.0 ->
100.0). All three arms preserve V0_25's ``ambient_influx_rate=1.0``,
...
Tick-50 cells anchor v0.48 / v0.53 / v0.53b / v0.53c / v0.53d (Tier-1 priority 2.a).
Tick-100 cells are descriptive. Tick-200 cells are verdict-gating on the C arm.

Labels:
  Label A: high_sensor_radius_lineage. Trait-resolved (identical to v0.48-v0.53d).
...
Tier-2 categorical anchor (locked, pre-data):
  ...
  (categorical, not float-tolerance — locked from v0.53c / v0.53d's measured
  0/64 result.)

Verdict-gating reachability (locked, pre-data):
  c_widened_starting_energy_100_reachability_run_share_tick_200  vs threshold 0.25.

Per-arm tick-200 sub-verdicts (locked, 4-way each):
  ...
  C_STARTING_ENERGY_100_TICK200_BRIDGE_{PRESENT, PARTIAL, NOT_FOUND, OPPOSITE_SIGN_HALT}
...
Slice rollup (locked, 6 outcomes, priority-ordered):
  ...
  3. RELAXED_OPPOSITE_SIGN_HALT  (C_widened_starting_energy_100 tick-200 wrong-sign)
...
Conservation framing: one additive ``src/`` seam in ``fear_hunger_chamber.py``
(``body_config`` parameter, default-preserving). No other ``src/`` modifications.

Usage:
    uv run python scripts/v0_53e_substrate_axis_starting_energy_audit.py
    uv run python scripts/v0_53e_substrate_axis_starting_energy_audit.py \
        --out-dir runs/v0.53e-substrate-axis-starting-energy-100
"""
...
ARM_C_WIDENED_STARTING_ENERGY_100: str = "C_widened_starting_energy_100"
ARMS: tuple[str, ...] = (ARM_A_NULL_V025, ARM_B_WIDENED_V025, ARM_C_WIDENED_STARTING_ENERGY_100)

LAYOUT_NAME_BY_ARM: dict[str, str] = {
    ARM_A_NULL_V025: "tight_gradient",
    ARM_B_WIDENED_V025: "widened_gradient",
    ARM_C_WIDENED_STARTING_ENERGY_100: "widened_gradient",
}
...
V0_25_AMBIENT_INFLUX_RATE: float = 1.0  # baseline (preserved on ALL arms in v0.53e).
V0_25_BODY_STARTING_ENERGY: float = 60.0  # default BodyConfig().starting_energy
STARTING_ENERGY_100_DOSE: float = 100.0  # v0.53e test dose (= max_energy bound)

# Per-arm body_starting_energy override (locked).
# A and B use V0_25 baseline (60.0); C is the starting-energy=100 variant.
BODY_STARTING_ENERGY_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_STARTING_ENERGY_100: STARTING_ENERGY_100_DOSE,
}
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    if arm in (ARM_B_WIDENED_V025, ARM_C_WIDENED_STARTING_ENERGY_100):
        return widened_gradient_layout()
    msg = f"v0.53e: unknown arm {arm!r}"
    raise V053eReducerError(msg)
...
def _body_config_for_arm(arm: str) -> BodyConfig | None:
    ...
    starting_energy=60.0). C returns ``BodyConfig(starting_energy=100.0)`` —
    all other BodyConfig fields preserved at default.
    """
    if arm in (ARM_A_NULL_V025, ARM_B_WIDENED_V025):
        return None
    if arm == ARM_C_WIDENED_STARTING_ENERGY_100:
        return BodyConfig(starting_energy=STARTING_ENERGY_100_DOSE)
    msg = f"v0.53e: unknown arm {arm!r}"
    raise V053eReducerError(msg)
...
# v0.48 / v0.53 / v0.53b / v0.53c / v0.53d published bridge cells
...
SUBVERDICT_C_STARTING_ENERGY_100_TICK200_PRESENT = "C_STARTING_ENERGY_100_TICK200_BRIDGE_PRESENT"
SUBVERDICT_C_STARTING_ENERGY_100_TICK200_PARTIAL = "C_STARTING_ENERGY_100_TICK200_BRIDGE_PARTIAL"
SUBVERDICT_C_STARTING_ENERGY_100_TICK200_NOT_FOUND = (
    "C_STARTING_ENERGY_100_TICK200_BRIDGE_NOT_FOUND"
)
SUBVERDICT_C_STARTING_ENERGY_100_TICK200_OPPOSITE = (
    "C_STARTING_ENERGY_100_TICK200_OPPOSITE_SIGN_HALT"
)
...
ROLLUP_WIDENED_BRIDGE_RESCUED_BY_STARTING_ENERGY_100 = (
    "WIDENED_BRIDGE_RESCUED_BY_STARTING_ENERGY_100"
)
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_STARTING_ENERGY_100 = (
    "WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_STARTING_ENERGY_100"
)
ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_STARTING_ENERGY_100 = (
    "WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_STARTING_ENERGY_100"
)
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53e's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_ANCHOR_REPLICATION_HALT: (
        "Halt: v0.53e's A_null_V0_25 arm does not reproduce v0.48 / v0.53 / v0.53b / "
        "v0.53c / v0.53d's tick-50 spatial bridge, OR v0.53e's B_widened_V0_25 arm "
        "does not reproduce v0.53c / v0.53d's `0/64` reachability lock at tick-200. "
        "v0.53e cannot interpret the C_widened_starting_energy_100 cells without an "
        "established baseline on the V0_25 substrate."
    ),
    ROLLUP_RELAXED_OPPOSITE_SIGN_HALT: (
        "Halt: a v0.53e C_widened_starting_energy_100 tick-200 spatial / foraging "
        "primary fires in the WRONG direction under a gating label; raising "
        "`BodyConfig.starting_energy` from V0_25 baseline `60.0` to the maximal legal "
        "`100.0` on the `widened_gradient` layout surfaces a regime incompatible with "
        "the locked expected signs."
    ),
    ROLLUP_WIDENED_BRIDGE_RESCUED_BY_STARTING_ENERGY_100: (
        "On the modern A_null corpus with the V0_25 substrate held constant except for "
        "`BodyConfig.starting_energy = 100.0` (raised from the V0_25 baseline of `60.0` "
        "to the maximal legal one-knob value under the existing `BodyConfig.max_energy "
        "= 100.0` bound) on `widened_gradient`, the v0.48 sensor_radius spatial / "
        "foraging bridge fires PRESENT under the locked +0.5 paired_d threshold at "
        "tick-200. The geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200` and v0.53d locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX` admits a "
        "measurable bridge under the maximal one-knob founder starting-energy "
        "envelope: B_widened_V0_25 reachability remains `0/64` at tick-200 "
        "(predecessor lock holds), C_widened_starting_energy_100 reachability clears "
        "the locked 25% threshold, and ≥ 2/3 spatial / foraging primaries fire "
        "PRESENT under both gating labels. The (V0_25 × `widened_gradient`) "  # noqa: RUF001
        "reachability ceiling is bounded by the V0_25 founder starting-energy budget "
        "at the `max_energy` bound, not by the geometry alone: "
        "`WIDENED_BRIDGE_RESCUED_BY_STARTING_ENERGY_100`."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_STARTING_ENERGY_100: (
        "On the modern A_null corpus with the V0_25 substrate held constant except for "
        "`BodyConfig.starting_energy = 100.0` (raised from the V0_25 baseline of `60.0` "
        "to the maximal legal one-knob value under the existing `BodyConfig.max_energy "
        "= 100.0` bound) on `widened_gradient`, fewer than 25% of "
        "C_widened_starting_energy_100 runs have any founder lineage with pre200 food "
        "events. C_widened_starting_energy_100's reachability is below the locked 25% "
        "threshold at tick-200; the geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200` and v0.53d locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX` remains below "
        "the reachability threshold under the maximal one-knob founder "
        "starting-energy envelope. The bridge is not rescued by "
        "`BodyConfig.starting_energy = 100` under V0_25 × `widened_gradient` × "  # noqa: RUF001
        "N_TICKS=200; v0.53f along `base_metabolic_cost` lowered (per-tick depletion "
        "rate, the orthogonal founder-facing budget axis) remains the natural "
        "follow-up: `WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_STARTING_ENERGY_100`."
    ),
    ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_STARTING_ENERGY_100: (
        "On the modern A_null corpus with the V0_25 substrate held constant except for "
        "`BodyConfig.starting_energy = 100.0` (raised from the V0_25 baseline of `60.0` "
        "to the maximal legal one-knob value under the existing `BodyConfig.max_energy "
        "= 100.0` bound) on `widened_gradient`, C_widened_starting_energy_100's "
        "reachability clears the locked 25% threshold at tick-200 but the v0.48 "
        "sensor_radius spatial / foraging bridge does not fully fire PRESENT — fewer "
        "than 2/3 primaries fire across both gating labels at tick-200 under the "
        "strict NaN rule. The geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200` and v0.53d locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX` is partially "
        "rescued under the maximal one-knob founder starting-energy envelope: "
...
class V053eReducerError(Exception):
    """Halt condition raised when a v0.53e invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53e audit (read-only).

    ``founder_body_energy_tick0`` captures ``founder.body.energy`` at
    setup_observer time (read-only). Used by Test #5 to verify the
    ``body_config`` seam took effect at config-time founder construction —
    B founders have ``energy == 60.0`` (V0_25 baseline), C founders have
    ``energy == 100.0`` (overridden ``starting_energy``).
    """
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53e: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    Reads ``body.energy`` at tick-0 to verify the v0.53e ``body_config`` seam
    propagated to founder construction (B arm should observe 60.0, C arm
    should observe 100.0). Strictly read-only on every founder body field.
    """
    founders = sorted(model.agents, key=lambda a: int(a.body.lineage_id))
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53e invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53e layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(
    ...
    Order (mirrors v0.53c / v0.53d merged implementation):
      (a) capture v0.53e founder audit table from live bodies (read-only;
          INCLUDES body.energy at tick 0 to verify the body_config seam).
    ...
    setup_observer is strictly READ-ONLY on every founder body field — no
    patching, no clamps, no overrides. The starting_energy=100 change
    propagates via the body_config parameter to run_chamber() at config-time
    founder construction, NOT via any post-construction body patch.
    """
...
def _run_one_arm(arm: str, version: str, seed: int, hazard: int, runs_root: Path) -> _RunCapture:
    ...
    ``starting_energy=60.0``). C overrides ``body_config=BodyConfig(
    starting_energy=100.0)``. All other WorldConfig / ReproductionConfig /
    BodyConfig knobs are inherited byte-identically from the V0_25 baseline
    arm; ``ambient_influx_rate=1.0`` (V0_25 baseline) on all three arms.
    """
    if arm not in ARMS:
        msg = f"v0.53e: unknown arm {arm!r}"
        raise V053eReducerError(msg)
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53e: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53e-{arm}-{version}-A_null-hzd{hazard}",
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53e: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053eReducerError(msg)
...
_SUBVERDICT_TICK200_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_WIDENED_STARTING_ENERGY_100: (
        SUBVERDICT_C_STARTING_ENERGY_100_TICK200_PRESENT,
        SUBVERDICT_C_STARTING_ENERGY_100_TICK200_PARTIAL,
        SUBVERDICT_C_STARTING_ENERGY_100_TICK200_NOT_FOUND,
        SUBVERDICT_C_STARTING_ENERGY_100_TICK200_OPPOSITE,
    ),
}
...
def _arm_subverdict_tick200(
    ...
    if arm not in _SUBVERDICT_TICK200_TABLE:
        msg = f"v0.53e: unknown arm {arm!r} in tick-200 subverdict"
        raise V053eReducerError(msg)
...
def _check_bridge_re_anchor_tick50(
    ...
    v0.48 / v0.53 / v0.53b / v0.53c / v0.53d."""
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    c_widened_starting_energy_100_reachability_tick200: float,
) -> tuple[str, str]:
    ...
    Priority 3 fires only on C_widened_starting_energy_100 tick-200 wrong-sign
    (A and B OPPOSITE flags do NOT trigger priority 3 in v0.53e — A/B are
    anchor arms and surface as priority-2 halts via sub-verdict / categorical
    lock).

    Priorities 4/5/6 gate on C_widened_starting_energy_100 reachability +
    sub-verdict only.
    """
    ...
    # Priority 3: RELAXED_OPPOSITE_SIGN_HALT — C_widened_starting_energy_100 only.
    if c_tick200_subverdict == SUBVERDICT_C_STARTING_ENERGY_100_TICK200_OPPOSITE:
        return (
            ROLLUP_RELAXED_OPPOSITE_SIGN_HALT,
    ...
    if c_widened_starting_energy_100_reachability_tick200 < B_REACHABILITY_THRESHOLD:
        # Priority 5: below threshold, regardless of C sub-verdict.
        return (
            ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_STARTING_ENERGY_100,
            LOCKED_PHRASES[ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_STARTING_ENERGY_100],
        )
    if c_tick200_subverdict == SUBVERDICT_C_STARTING_ENERGY_100_TICK200_PRESENT:
        # Priority 4: rescued.
        return (
            ROLLUP_WIDENED_BRIDGE_RESCUED_BY_STARTING_ENERGY_100,
            LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_RESCUED_BY_STARTING_ENERGY_100],
        )
    # Priority 6: partially rescued (C sub-verdict in {PARTIAL, NOT_FOUND}).
    return (
        ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_STARTING_ENERGY_100,
        LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_STARTING_ENERGY_100],
    )
    ...
    for arm, (reach, n) in arm_reach.items():
        ...
        writer.writerow(
            [
                ...
                _format_value(is_gating and arm == ARM_C_WIDENED_STARTING_ENERGY_100),
            ]
        )
...
def _write_audit_summary_csv(
    ...
    with path.open("w", newline="") as f:
        ...
        writer.writerow(
            ["sub_verdicts_tick200", ARM_C_WIDENED_STARTING_ENERGY_100, c_tick200_subverdict]
        )
...
def _write_audit_log(  # noqa: PLR0912
    ...
    lines: list[str] = []
    lines.append(
        "=== v0.53e substrate-axis disambiguation audit (starting-energy=100 tick-200) ===\n"
    )
    ...
    lines.append(
        ...
        "(A_null_V0_25 tick-50 vs v0.48 / v0.53 / v0.53b / v0.53c / v0.53d published):\n"
    )
    for br in bridge_re_anchor_tick50:
    ...
    for window in WINDOWS:
        for arm in ARMS:
            ...
            elif window == TICK_200 and arm == ARM_C_WIDENED_STARTING_ENERGY_100:
                tag = " (verdict-gating)"
            else:
                tag = " (descriptive)"
    ...
    lines.append(
        ...
        f"{ARM_C_WIDENED_STARTING_ENERGY_100}={c_tick200_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53e substrate-axis disambiguation audit (starting-energy=100 tick-200) ===")
    ...
    c_tick200_subverdict = _arm_subverdict_tick200(
        ARM_C_WIDENED_STARTING_ENERGY_100,
        summaries_by_arm_label_tick200.get((ARM_C_WIDENED_STARTING_ENERGY_100, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get(
            (ARM_C_WIDENED_STARTING_ENERGY_100, LABEL_B_TICK200_NAME), []
        ),
    )
    ...
    rollup_verdict, rollup_phrase = _evaluate_rollup(
        ...
        c_widened_starting_energy_100_reachability_tick200=(
            reachability_by_window[TICK_200][ARM_C_WIDENED_STARTING_ENERGY_100][0]
        ),
    )

    per_lineage_path = out_dir / "per_run_per_lineage_v053e.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053eReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(
        description=("v0.53e substrate-axis disambiguation audit (starting-energy=100 tick-200)")
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53e-substrate-axis-starting-energy-100"),
        help=(
            "Directory for v0.53e outputs "
            "(default: runs/v0.53e-substrate-axis-starting-energy-100)."
        ),
    )
