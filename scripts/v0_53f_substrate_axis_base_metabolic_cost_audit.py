"""v0.53f substrate-axis disambiguation audit (founder per-tick metabolic-cost rescue probe).

Pre-reg: [[docs/experiments/fear_hunger_v0.53f.md]]. Question: is v0.53c /
v0.53d / v0.53e's reachability-bound result on ``widened_gradient`` driven by
the per-tick metabolic depletion rate? Specifically: does lowering
``BodyConfig.base_metabolic_cost`` from the V0_25 baseline of ``0.25`` to
``0.10`` (a 60% reduction; the orthogonal founder-facing budget axis to
v0.53e's ``starting_energy=100``), with all other V0_25 substrate parameters
held identical (including ``starting_energy=60.0``), lift ``widened_gradient``
reachability above the locked 25% threshold at tick-200 AND admit a measurable
v0.48 sensor_radius spatial / foraging bridge?

Corpus (locked, identical shape to v0.53 / v0.53b / v0.53c / v0.53d / v0.53e):
A_null arm only across v0.42 / v0.43R / v0.44 / v0.45, hazards {0, 8}, seeds
41..72. 64 runs per arm; 3 arms (asymmetric BodyConfig); 192 runs total.
...
Arms (locked, 3 — asymmetric BodyConfig):
  ...
  C_widened_metabolic_010:       widened_gradient_layout() +
                                 body_config=BodyConfig(base_metabolic_cost=0.10)

A and B preserve V0_25 baseline (default ``BodyConfig()``: starting_energy=60.0,
base_metabolic_cost=0.25). C differs from B by exactly one knob
(``BodyConfig.base_metabolic_cost``: 0.25 -> 0.10, a 60% reduction). All other
BodyConfig fields preserved at default on C, including ``starting_energy=60.0``
(v0.53e's ``starting_energy=100`` is NOT carried forward — v0.53f isolates the
metabolic-cost axis). All three arms preserve V0_25's ``ambient_influx_rate=1.0``,
...
Tick-50 cells anchor v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e
(Tier-1 priority 2.a). Tick-100 cells are descriptive. Tick-200 cells are
verdict-gating on the C arm.

Labels:
  Label A: high_sensor_radius_lineage. Trait-resolved (identical to v0.48-v0.53e).
...
Tier-2 categorical anchor (locked, pre-data):
  ...
  (categorical, not float-tolerance — locked from v0.53c / v0.53d / v0.53e's
  measured 0/64 result.)

Verdict-gating reachability (locked, pre-data):
  c_widened_metabolic_010_reachability_run_share_tick_200  vs threshold 0.25.

Per-arm tick-200 sub-verdicts (locked, 4-way each — UNCHANGED from v0.53d/e):
  ...
  C_METABOLIC_010_TICK200_BRIDGE_{PRESENT, PARTIAL, NOT_FOUND, OPPOSITE_SIGN_HALT}
...
REACHABILITY-GATED, NEW vs v0.53e):
...
forward and supports v0.53f's BodyConfig override directly.

Usage:
    uv run python scripts/v0_53f_substrate_axis_base_metabolic_cost_audit.py
    uv run python scripts/v0_53f_substrate_axis_base_metabolic_cost_audit.py \
        --out-dir runs/v0.53f-substrate-axis-base-metabolic-cost-010
"""
...
ARM_C_WIDENED_METABOLIC_010: str = "C_widened_metabolic_010"
ARMS: tuple[str, ...] = (ARM_A_NULL_V025, ARM_B_WIDENED_V025, ARM_C_WIDENED_METABOLIC_010)

LAYOUT_NAME_BY_ARM: dict[str, str] = {
    ARM_A_NULL_V025: "tight_gradient",
    ARM_B_WIDENED_V025: "widened_gradient",
    ARM_C_WIDENED_METABOLIC_010: "widened_gradient",
}
...
V0_25_AMBIENT_INFLUX_RATE: float = 1.0  # baseline (preserved on ALL arms in v0.53f).
...
BASE_METABOLIC_COST_010_DOSE: float = 0.10  # v0.53f test dose (60% reduction)
...
BODY_BASE_METABOLIC_COST_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_METABOLIC_010: BASE_METABOLIC_COST_010_DOSE,
}
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    if arm in (ARM_B_WIDENED_V025, ARM_C_WIDENED_METABOLIC_010):
        return widened_gradient_layout()
    msg = f"v0.53f: unknown arm {arm!r}"
    raise V053fReducerError(msg)
...
def _body_config_for_arm(arm: str) -> BodyConfig | None:
    ...
    ``BodyConfig(base_metabolic_cost=0.10)`` — all other BodyConfig fields
    preserved at default (including starting_energy=60.0; v0.53e's
    starting_energy=100 is NOT carried forward).
    """
    if arm in (ARM_A_NULL_V025, ARM_B_WIDENED_V025):
        return None
    if arm == ARM_C_WIDENED_METABOLIC_010:
        return BodyConfig(base_metabolic_cost=BASE_METABOLIC_COST_010_DOSE)
    msg = f"v0.53f: unknown arm {arm!r}"
    raise V053fReducerError(msg)
...
# v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e published bridge cells
...
SUBVERDICT_C_METABOLIC_010_TICK200_PRESENT = "C_METABOLIC_010_TICK200_BRIDGE_PRESENT"
SUBVERDICT_C_METABOLIC_010_TICK200_PARTIAL = "C_METABOLIC_010_TICK200_BRIDGE_PARTIAL"
SUBVERDICT_C_METABOLIC_010_TICK200_NOT_FOUND = "C_METABOLIC_010_TICK200_BRIDGE_NOT_FOUND"
SUBVERDICT_C_METABOLIC_010_TICK200_OPPOSITE = "C_METABOLIC_010_TICK200_OPPOSITE_SIGN_HALT"
...
ROLLUP_WIDENED_BRIDGE_RESCUED_BY_BASE_METABOLIC_COST_010 = (
    "WIDENED_BRIDGE_RESCUED_BY_BASE_METABOLIC_COST_010"
)
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010 = (
    "WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010"
)
ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_BASE_METABOLIC_COST_010 = (
    "WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_BASE_METABOLIC_COST_010"
)
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53f's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_ANCHOR_REPLICATION_HALT: (
        "Halt: v0.53f's A_null_V0_25 arm does not reproduce v0.48 / v0.53 / v0.53b / "
        "v0.53c / v0.53d / v0.53e's tick-50 spatial bridge, OR v0.53f's "
        "B_widened_V0_25 arm does not reproduce v0.53c / v0.53d / v0.53e's `0/64` "
        "reachability lock at tick-200. v0.53f cannot interpret the "
        "C_widened_metabolic_010 cells without an established baseline on the V0_25 "
        "substrate."
    ),
    ROLLUP_RELAXED_OPPOSITE_SIGN_HALT: (
        "Halt: a v0.53f C_widened_metabolic_010 tick-200 spatial / foraging primary "
        "fires in the WRONG direction under a gating label, AND C reachability at "
        "tick-200 clears the locked 25% threshold (so the bridge framework is "
        "meaningful at this measurement). Lowering `BodyConfig.base_metabolic_cost` "
        "from V0_25 baseline `0.25` to `0.10` on the `widened_gradient` layout "
    ...
    ROLLUP_WIDENED_BRIDGE_RESCUED_BY_BASE_METABOLIC_COST_010: (
        ...
        "for `BodyConfig.base_metabolic_cost = 0.10` (a 60% reduction from the V0_25 "
        "baseline of `0.25` per tick) on `widened_gradient`, the v0.48 sensor_radius "
        "spatial / foraging bridge fires PRESENT under the locked +0.5 paired_d "
        ...
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX`, and v0.53e "
        "locked as `RELAXED_OPPOSITE_SIGN_HALT` (with extended-survival-without-"
        "reachability secondary finding) admits a measurable bridge under the "
        "relaxed-metabolic-cost envelope: B_widened_V0_25 reachability remains "
        "`0/64` at tick-200 (predecessor lock holds), C_widened_metabolic_010 "
        "reachability clears the locked 25% threshold, and ≥ 2/3 spatial / foraging "
        "primaries fire PRESENT under both gating labels. The (V0_25 × "  # noqa: RUF001
        "`widened_gradient`) reachability ceiling is bounded by the V0_25 founder "
        "per-tick metabolic depletion rate, not by the geometry alone: "
        "`WIDENED_BRIDGE_RESCUED_BY_BASE_METABOLIC_COST_010`."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010: (
        "On the modern A_null corpus with the V0_25 substrate held constant except "
        "for `BodyConfig.base_metabolic_cost = 0.10` (a 60% reduction from the V0_25 "
        "baseline of `0.25` per tick) on `widened_gradient`, fewer than 25% of "
        "C_widened_metabolic_010 runs have any founder lineage with pre200 food "
        "events. C_widened_metabolic_010's reachability is below the locked 25% "
        "threshold at tick-200; the geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200`, v0.53d locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX`, and v0.53e "
        "locked as `RELAXED_OPPOSITE_SIGN_HALT` remains below the reachability "
        "threshold under the relaxed-metabolic-cost envelope. The bridge is not "
        "rescued by `BodyConfig.base_metabolic_cost = 0.10` under V0_25 × "  # noqa: RUF001
        "`widened_gradient` × N_TICKS=200; v0.53g multi-knob co-variation "  # noqa: RUF001
        "(`starting_energy=100 + base_metabolic_cost=0.10` together) remains the "
        "natural follow-up: "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010`."
    ),
    ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_BASE_METABOLIC_COST_010: (
        "On the modern A_null corpus with the V0_25 substrate held constant except "
        "for `BodyConfig.base_metabolic_cost = 0.10` (a 60% reduction from the V0_25 "
        "baseline of `0.25` per tick) on `widened_gradient`, "
        "C_widened_metabolic_010's reachability clears the locked 25% threshold at "
        "tick-200 but the v0.48 sensor_radius spatial / foraging bridge does not "
        "fully fire PRESENT — fewer than 2/3 primaries fire across both gating "
        "labels at tick-200 under the strict NaN rule. The geometry/substrate cell "
        "that v0.53c locked as `WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200`, "
        "v0.53d locked as `WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_"
        "INFLUX`, and v0.53e locked as `RELAXED_OPPOSITE_SIGN_HALT` is partially "
        "rescued under the relaxed-metabolic-cost envelope: reachability becomes "
        "measurable but the bridge does not fully replicate. Layout-specific "
        "partial-rescue logged in Results: "
        "`WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_BASE_METABOLIC_COST_010`."
    ),
}


class V053fReducerError(Exception):
    """Halt condition raised when a v0.53f invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53f audit (read-only).

    Captures founder ``body.energy`` AND the model-level
    ``body_config.base_metabolic_cost`` at setup_observer time (read-only).
    Used by Test #5 to verify the ``body_config`` seam took effect at
    config-time founder construction — B founders observe
    ``base_metabolic_cost == 0.25`` AND ``energy == 60.0`` (V0_25 baseline);
    C founders observe ``base_metabolic_cost == 0.10`` (overridden) AND
    ``energy == 60.0`` (V0_25 baseline preserved — only base_metabolic_cost
    differs on C).
    """
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53f: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    Reads ``body.energy`` AND the model-level ``body_config.base_metabolic_cost``
    at tick-0 to verify the v0.53f ``body_config`` seam propagated to founder
    construction (B arm should observe base_metabolic_cost=0.25 and
    energy=60.0; C arm should observe base_metabolic_cost=0.10 and energy=60.0).
    Strictly read-only on every founder body field and on model.body_config.
    """
    founders = sorted(model.agents, key=lambda a: int(a.body.lineage_id))
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53f invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53f layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(
    ...
    Order (mirrors v0.53c / v0.53d / v0.53e merged implementation):
      (a) capture v0.53f founder audit table from live bodies (read-only;
          INCLUDES body.energy AND body_config.base_metabolic_cost at tick 0
          to verify the body_config seam).
    ...
    model.body_config — no patching, no clamps, no overrides. The
    base_metabolic_cost=0.10 change propagates via the body_config parameter
    to run_chamber() at config-time founder construction, NOT via any
    post-construction body patch.
    """
...
def _run_one_arm(arm: str, version: str, seed: int, hazard: int, runs_root: Path) -> _RunCapture:
    ...
    ``base_metabolic_cost=0.25``, ``starting_energy=60.0``). C overrides
    ``body_config=BodyConfig(base_metabolic_cost=0.10)`` — all other
    BodyConfig fields preserved at default (including starting_energy=60.0).
    All other WorldConfig / ReproductionConfig knobs are inherited
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53f: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53f-{arm}-{version}-A_null-hzd{hazard}",
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53f: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053fReducerError(msg)
...
_SUBVERDICT_TICK200_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_WIDENED_METABOLIC_010: (
        SUBVERDICT_C_METABOLIC_010_TICK200_PRESENT,
        SUBVERDICT_C_METABOLIC_010_TICK200_PARTIAL,
        SUBVERDICT_C_METABOLIC_010_TICK200_NOT_FOUND,
        SUBVERDICT_C_METABOLIC_010_TICK200_OPPOSITE,
    ),
}
...
def _arm_subverdict_tick200(
    ...
    if arm not in _SUBVERDICT_TICK200_TABLE:
        msg = f"v0.53f: unknown arm {arm!r} in tick-200 subverdict"
        raise V053fReducerError(msg)
...
def _check_bridge_re_anchor_tick50(
    ...
    v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e."""
...
def _collect_c_tick200_wrong_sign_cells(
    ...
    for s in summaries_tick200:
        if s.arm != ARM_C_WIDENED_METABOLIC_010:
            continue
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    c_widened_metabolic_010_reachability_tick200: float,
) -> tuple[str, str]:
    ...
    BOTH:
      - C_widened_metabolic_010 tick-200 sub-verdict == OPPOSITE_SIGN_HALT, AND
      - c_widened_metabolic_010_reachability_run_share_tick_200 >= 0.25.
    ...
    Priorities 4/5/6 gate on C_widened_metabolic_010 reachability +
    sub-verdict only.
    """
    ...
    if (
        c_tick200_subverdict == SUBVERDICT_C_METABOLIC_010_TICK200_OPPOSITE
        and c_widened_metabolic_010_reachability_tick200 >= B_REACHABILITY_THRESHOLD
    ):
        return (
            ROLLUP_RELAXED_OPPOSITE_SIGN_HALT,
    ...
    if c_widened_metabolic_010_reachability_tick200 < B_REACHABILITY_THRESHOLD:
    ...
    if c_tick200_subverdict == SUBVERDICT_C_METABOLIC_010_TICK200_PRESENT:
        # Priority 4: rescued.
        return (
            ROLLUP_WIDENED_BRIDGE_RESCUED_BY_BASE_METABOLIC_COST_010,
            LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_RESCUED_BY_BASE_METABOLIC_COST_010],
        )
    # Priority 6: partially rescued (C sub-verdict in {PARTIAL, NOT_FOUND}).
    return (
        ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_BASE_METABOLIC_COST_010,
        LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_BASE_METABOLIC_COST_010],
    )
    ...
    for arm, (reach, n) in arm_reach.items():
        ...
        writer.writerow(
            [
                ...
                _format_value(is_gating and arm == ARM_C_WIDENED_METABOLIC_010),
            ]
        )
...
def _write_audit_summary_csv(
    ...
    with path.open("w", newline="") as f:
        ...
        # NEW v0.53f: wrong_sign_cells_under_reachability_below_threshold.
        ...
        writer.writerow(["sub_verdicts_tick200", ARM_C_WIDENED_METABOLIC_010, c_tick200_subverdict])
...
def _write_audit_log(  # noqa: PLR0912
    ...
    lines: list[str] = []
    lines.append(
        "=== v0.53f substrate-axis disambiguation audit (base_metabolic_cost=0.10 tick-200) ===\n"
    )
    ...
    lines.append(
        ...
        "(A_null_V0_25 tick-50 vs v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e published):\n"
    )
    for br in bridge_re_anchor_tick50:
    ...
    for window in WINDOWS:
        for arm in ARMS:
            ...
            elif window == TICK_200 and arm == ARM_C_WIDENED_METABOLIC_010:
                tag = " (verdict-gating)"
            else:
                tag = " (descriptive)"
    ...
    if wrong_sign_cells:
        lines.append(
            "\nWrong-sign cells under C_widened_metabolic_010 tick-200 "
    ...
    lines.append(
        ...
        f"{ARM_C_WIDENED_METABOLIC_010}={c_tick200_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53f substrate-axis disambiguation audit (base_metabolic_cost=0.10 tick-200) ===")
    ...
    c_tick200_subverdict = _arm_subverdict_tick200(
        ARM_C_WIDENED_METABOLIC_010,
        summaries_by_arm_label_tick200.get((ARM_C_WIDENED_METABOLIC_010, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get((ARM_C_WIDENED_METABOLIC_010, LABEL_B_TICK200_NAME), []),
    )
    ...
    c_reach_t200 = reachability_by_window[TICK_200][ARM_C_WIDENED_METABOLIC_010][0]
    ...
    rollup_verdict, rollup_phrase = _evaluate_rollup(
        ...
        c_widened_metabolic_010_reachability_tick200=c_reach_t200,
    )

    per_lineage_path = out_dir / "per_run_per_lineage_v053f.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053fReducerError(rollup_phrase)
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53f-substrate-axis-base-metabolic-cost-010"),
        help=(
            "Directory for v0.53f outputs "
            "(default: runs/v0.53f-substrate-axis-base-metabolic-cost-010)."
        ),
    )
