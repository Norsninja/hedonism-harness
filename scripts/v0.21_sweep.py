"""v0.21 influx-frontier sweep driver.

Runs ``V0_21_ARMS`` (5 arms) x 2 chambers (tight_gradient, food_ladder)
x 8 seeds = 80 runs, persisting per-arm comparison.csv into
``runs/fear-hunger-v0.21-{chamber}/``. Pre-reg:
``docs/experiments/fear_hunger_v0.21.md``.

Usage:
    uv run python scripts/v0.21_sweep.py
"""
...
from hedonism_harness.experiments.comparison_grid import V0_21_ARMS, run_comparison_grid
...
def main() -> None:
    ...
    for chamber in CHAMBERS:
        batch_id = f"fear-hunger-v0.21-{chamber}"
        ...
        # Headline table mirrors v0.20 plus an "influx" column up front so
        # the frontier curve reads top-to-bottom. pool_min and the
        # in_influx accumulator are both included so the H1 invariant
        # check is visible at a glance.
        for agg in aggregates:
            ...
            print(
                ...
                f"influx={agg.total_pool_in_ambient_influx:6.0f} "
