"""v0.53 cross-layout generalization audit (3-arm observational bridge probe).

Pre-reg: [[docs/experiments/fear_hunger_v0.53.md]]. Question: does the v0.48
sensor_radius spatial / foraging bridge — established on the ``tight_gradient``
(V0_25) layout — generalize to ``widened_gradient`` and ``food_ladder``
layout geometries when the V0_25 substrate is otherwise held constant?

Corpus (locked, same substrate as v0.46-v0.52b): A_null arm only across
...
patch, no override, no helper RNG, no permutation). The layout swap is the
only configured difference. Runtime invariant: setup_observer reads back
``model.world.width`` and asserts it matches the arm's expected layout
width; halts loud (V053ReducerError) on mismatch.

Primary observables (locked, identical to v0.48-v0.52b):
  pre50_food_events_count           (+)
  pre50_food_energy_acquired        (+)
  mean_distance_to_nearest_food_cell(-)

Labels (both gate for all three arms):
  Label A: argmax(int(body.traits.sensor_radius)), tiebreak min(lineage_id).
           Identical to v0.48-v0.52 Label A. NOT v0.52b's
           ``label_a_information_radius`` — under v0.53 every founder has
           ``effective_sensor_radius_override = None`` so the resolver
           falls through to trait identically across arms.
  Label B: identical to v0.47-v0.52b (3-tier readiness fraction tiebreak).
...
Per-arm sub-verdicts (locked):
  A_null_V0_25:       A_NULL_V025_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      A_NULL_V025_OPPOSITE_SIGN_HALT
  B_widened_gradient: B_WIDENED_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      B_WIDENED_OPPOSITE_SIGN_HALT
  C_food_ladder:      C_LADDER_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      C_LADDER_OPPOSITE_SIGN_HALT

Slice rollup (locked, 6 outcomes, priority-ordered):
  1. CORPUS_REDERIVE_DRIFT_HALT          (A_null_V0_25 a_share_h8 drift > 1e-3)
  2. BRIDGE_REPLICATION_HALT             (A_null_V0_25 vs v0.48; PRESENT required)
  3. LAYOUT_OPPOSITE_SIGN_HALT           (any signed_d <= -0.5 under arm B or C)
  4. BRIDGE_GENERALIZES_CROSS_LAYOUT     (B PRESENT, C PRESENT)
  5. BRIDGE_LAYOUT_LOCAL                 (B NOT_FOUND, C NOT_FOUND)
  6. BRIDGE_PARTIALLY_GENERALIZES        (any other (B, C) over {PRESENT, PARTIAL, NOT_FOUND})

Conservation framing: no `src/` modifications; `widened_gradient_layout()`
and `food_ladder_layout()` already exist in
`src/hedonism_harness/experiments/layouts.py`. v0.53 sets no override and
performs no patch — every founder's `effective_sensor_radius_override` is
None.

Usage:
    uv run python scripts/v0_53_cross_layout_generalization_audit.py
    uv run python scripts/v0_53_cross_layout_generalization_audit.py \
        --out-dir runs/v0.53-cross-layout-generalization
"""
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    msg = f"v0.53: unknown arm {arm!r}"
    raise V053ReducerError(msg)
...
# Primary observables (identical to v0.48-v0.52b).
...
# v0.53's Label A under A_null_V0_25 resolves to int(body.traits.sensor_radius)
# (override is None) — identical to v0.48-v0.52 Label A.
...
SUBVERDICT_A_NULL_V025_PRESENT = "A_NULL_V025_BRIDGE_PRESENT"
SUBVERDICT_A_NULL_V025_PARTIAL = "A_NULL_V025_BRIDGE_PARTIAL"
SUBVERDICT_A_NULL_V025_NOT_FOUND = "A_NULL_V025_BRIDGE_NOT_FOUND"
SUBVERDICT_A_NULL_V025_OPPOSITE = "A_NULL_V025_OPPOSITE_SIGN_HALT"

SUBVERDICT_B_WIDENED_PRESENT = "B_WIDENED_BRIDGE_PRESENT"
SUBVERDICT_B_WIDENED_PARTIAL = "B_WIDENED_BRIDGE_PARTIAL"
SUBVERDICT_B_WIDENED_NOT_FOUND = "B_WIDENED_BRIDGE_NOT_FOUND"
SUBVERDICT_B_WIDENED_OPPOSITE = "B_WIDENED_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_LADDER_PRESENT = "C_LADDER_BRIDGE_PRESENT"
SUBVERDICT_C_LADDER_PARTIAL = "C_LADDER_BRIDGE_PARTIAL"
SUBVERDICT_C_LADDER_NOT_FOUND = "C_LADDER_BRIDGE_NOT_FOUND"
SUBVERDICT_C_LADDER_OPPOSITE = "C_LADDER_OPPOSITE_SIGN_HALT"
...
ROLLUP_BRIDGE_GENERALIZES = "BRIDGE_GENERALIZES_CROSS_LAYOUT"
ROLLUP_BRIDGE_LAYOUT_LOCAL = "BRIDGE_LAYOUT_LOCAL"
ROLLUP_BRIDGE_PARTIALLY_GENERALIZES = "BRIDGE_PARTIALLY_GENERALIZES"
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_BRIDGE_REPLICATION_HALT: (
        "Halt: v0.53's A_null_V0_25 arm does not reproduce v0.48's spatial bridge — "
        "either a paired_d cell drifts beyond 1e-3 of the published value, or the "
        "A_null_V0_25 sub-verdict does not resolve to PRESENT. v0.53 cannot interpret "
        "the cross-layout arms without an established baseline."
    ),
    ROLLUP_LAYOUT_OPPOSITE_HALT: (
        "Halt: a v0.53 spatial / foraging primary fires in the WRONG direction under "
        "a gating label on a non-V0_25 layout; the layout swap is incompatible with "
        "the locked expected signs and the cross-layout interpretation cannot proceed "
        "without isolating the regime change."
    ),
    ROLLUP_BRIDGE_GENERALIZES: (
        "On the modern A_null corpus with the V0_25 substrate held constant, the v0.48 "
        "spatial / foraging bridge fires PRESENT on both `widened_gradient` and "
        "`food_ladder` under the locked +0.5 paired_d threshold. The bridge generalizes "
        "across the three tested layout geometries (`tight_gradient`, "
        "`widened_gradient`, `food_ladder`) on this corpus."
    ),
    ROLLUP_BRIDGE_LAYOUT_LOCAL: (
        "On the modern A_null corpus with the V0_25 substrate held constant, the v0.48 "
        "spatial / foraging bridge does not fire PRESENT on either `widened_gradient` "
        "or `food_ladder` under the locked +0.5 paired_d threshold. The bridge is local "
        "to the `tight_gradient` (V0_25) layout geometry on this corpus; it does not "
        "generalize to `widened_gradient` or `food_ladder`."
    ),
    ROLLUP_BRIDGE_PARTIALLY_GENERALIZES: (
        "On the modern A_null corpus with the V0_25 substrate held constant, the v0.48 "
        "spatial / foraging bridge does not resolve to a single categorical outcome "
        "across `widened_gradient` and `food_ladder` under the locked +0.5 paired_d "
        "threshold. The bridge partially generalizes across the three tested layout "
        "geometries; the asymmetric pattern (per-arm sub-verdicts) is logged in Results."
    ),
}


class V053ReducerError(Exception):
    """Halt condition raised when a v0.53 invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53 audit (read-only)."""
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53 invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53 layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(
    ...
    def setup(model: HHModel) -> None:
        ...
        # AgentBorn listener: lineage tracking only (v0.48-v0.51 pattern).
        # No descendant audit on v0.53 — no override is set on any founder.
...
def _run_one_arm(arm: str, version: str, seed: int, hazard: int, runs_root: Path) -> _RunCapture:
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53-{arm}-{version}-A_null-hzd{hazard}",
    ...
    if len(capture.tick_records) != expected_ticks:
        msg = (
            f"v0.53 invariant: per-tick observer must capture {expected_ticks} ticks for "
            f"{run_id}; got {len(capture.tick_records)}"
        )
        raise V053ReducerError(msg)
    if len(capture.founder_records) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53 invariant: expected exactly {EXPECTED_FOUNDERS} founder records for "
...
def _select_sensor_radius_label(
    ...
    v0.53 reads the trait directly — every founder has
    ``effective_sensor_radius_override = None``, so the v0.52b
    override-or-trait resolver falls through to ``int(traits.sensor_radius)``
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053ReducerError(msg)
...
_SUBVERDICT_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_A_NULL_V025: (
        SUBVERDICT_A_NULL_V025_PRESENT,
        SUBVERDICT_A_NULL_V025_PARTIAL,
        SUBVERDICT_A_NULL_V025_NOT_FOUND,
        SUBVERDICT_A_NULL_V025_OPPOSITE,
    ),
    ARM_B_WIDENED: (
        SUBVERDICT_B_WIDENED_PRESENT,
        SUBVERDICT_B_WIDENED_PARTIAL,
        SUBVERDICT_B_WIDENED_NOT_FOUND,
        SUBVERDICT_B_WIDENED_OPPOSITE,
    ),
    ARM_C_LADDER: (
        SUBVERDICT_C_LADDER_PRESENT,
        SUBVERDICT_C_LADDER_PARTIAL,
        SUBVERDICT_C_LADDER_NOT_FOUND,
        SUBVERDICT_C_LADDER_OPPOSITE,
    ),
}
...
def _arm_subverdict(
    ...
    """Per-arm 4-way sub-verdict; both labels gate for all arms in v0.53."""
    if arm not in _SUBVERDICT_TABLE:
        msg = f"v0.53: unknown arm {arm!r} in subverdict"
        raise V053ReducerError(msg)
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    # Priority 2: bridge replication halt (A_null_V0_25 vs v0.48; PRESENT required).
    ...
    if a_null_subverdict != SUBVERDICT_A_NULL_V025_PRESENT:
        return ROLLUP_BRIDGE_REPLICATION_HALT, LOCKED_PHRASES[ROLLUP_BRIDGE_REPLICATION_HALT]

    # Priority 3: layout opposite-sign halt (B or C, NOT A_null_V0_25).
    if (
        b_subverdict == SUBVERDICT_B_WIDENED_OPPOSITE
        or c_subverdict == SUBVERDICT_C_LADDER_OPPOSITE
    ):
        return ROLLUP_LAYOUT_OPPOSITE_HALT, LOCKED_PHRASES[ROLLUP_LAYOUT_OPPOSITE_HALT]

    # Outcome conditions (no halt fired). A_null_V0_25 must be PRESENT.
    b_is_present = b_subverdict == SUBVERDICT_B_WIDENED_PRESENT
    b_is_not_found = b_subverdict == SUBVERDICT_B_WIDENED_NOT_FOUND
    c_is_present = c_subverdict == SUBVERDICT_C_LADDER_PRESENT
    c_is_not_found = c_subverdict == SUBVERDICT_C_LADDER_NOT_FOUND

    # Priority 4: BRIDGE_GENERALIZES_CROSS_LAYOUT (PRESENT, PRESENT).
    if b_is_present and c_is_present:
        return ROLLUP_BRIDGE_GENERALIZES, LOCKED_PHRASES[ROLLUP_BRIDGE_GENERALIZES]

    # Priority 5: BRIDGE_LAYOUT_LOCAL (NOT_FOUND, NOT_FOUND).
    if b_is_not_found and c_is_not_found:
        return ROLLUP_BRIDGE_LAYOUT_LOCAL, LOCKED_PHRASES[ROLLUP_BRIDGE_LAYOUT_LOCAL]

    # Priority 6: BRIDGE_PARTIALLY_GENERALIZES (catch-all).
    return (
        ROLLUP_BRIDGE_PARTIALLY_GENERALIZES,
        LOCKED_PHRASES[ROLLUP_BRIDGE_PARTIALLY_GENERALIZES],
    )
...
def _write_audit_summary_csv(
    ...
    with path.open("w", newline="") as f:
        ...
        writer.writerow(["sub_verdicts", ARM_A_NULL_V025, a_null_subverdict])
        writer.writerow(["sub_verdicts", ARM_B_WIDENED, b_subverdict])
        writer.writerow(["sub_verdicts", ARM_C_LADDER, c_subverdict])
...
def _write_audit_log(
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("=== v0.53 cross-layout generalization audit ===\n")
    ...
    lines.append("\nTier-1 bridge re-anchor (A_null_V0_25 arm vs v0.48 published signed_d):\n")
    for br in bridge_re_anchor:
    ...
    lines.append(
        f"\nSub-verdicts: {ARM_A_NULL_V025}={a_null_subverdict}  "
        f"{ARM_B_WIDENED}={b_subverdict}  {ARM_C_LADDER}={c_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53 cross-layout generalization audit ===")
    ...
    a_null_subverdict = _arm_subverdict(
        ARM_A_NULL_V025,
        summaries_by_arm_label.get((ARM_A_NULL_V025, LABEL_A_NAME), []),
        summaries_by_arm_label.get((ARM_A_NULL_V025, LABEL_B_NAME), []),
    )
    b_subverdict = _arm_subverdict(
        ARM_B_WIDENED,
        summaries_by_arm_label.get((ARM_B_WIDENED, LABEL_A_NAME), []),
        summaries_by_arm_label.get((ARM_B_WIDENED, LABEL_B_NAME), []),
    )
    c_subverdict = _arm_subverdict(
        ARM_C_LADDER,
        summaries_by_arm_label.get((ARM_C_LADDER, LABEL_A_NAME), []),
        summaries_by_arm_label.get((ARM_C_LADDER, LABEL_B_NAME), []),
    )
    ...
    per_lineage_path = out_dir / "per_run_per_lineage_v053.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053ReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(description="v0.53 cross-layout generalization audit")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53-cross-layout-generalization"),
        help=("Directory for v0.53 outputs (default: runs/v0.53-cross-layout-generalization)."),
    )
