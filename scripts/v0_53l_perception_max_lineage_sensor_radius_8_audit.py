"""v0.53l per-lineage perception heterogeneity probe — 4-arm
asymmetric-horizon reducer with Tier-3 in-slice positive anchor on D.

Pre-reg: [[docs/experiments/fear_hunger_v0.53l.md]]. Question: does
**lineage-specific** perception rescue restore the v0.48 Label A bridge
fingerprint? Specifically: with the script-local ``widened_food_near1``
layout, ``BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)``,
``n_ticks=400``, AND **only the single max-sensor founder per run receiving
``Traits(effective_sensor_radius_override=8)``** (other four founders on
trait-fallback default), does C tick-400 reachability cross 0.25 AND does
the v0.48 Label A bridge fire PRESENT?

Corpus (locked, identical shape to v0.53..v0.53k): A_null arm only across
...
Arms (locked, 4):
  ...
  C_widened_food_near1_combined_max_lineage_sr8_N400:
      <script-local widened_food_near1> +
      body_config=BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10) +
                              # NO model-wide override (v0.53k carried this; v0.53l does NOT)
      per_founder_traits_overrides[max_lid] = Traits(..., effective_sensor_radius_override=8)
                              # per-founder override on the SINGLE max-sensor founder per run
  ...
  D_widened_food_near2_combined_N400:           <script-local widened_food_near2> +
      ...
      n_ticks=400 (IN-SLICE POSITIVE ANCHOR; byte-identical to v0.53i C / v0.53j D / v0.53k D)
...
Anchor stack (locked, 3-tier — Tier-3 carries forward from v0.53j):
...
REACHABILITY-GATED on C tick-400):
  ...
  3. MAX_LINEAGE_PERCEPTION_OPPOSITE_SIGN_HALT  (gates on C tick-400 sub-verdict + reach >= 0.25)
...
Conservation framing: v0.53l carries forward the two v0.53l-locked additive
``src/`` carve-outs (chamber driver ``per_founder_traits_overrides`` parameter
plus model.py always-consume founder-trait sampling invariant) — both pinned
through ``tests/sha_pins.py``. Both ``ChamberLayout`` instances constructed
script-local (NOT added to ``layouts.py``). The per-lineage perception
override is consumed via ``Traits.effective_sensor_radius_override`` (already
exposed since v0.52b) and carried into the chamber via the C-arm-only
``per_founder_traits_overrides`` parameter.

Usage:
    uv run python scripts/v0_53l_perception_max_lineage_sensor_radius_8_audit.py
    uv run python scripts/v0_53l_perception_max_lineage_sensor_radius_8_audit.py \
        --out-dir runs/v0.53l-perception-max-lineage-sensor-radius-8
"""
...
# v0.53l directly targets the same lineage Label A picks for its per-founder
# perception override; the alignment is the central scientific move.
# ---------------------------------------------------------------------------
...
# Reuse v0.53k's Label A selector verbatim via cross-script import. v0.53l
# directly targets the same lineage this selector picks per run (argmax of
# sensor_radius with tiebreak min(lineage_id)). The intervention-to-label
# alignment is the central scientific move of v0.53l.
...
N_TICKS_BY_ARM: dict[str, int] = {
    ...
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_MAX_LINEAGE_SR8_N400: N_TICKS_CD,
...
WINDOWS_BY_ARM: dict[str, tuple[int, ...]] = {
    ...
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_MAX_LINEAGE_SR8_N400: WINDOWS_CD,
...
STARTING_ENERGY_100_DOSE: float = 100.0  # v0.53e/g/h/i dose (carried into v0.53j/k/l C/D)
BASE_METABOLIC_COST_010_DOSE: float = 0.10  # v0.53f/g/h/i dose (carried into v0.53j/k/l C/D)
# v0.53l C-only PER-FOUNDER perception override dose. Unlike v0.53k (where the
# value lived on body_config), v0.53l carries it on the targeted founder's
# ``Traits.effective_sensor_radius_override`` only — applied to the
# single max-sensor founder per run (argmax sensor_radius, tiebreak min
# lineage_id) via the chamber's ``per_founder_traits_overrides`` parameter.
...
# v0.53l: ALL arms have ``None`` here (the model-wide body_config override that
# v0.53k carried on C is GONE in v0.53l; the per-lineage override now lives on
# the targeted founder's Traits via ``per_founder_traits_overrides`` on C).
# The column is preserved in the CSV for compatibility with v0.53k-style
# audit consumers and to make the carve-out delta explicit in the audit_log.
BODY_EFFECTIVE_SENSOR_RADIUS_OVERRIDE_BY_ARM: dict[str, int | None] = {
    ...
    ARM_D_WIDENED_FOOD_NEAR2_COMBINED_N400: None,
}

# Per-arm flag for whether the C-arm per-founder override scheme applies (only
# C). v0.53l routes the override through ``per_founder_traits_overrides``
# targeting the single max-sensor founder per run.
ARM_USES_MAX_LINEAGE_OVERRIDE: dict[str, bool] = {
    ARM_A_NULL_V025: False,
    ARM_B_WIDENED_V025: False,
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_MAX_LINEAGE_SR8_N400: True,
    ARM_D_WIDENED_FOOD_NEAR2_COMBINED_N400: False,
}
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    msg = f"v0.53l: unknown arm {arm!r}"
    raise V053lReducerError(msg)


def _body_config_for_arm(arm: str) -> BodyConfig | None:
    """Per-arm BodyConfig override.

    A and B return ``None`` (default ``BodyConfig()`` fallback). C and D BOTH
    return ``BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)`` —
    NO ``effective_sensor_radius_override`` on C (v0.53k carried this; v0.53l
    drops it; the per-lineage override now lives on the targeted founder's
    Traits via ``per_founder_traits_overrides``). D matches v0.53i C / v0.53j
    D / v0.53k D byte-identically (no override on body_config OR per founder).
    """
...
def build_c_arm_per_founder_overrides(
    *,
    seed: int,
    trait_config: TraitConfig,
    n_founders: int = 5,
) -> tuple[list[Traits | None], int | None]:
    """Pre-sample ``n_founders`` Traits using the same RNG state the chamber
    will use, identify the max-sensor lineage (argmax sensor_radius, tiebreak
    min(lineage_id) — the v0.48 Label A selector reused from v0.53k's
    reducer), and build the per-founder override list.

    Returns ``(overrides, max_lid)``. ``overrides`` is a length-``n_founders``
    list where exactly the element at index ``max_lid`` is a ``Traits``
    instance carrying ``effective_sensor_radius_override = 8``; all other
    entries are ``None`` (the chamber falls back to its own
    ``random_traits(...)`` sampling for those founders).

    Determinism guarantee: ``make_streams(seed)`` is deterministic;
    ``random_traits(config, stream)`` is deterministic given a stream; AND
    the v0.53l always-consume invariant on ``model.py:_spawn_founder``
    guarantees the chamber will internally consume the same ``n_founders``
    mutation-stream draws this function consumes here, producing the same
    sampled ``Traits`` for the four non-overridden founders and discarding
    the sample for the override-targeted founder. The sampled value for the
    max lineage is also discarded by the chamber — the override carries the
    full ``Traits`` for that lineage (so ``sensor_radius`` and all other
    fields come from the SAMPLED instance, with only
    ``effective_sensor_radius_override`` modified via
    ``dataclasses.replace``).
    """
...
ROLLUP_MAX_LINEAGE_PERCEPTION_OPPOSITE_SIGN_HALT = "MAX_LINEAGE_PERCEPTION_OPPOSITE_SIGN_HALT"
ROLLUP_WIDENED_BRIDGE_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8 = (
    "WIDENED_BRIDGE_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8"
)
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_MAX_LINEAGE_SENSOR_RADIUS_8 = (
    "WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_MAX_LINEAGE_SENSOR_RADIUS_8"
)
ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8 = (
    "WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8"
)

# Locked phrases (verbatim per pre-reg, v0.53l). Predecessor stack now NINE
# long: v0.53c, v0.53d, v0.53e, v0.53f, v0.53g, v0.53h, v0.53i, v0.53j,
# v0.53k. These predecessor refs are preserved VERBATIM; only the slice's
# own self-reference (v0.53l) is new.
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53l's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_ANCHOR_REPLICATION_HALT: (
        "Halt: v0.53l's A_null_V0_25 arm does not reproduce v0.48–v0.53k's "  # noqa: RUF001
        "tick-50 spatial bridge, OR v0.53l's B_widened_V0_25 arm does not "
        "reproduce v0.53c–v0.53k's `0/64` reachability lock at tick-200, OR "  # noqa: RUF001
        "v0.53l's D_widened_food_near2_combined_N400 arm does not reproduce "
        ...
        "within 1e-3). v0.53l cannot interpret the "
        "C_widened_food_near1_combined_max_lineage_sr8_N400 cells without "
        "reproducing both the V0_25 baseline anchors and the v0.53i "
        "FOOD_NEAR2 positive anchor."
    ),
    ROLLUP_MAX_LINEAGE_PERCEPTION_OPPOSITE_SIGN_HALT: (
        "Halt: a v0.53l C_widened_food_near1_combined_max_lineage_sr8_N400 "
        "tick-400 spatial / foraging primary fires in the WRONG direction "
        "under a gating label, AND C reachability at tick-400 clears the "
        "locked 25% threshold. The per-lineage perception intervention "
    ...
    ROLLUP_WIDENED_BRIDGE_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8: (
        ...
        "`widened_food_near1` layout (`food_x∈[9,13]`, 1-column corridor at "
        "`x=8`; identical to v0.53j C / v0.53k C) AND the per-lineage "
        "perception intervention `per_founder_traits_overrides[max_lid] = "
        "Traits(effective_sensor_radius_override=8)` (the override applies "
        "ONLY to the single max-`sensor_radius` founder per run, with the "
        "v0.48 `is_high_sensor_radius_lineage` selector — argmax with "
        "min-lineage-id tiebreak — picking the same lineage Label A indexes; "
        "metabolic cost UNAFFECTED per the override's "
        "information-channel-only contract; override propagates to all "
        "descendants via `dataclasses.replace`'s preservation), the v0.48 "
        "sensor_radius spatial / foraging bridge fires PRESENT under the "
        "locked +0.5 paired_d threshold at tick-400 under both gating "
        "labels. The geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200`, v0.53d locked "
        ...
        "locked as `WIDENED_BRIDGE_RESCUED_BY_FOOD_NEAR2`, v0.53j locked "
        "as `WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_FOOD_NEAR1`, and "
        "v0.53k locked as `WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_SENSOR_RADIUS_8` "
        "admits a measurable bridge under the per-lineage perception "
        "intervention: B_widened_V0_25 reachability remains `0/64` at "
        "tick-200 (predecessor lock holds), D_widened_food_near2_combined_N400 "
        "reachability reproduces v0.53i's `35/64` positive anchor at "
        "tick-400 (positive lock holds), C reachability at tick-400 clears "
        "the locked 25% threshold, and ≥ 2/3 spatial / foraging primaries "
        "fire PRESENT under both gating labels at the C tick-400 panel. "
        "**The v0.53k Label A collapse is consistent with model-wide "
        "perception homogenization being the proximate cause of "
        "bridge-fingerprint loss, not sensor-reach itself**: lineage-specific "
        "perception heterogeneity restores Label A under sufficient targeted "
        "access. This is strong evidence but NOT exclusive causality — the "
        "targeted lineage may also drive Label A firing through secondary "
        "channels (asymmetric early survival, reproduction-rate "
        "differential, pleiotropy with other heritable traits). This does "
        "NOT prove `the v0.48 bridge is exclusively trait-mediated`; NOT "
        "`model-wide perception interventions are inferior in all "
        "settings`: `WIDENED_BRIDGE_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8`."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_MAX_LINEAGE_SENSOR_RADIUS_8: (
        "On the modern A_null corpus with the V0_25 substrate held constant "
        "except for the combined founder-facing budget relaxation "
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` AND the "
        "simulation horizon doubled to `n_ticks=400` AND the "
        "`widened_food_near1` layout AND the per-lineage perception "
        "intervention `per_founder_traits_overrides[max_lid] = "
        "Traits(effective_sensor_radius_override=8)`, fewer than 25% of "
        "C runs have any founder lineage with pre400 food events. C's "
        "reachability is below the locked 25% threshold at tick-400; "
        "D_widened_food_near2_combined_N400 reproduces v0.53i's `35/64` "
        "positive anchor at tick-400 in-slice. The single-max-lineage "
        "perception intervention is **insufficient** to lift FOOD_NEAR1 "
        "reachability under the tested envelope. The geometry/substrate cell "
        ...
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_FOOD_NEAR1`, and v0.53k "
        "locked as `WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_SENSOR_RADIUS_8` is "
        "not rescued by the per-lineage perception intervention at "
        "single-max scope. This does NOT prove `lineage-specific perception "
        "is irrelevant` — only that this specific single-lineage scheme "
        "does not clear the threshold. v0.53m (or later) candidates shift "
        "toward broader lineage coverage (top-K, threshold-based) or "
        "policy / hazard-avoidance / movement probes: "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_MAX_LINEAGE_SENSOR_RADIUS_8`."
    ),
    ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8: (
        "On the modern A_null corpus with the V0_25 substrate held constant "
        ...
        "`per_founder_traits_overrides[max_lid] = "
        "Traits(effective_sensor_radius_override=8)` on FOOD_NEAR1 at "
        "`n_ticks=400`, C's reachability clears the locked 25% threshold at "
        "tick-400 but the v0.48 sensor_radius spatial / foraging bridge does "
        "not fully fire PRESENT — fewer than 2/3 primaries fire across both "
        "gating labels at tick-400 under the strict NaN rule. "
        "D_widened_food_near2_combined_N400 reproduces v0.53i's `35/64` "
        "positive anchor at tick-400 in-slice. The single-max-lineage "
        "perception intervention admits measurable food access but does not "
        "fully replicate v0.53i's PRESENT bridge expression under the "
        "matched corpus. Layout-specific partial-rescue logged in Results: "
        "`WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8`."
    ),
}


class V053lReducerError(Exception):
    """Halt condition raised when a v0.53l invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53l audit (read-only)."""
    ...
    # v0.53l NEW: per-founder ``Traits.effective_sensor_radius_override``
    # captured at tick 0. Always ``None`` on A/B/D; ``8`` on the single
    # max-sensor founder of each C run (via ``per_founder_traits_overrides``).
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53l: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53l invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53l layout invariant violated for arm {arm!r}: "
...
def _run_one_arm(  # noqa: PLR0915
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53l: base arm must be A_null only; got "
            f"intervention_kind={base_arm.intervention_kind!r}"
        )
        raise V053lReducerError(msg)

    # v0.53l: C arm is the only arm that uses the per-founder override seam.
    # Pre-sample five founder Traits with the same RNG state the chamber will
    # consume internally, identify the max-sensor lineage, and override that
    # one lineage's Traits with effective_sensor_radius_override=8. The
    # chamber's always-consume invariant (v0.53l carve-out 2) guarantees
    # stream-state alignment between this pre-sampling and the chamber's
    # internal founder construction. A / B / D pass ``None`` (byte-identical
    # to v0.1..v0.53k callers).
    per_founder_overrides: list[Traits | None] | None = None
    if ARM_USES_MAX_LINEAGE_OVERRIDE[arm]:
        per_founder_overrides, _ = build_c_arm_per_founder_overrides(
            seed=seed,
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53l-{arm}-{version}-A_null-hzd{hazard}",
...
class PerLineageRow:
    ...
    body_starting_energy: float
    body_base_metabolic_cost: float
    body_effective_sensor_radius_override: object  # int | None — None on v0.53l everywhere
    # v0.53l NEW columns: per-lineage Traits.effective_sensor_radius_override
    # captured at tick-0 (8 on C max-sensor founder; None elsewhere) and a
    # descriptive cross-check flag for the max-sensor lineage per run.
...
def _aggregate_per_lineage(  # noqa: PLR0912, PLR0915
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53l: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053lReducerError(msg)
...
def _arm_subverdict_at_window(
    ...
    if window == TICK_50:
        ...
        if arm not in _SUBVERDICT_TICK400_TABLE:
            msg = f"v0.53l: tick-400 sub-verdict only defined on C and D; got arm {arm!r}"
            raise V053lReducerError(msg)
        table = _SUBVERDICT_TICK400_TABLE[arm]
    else:
        msg = f"v0.53l: unknown window {window!r}"
        raise V053lReducerError(msg)
...
def _collect_c_tick400_wrong_sign_cells(
    ...
    for s in summaries_tick400:
        if s.arm != ARM_C_WIDENED_FOOD_NEAR1_COMBINED_MAX_LINEAGE_SR8_N400:
            continue
...
def _evaluate_rollup(  # noqa: PLR0911
    *,
    ...
    if (
        ...
        return (
            ROLLUP_MAX_LINEAGE_PERCEPTION_OPPOSITE_SIGN_HALT,
            LOCKED_PHRASES[ROLLUP_MAX_LINEAGE_PERCEPTION_OPPOSITE_SIGN_HALT],
        )
    ...
    if c_tick400_subverdict == SUBVERDICT_C_FOOD_NEAR1_TICK400_PRESENT:
        return (
            ROLLUP_WIDENED_BRIDGE_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8,
            LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8],
        )
    return (
        ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8,
        LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_MAX_LINEAGE_SENSOR_RADIUS_8],
    )
...
def compute_slice_rollup_verdict(
    *,
    ...
    v0.53l carries forward v0.53j/k's **Tier-3 in-slice positive anchor on D**: D tick-400
    ...
    for arm, (reach, n) in arm_reach.items():
        ...
        writer.writerow(
            [
                ...
                _format_value(
                    is_gating and arm == ARM_C_WIDENED_FOOD_NEAR1_COMBINED_MAX_LINEAGE_SR8_N400
                ),
            ]
        )
...
def _write_audit_log(  # noqa: PLR0912, PLR0915
    ...
    *,
    ...
    lines: list[str] = []
    lines.append(
        "=== v0.53l perception max-lineage sensor_radius_8 audit (4-arm; n_ticks=400 on C/D) ===\n"
    )
    ...
    for window in WINDOWS_ALL:
        for arm in ARMS:
            ...
            if window == TICK_200 and arm == ARM_B_WIDENED_V025:
                ...
                window == TICK_400 and arm == ARM_C_WIDENED_FOOD_NEAR1_COMBINED_MAX_LINEAGE_SR8_N400
            ):
                tag = " (verdict-gating)"
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53l perception max-lineage sensor_radius_8 audit (4-arm; n_ticks=400 on C/D) ===")
    ...
    per_lineage_path = out_dir / "per_run_per_lineage_v053l.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053lReducerError(rollup_phrase)
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53l-perception-max-lineage-sensor-radius-8"),
        help=(
            "Directory for v0.53l outputs "
            "(default: runs/v0.53l-perception-max-lineage-sensor-radius-8)."
        ),
    )
