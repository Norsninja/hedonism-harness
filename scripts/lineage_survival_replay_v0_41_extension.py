"""v0.35 lineage_survival_replay extension over the v0.41 second fresh seed stream.

Thin wrapper that imports v0.34 + v0.35 helpers via ``importlib.util``
and runs them on a v0.41-only STREAM_CONFIGS (seeds 33..40 at
``runs/fear-hunger-v0.41-tight_gradient/arms/``), writing to a SEPARATE
output directory ``runs/lineage-v0.35-v0_41-fresh/``.

This script does NOT modify [[scripts/lineage_replay.py]],
[[scripts/lineage_survival_replay.py]],
[[scripts/lineage_replay_v0_39_extension.py]], or
[[scripts/lineage_survival_replay_v0_39_extension.py]] in any way. It does
NOT regenerate the sealed 96-row outputs at ``runs/lineage-v0.35/`` or the
sealed 32-row outputs at ``runs/lineage-v0.35-fresh/``.

The v0.41 fresh outputs are anchor sources for
...
Differences from v0.35's main():
  ...
  - v0.34 anchor source is the v0.41-FRESH v0.34 output
    (``runs/lineage-v0.34-v0_41-fresh/run_summary.csv``), NOT the sealed
    96-row v0.34 output, NOT the v0.39-fresh v0.34 output. The v0.41
    v0.34 extension reducer must run before this script.
  - reassert_b_pool_anchors is still called: that asserts the v0.34
    ...
    values, not a corpus check).
  - aggregate_pruning_summary is computed for the v0.41 fresh stream (4 rows).
  - Output dir: runs/lineage-v0.35-v0_41-fresh/.

Pre-reg: [[docs/experiments/fear_hunger_v0.41.md]] section "Mechanism"
subsection "v0.35 extension".
...
OUT_DIR_FRESH: Path = Path("runs/lineage-v0.35-v0_41-fresh")
...
def main() -> None:
    ...
    print(
        f"v0.35 lineage_survival_replay (v0.41 fresh extension): {len(runs)} fresh runs discovered",
        flush=True,
    )

    # Anchor source is the v0.41-FRESH v0.34 extension output.
    anchors = ls.load_v0_34_anchors(path=V0_34_V041_FRESH_RUN_SUMMARY)
    print(
        f"  loaded {len(anchors)} v0.41 fresh v0.34 anchors from {V0_34_V041_FRESH_RUN_SUMMARY}",
        flush=True,
    )
    ...
    print()
    print("v0.41 fresh-stream pruning summary (one row per hazard):")
