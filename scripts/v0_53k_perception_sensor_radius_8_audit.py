"""v0.53k perception sensor_radius_8 audit — 4-arm asymmetric-horizon
reducer with Tier-3 in-slice positive anchor on the D arm.

Pre-reg: [[docs/experiments/fear_hunger_v0.53k.md]]. Question: is the sharp
v0.53j FOOD_NEAR1/FOOD_NEAR2 boundary perception-bound, OR policy /
hazard-avoidance / movement-bound? Specifically: with the script-local
``widened_food_near1`` layout, ``BodyConfig(starting_energy=100.0,
base_metabolic_cost=0.10, effective_sensor_radius_override=8)``, and
``n_ticks=400``, does C tick-400 reachability cross 0.25?

Corpus (locked, identical shape to v0.53..v0.53j): A_null arm only across
...
Arms (locked, 4):
  ...
  C_widened_food_near1_combined_sr8_N400:       <script-local widened_food_near1> +
  ...
  D_widened_food_near2_combined_N400:           <script-local widened_food_near2> +
      body_config=BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10) +
      n_ticks=400 (IN-SLICE POSITIVE ANCHOR; byte-identical to v0.53i C / v0.53j D)
...
REACHABILITY-GATED on C tick-400):
  ...
  3. PERCEPTION_OPPOSITE_SIGN_HALT  (gates on C tick-400 sub-verdict + reach >= 0.25)
...
instances constructed script-local (NOT added to ``layouts.py``). The
perception override is consumed via ``BodyConfig.effective_sensor_radius_override``,
already exposed since v0.52.

Usage:
    uv run python scripts/v0_53k_perception_sensor_radius_8_audit.py
    uv run python scripts/v0_53k_perception_sensor_radius_8_audit.py \
        --out-dir runs/v0.53k-perception-sensor-radius-8
"""
...
ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400: str = "C_widened_food_near1_combined_sr8_N400"
ARM_D_WIDENED_FOOD_NEAR2_COMBINED_N400: str = "D_widened_food_near2_combined_N400"
ARMS: tuple[str, ...] = (
    ARM_A_NULL_V025,
    ARM_B_WIDENED_V025,
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400,
...
LAYOUT_NAME_BY_ARM: dict[str, str] = {
    ...
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400: "widened_food_near1",
...
N_TICKS_BY_ARM: dict[str, int] = {
    ...
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400: N_TICKS_CD,
...
WINDOWS_BY_ARM: dict[str, tuple[int, ...]] = {
    ...
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400: WINDOWS_CD,
...
STARTING_ENERGY_100_DOSE: float = 100.0  # v0.53e/g/h/i test dose (carried into v0.53j/k C/D)
BASE_METABOLIC_COST_010_DOSE: float = 0.10  # v0.53f/g/h/i test dose (carried into v0.53j/k C/D)
EFFECTIVE_SENSOR_RADIUS_OVERRIDE_8_DOSE: int = 8  # v0.53k C-only perception override dose
...
BODY_STARTING_ENERGY_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400: STARTING_ENERGY_100_DOSE,
...
BODY_BASE_METABOLIC_COST_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400: BASE_METABOLIC_COST_010_DOSE,
    ARM_D_WIDENED_FOOD_NEAR2_COMBINED_N400: BASE_METABOLIC_COST_010_DOSE,
}

# Per-arm effective_sensor_radius_override (locked, NEW in v0.53k).
# C is the perception arm (override = 8); A / B / D have None (default).
BODY_EFFECTIVE_SENSOR_RADIUS_OVERRIDE_BY_ARM: dict[str, int | None] = {
    ARM_A_NULL_V025: None,
    ARM_B_WIDENED_V025: None,
    ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400: EFFECTIVE_SENSOR_RADIUS_OVERRIDE_8_DOSE,
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    if arm == ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400:
        return FOOD_NEAR1_LAYOUT
    if arm == ARM_D_WIDENED_FOOD_NEAR2_COMBINED_N400:
        return FOOD_NEAR2_LAYOUT
    msg = f"v0.53k: unknown arm {arm!r}"
    raise V053kReducerError(msg)


def _body_config_for_arm(arm: str) -> BodyConfig | None:
    """Per-arm BodyConfig override.

    A and B return ``None`` (default ``BodyConfig()`` fallback). D returns
    ``BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)`` (NO
    override; matches v0.53i C / v0.53j D). C returns
    ``BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10,
    effective_sensor_radius_override=8)`` — the perception-only override is
    NEW in v0.53k and lives ONLY on the C arm.
    """
...
SUBVERDICT_C_FOOD_NEAR1_TICK50_PRESENT = "C_WIDENED_FOOD_NEAR1_SR8_TICK50_BRIDGE_PRESENT"
SUBVERDICT_C_FOOD_NEAR1_TICK50_PARTIAL = "C_WIDENED_FOOD_NEAR1_SR8_TICK50_BRIDGE_PARTIAL"
SUBVERDICT_C_FOOD_NEAR1_TICK50_NOT_FOUND = "C_WIDENED_FOOD_NEAR1_SR8_TICK50_BRIDGE_NOT_FOUND"
SUBVERDICT_C_FOOD_NEAR1_TICK50_OPPOSITE = "C_WIDENED_FOOD_NEAR1_SR8_TICK50_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_FOOD_NEAR1_TICK100_PRESENT = "C_WIDENED_FOOD_NEAR1_SR8_TICK100_BRIDGE_PRESENT"
SUBVERDICT_C_FOOD_NEAR1_TICK100_PARTIAL = "C_WIDENED_FOOD_NEAR1_SR8_TICK100_BRIDGE_PARTIAL"
SUBVERDICT_C_FOOD_NEAR1_TICK100_NOT_FOUND = "C_WIDENED_FOOD_NEAR1_SR8_TICK100_BRIDGE_NOT_FOUND"
SUBVERDICT_C_FOOD_NEAR1_TICK100_OPPOSITE = "C_WIDENED_FOOD_NEAR1_SR8_TICK100_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_FOOD_NEAR1_TICK200_PRESENT = "C_WIDENED_FOOD_NEAR1_SR8_TICK200_BRIDGE_PRESENT"
SUBVERDICT_C_FOOD_NEAR1_TICK200_PARTIAL = "C_WIDENED_FOOD_NEAR1_SR8_TICK200_BRIDGE_PARTIAL"
SUBVERDICT_C_FOOD_NEAR1_TICK200_NOT_FOUND = "C_WIDENED_FOOD_NEAR1_SR8_TICK200_BRIDGE_NOT_FOUND"
SUBVERDICT_C_FOOD_NEAR1_TICK200_OPPOSITE = "C_WIDENED_FOOD_NEAR1_SR8_TICK200_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_FOOD_NEAR1_TICK400_PRESENT = "C_WIDENED_FOOD_NEAR1_SR8_TICK400_BRIDGE_PRESENT"
SUBVERDICT_C_FOOD_NEAR1_TICK400_PARTIAL = "C_WIDENED_FOOD_NEAR1_SR8_TICK400_BRIDGE_PARTIAL"
SUBVERDICT_C_FOOD_NEAR1_TICK400_NOT_FOUND = "C_WIDENED_FOOD_NEAR1_SR8_TICK400_BRIDGE_NOT_FOUND"
SUBVERDICT_C_FOOD_NEAR1_TICK400_OPPOSITE = "C_WIDENED_FOOD_NEAR1_SR8_TICK400_OPPOSITE_SIGN_HALT"
...
ROLLUP_PERCEPTION_OPPOSITE_SIGN_HALT = "PERCEPTION_OPPOSITE_SIGN_HALT"
ROLLUP_WIDENED_BRIDGE_RESCUED_BY_SENSOR_RADIUS_8 = "WIDENED_BRIDGE_RESCUED_BY_SENSOR_RADIUS_8"
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_SENSOR_RADIUS_8 = (
    "WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_SENSOR_RADIUS_8"
)
ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_SENSOR_RADIUS_8 = (
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53k's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_ANCHOR_REPLICATION_HALT: (
        "Halt: v0.53k's A_null_V0_25 arm does not reproduce v0.48–v0.53j's "  # noqa: RUF001
        "tick-50 spatial bridge, OR v0.53k's B_widened_V0_25 arm does not "
        "reproduce v0.53c–v0.53j's `0/64` reachability lock at tick-200, OR "  # noqa: RUF001
        "v0.53k's D_widened_food_near2_combined_N400 arm does not reproduce "
        ...
        "within 1e-3). v0.53k cannot interpret the "
        "C_widened_food_near1_combined_sr8_N400 cells without reproducing both "
        "the V0_25 baseline anchors and the v0.53i FOOD_NEAR2 positive anchor."
    ),
    ROLLUP_PERCEPTION_OPPOSITE_SIGN_HALT: (
        "Halt: a v0.53k C_widened_food_near1_combined_sr8_N400 tick-400 spatial "
        "/ foraging primary fires in the WRONG direction under a gating label, "
        "AND C reachability at tick-400 clears the locked 25% threshold. The "
        "perception-only intervention (`BodyConfig.effective_sensor_radius_override = 8`) "
        "under FOOD_NEAR1 + combined founder-facing budget envelope at "
        "`n_ticks = 400` surfaces a regime where the locked expected signs do "
        "not hold under measurable food access. The reachability-gated trigger "
    ...
    ROLLUP_WIDENED_BRIDGE_RESCUED_BY_SENSOR_RADIUS_8: (
        ...
        "`x=8`; identical to v0.53j C) AND the perception-only override "
        "`BodyConfig.effective_sensor_radius_override = 8` (the minimum "
        "east-ray reach from `spawn_x=1` to FOOD_NEAR1's `food_x_min=9`; "
        "metabolic cost UNAFFECTED per the override's "
        "information-channel-only contract), the v0.48 sensor_radius spatial "
        "/ foraging bridge fires PRESENT under the locked +0.5 paired_d "
        "threshold at tick-400. The geometry/substrate cell that v0.53c "
        ...
        "locked as `WIDENED_BRIDGE_RESCUED_BY_FOOD_NEAR2`, and v0.53j locked "
        "as `WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_FOOD_NEAR1` admits a "
        "measurable bridge under the perception-only intervention: "
        "B_widened_V0_25 reachability remains `0/64` at tick-200 (predecessor "
        "lock holds), D_widened_food_near2_combined_N400 reachability "
        "reproduces v0.53i's `35/64` positive anchor at tick-400 (positive "
        "lock holds), C reachability at tick-400 clears the locked 25% "
        "threshold, and ≥ 2/3 spatial / foraging primaries fire PRESENT "
        "under both gating labels at the C tick-400 panel. The v0.53j "
        "FOOD_NEAR1/FOOD_NEAR2 boundary is **consistent with sensor-reach "
        "being a binding constraint** under the tested envelope. This is "
        "strong evidence but NOT exclusive causality — sensor-radius "
        "override may alter policy gradients, hazard anticipation, or "
        "food-attraction signal strength beyond simple visibility (per the "
        "`signal += value/distance` gradient in `core/sensors.py:11`). This "
        "does NOT prove `perception is the binding cause` universally; NOT "
        "`policy/hazard/movement are ruled out` (other interventions could "
        "also rescue): `WIDENED_BRIDGE_RESCUED_BY_SENSOR_RADIUS_8`."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_SENSOR_RADIUS_8: (
        "On the modern A_null corpus with the V0_25 substrate held constant "
        "except for the combined founder-facing budget relaxation "
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` AND the "
        "simulation horizon doubled to `n_ticks=400` AND the "
        "`widened_food_near1` layout AND the perception-only override "
        "`BodyConfig.effective_sensor_radius_override = 8`, fewer than 25% of "
        ...
        "positive anchor at tick-400 in-slice. The v0.53j "
        "FOOD_NEAR1/FOOD_NEAR2 boundary is **NOT explained by sensor-reach "
        "alone** under the tested envelope. The geometry/substrate cell that "
        ...
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TIME_HORIZON_400`, v0.53i "
        "locked as `WIDENED_BRIDGE_RESCUED_BY_FOOD_NEAR2`, and v0.53j locked "
        "as `WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_FOOD_NEAR1` is not "
        "rescued by the perception-only intervention at `r=8`. This does NOT "
        "prove `perception is irrelevant` or `the binding cause is policy "
        "specifically` — only that this specific perception override does "
        "not lift reachability under the tested envelope. v0.53l (or later) "
        "candidates shift toward policy / hazard-avoidance / "
    ...
    ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_SENSOR_RADIUS_8: (
        ...
        "perception-only override `effective_sensor_radius_override = 8` on "
        "FOOD_NEAR1 at `n_ticks=400`, C's reachability clears the locked 25% "
        ...
        "D_widened_food_near2_combined_N400 reproduces v0.53i's `35/64` "
        "positive anchor at tick-400 in-slice. The perception override "
        ...
        "Results: `WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_SENSOR_RADIUS_8`."
    ),
}


class V053kReducerError(Exception):
    """Halt condition raised when a v0.53k invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53k audit (read-only)."""
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53k: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53k invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53k layout invariant violated for arm {arm!r}: "
...
def _run_one_arm(  # noqa: PLR0915
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53k: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53k-{arm}-{version}-A_null-hzd{hazard}",
...
class PerLineageRow:
    ...
    body_starting_energy: float
    body_base_metabolic_cost: float
    body_effective_sensor_radius_override: object  # int | None (None on A/B/D, 8 on C)
    n_ticks: int
    version: str
    seed: int
    hazard: int
...
def _aggregate_per_lineage(  # noqa: PLR0912, PLR0915
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53k: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053kReducerError(msg)
...
def _arm_subverdict_at_window(
    ...
    if window == TICK_50:
        ...
        if arm not in _SUBVERDICT_TICK400_TABLE:
            msg = f"v0.53k: tick-400 sub-verdict only defined on C and D; got arm {arm!r}"
            raise V053kReducerError(msg)
        table = _SUBVERDICT_TICK400_TABLE[arm]
    else:
        msg = f"v0.53k: unknown window {window!r}"
        raise V053kReducerError(msg)
...
def _collect_c_tick400_wrong_sign_cells(
    ...
    for s in summaries_tick400:
        if s.arm != ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400:
            continue
...
def _evaluate_rollup(  # noqa: PLR0911
    *,
    ...
    if (
        ...
        return (
            ROLLUP_PERCEPTION_OPPOSITE_SIGN_HALT,
            LOCKED_PHRASES[ROLLUP_PERCEPTION_OPPOSITE_SIGN_HALT],
        )
    ...
    if c_reachability_tick400 < B_REACHABILITY_THRESHOLD:
        return (
            ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_SENSOR_RADIUS_8,
            LOCKED_PHRASES[ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_SENSOR_RADIUS_8],
        )
    if c_tick400_subverdict == SUBVERDICT_C_FOOD_NEAR1_TICK400_PRESENT:
        return (
            ROLLUP_WIDENED_BRIDGE_RESCUED_BY_SENSOR_RADIUS_8,
            LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_RESCUED_BY_SENSOR_RADIUS_8],
        )
    return (
        ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_SENSOR_RADIUS_8,
        LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_SENSOR_RADIUS_8],
    )
...
def compute_slice_rollup_verdict(
    *,
    ...
    v0.53k carries forward v0.53j's **Tier-3 in-slice positive anchor on D**: D tick-400
    ...
    for arm, (reach, n) in arm_reach.items():
        ...
        writer.writerow(
            [
                ...
                _format_value(is_gating and arm == ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400),
            ]
        )
...
def _write_audit_log(  # noqa: PLR0912, PLR0915
    ...
    *,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("=== v0.53k perception sensor_radius_8 audit (4-arm; n_ticks=400 on C/D) ===\n")
    ...
    for window in WINDOWS_ALL:
        for arm in ARMS:
            ...
            elif window == TICK_400 and arm == ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400:
                tag = " (verdict-gating)"
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53k perception sensor_radius_8 audit (4-arm; n_ticks=400 on C/D) ===")
    ...
    c_tick400_subverdict = _arm_subverdict_at_window(
        ...
        summaries_by_arm_label[TICK_400].get(
            (ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400, LABEL_A_NAME), []
        ),
        summaries_by_arm_label[TICK_400].get(
            (ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400, LABEL_B_TICK400_NAME), []
        ),
    )
    ...
    c_reach_t400 = reachability_by_window[TICK_400][ARM_C_WIDENED_FOOD_NEAR1_COMBINED_SR8_N400][0]
    ...
    per_lineage_path = out_dir / "per_run_per_lineage_v053k.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053kReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(
        description="v0.53k perception sensor_radius_8 audit (4-arm; n_ticks=400 on C/D)"
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53k-perception-sensor-radius-8"),
        help=("Directory for v0.53k outputs (default: runs/v0.53k-perception-sensor-radius-8)."),
    )
