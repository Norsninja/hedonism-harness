"""v0.53g substrate-axis disambiguation audit (combined founder starting-energy +
per-tick metabolic-cost rescue probe).

Pre-reg: [[docs/experiments/fear_hunger_v0.53g.md]]. Question: does the
combined effect of v0.53e's and v0.53f's single-knob founder-facing budget
relaxations — ``BodyConfig.starting_energy = 100.0`` AND
``BodyConfig.base_metabolic_cost = 0.10`` applied together on the same C arm —
lift ``widened_gradient`` reachability above the locked 25% threshold at
tick-200 AND admit a measurable v0.48 sensor_radius spatial / foraging bridge?
This is the first multi-knob slice in the v0.53d→v0.53g substrate-axis stack
(v0.53d / v0.53e / v0.53f each varied one knob; v0.53g varies two).

Corpus (locked, identical shape to v0.53 / v0.53b / v0.53c / v0.53d / v0.53e /
v0.53f): A_null arm only across v0.42 / v0.43R / v0.44 / v0.45, hazards {0, 8},
seeds 41..72. 64 runs per arm; 3 arms (asymmetric BodyConfig); 192 runs total.

Arms (locked, 3 — asymmetric BodyConfig):
  A_null_V0_25:                          tight_gradient_layout()   + body_config=None
  B_widened_V0_25:                       widened_gradient_layout() + body_config=None
  C_widened_combined_se100_bmc010:       widened_gradient_layout() +
      body_config=BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)

A and B preserve V0_25 baseline (default ``BodyConfig()``: starting_energy=60.0,
base_metabolic_cost=0.25). C differs from B by exactly TWO knobs
(``BodyConfig.starting_energy``: 60.0 -> 100.0, the maximal legal one-knob
value under max_energy=100.0; AND ``BodyConfig.base_metabolic_cost``: 0.25 ->
0.10, a 60% reduction). All other BodyConfig fields preserved at default on C
(``max_energy=100.0``, ``starting_health=100.0``, ``max_health=100.0``,
``sensor_radius_metabolic_cost=0.05``, ``effective_sensor_radius_override=None``).
All three arms preserve V0_25's ``ambient_influx_rate=1.0``,
``energy_pool_initial=1500.0``, ``food_respawn_cooldown=50``,
``child_funding_mode=PARENT_TRANSFER_POOL_GAP`` baseline.

Primary observables (locked, 3, computed at three windows):
  pre{50,100,200}_food_events_count                     (+)
  pre{50,100,200}_food_energy_acquired                  (+)
  mean_distance_to_nearest_food_cell_tick{50,100,200}   (-)

Tick-50 cells anchor v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e / v0.53f / v0.53f
(Tier-1 priority 2.a). Tick-100 cells are descriptive. Tick-200 cells are
verdict-gating on the C arm.

Labels:
  Label A: high_sensor_radius_lineage. Trait-resolved (identical to v0.48-v0.53f).
  ...
  Label B (tick-200): high_tick200_readiness_fraction_lineage — verdict-gating.
...
Tier-2 categorical anchor (locked, pre-data):
  b_widened_v025_reachability_run_share_tick_200 == 0.0  exactly
  (categorical, not float-tolerance — locked from v0.53c / v0.53d / v0.53e /
  v0.53f's measured 0/64 result.)

Verdict-gating reachability (locked, pre-data):
  c_widened_combined_se100_bmc010_reachability_run_share_tick_200  vs threshold 0.25.

Per-arm tick-200 sub-verdicts (locked, 4-way each — UNCHANGED from v0.53d/e/f):
  A_NULL_V025_TICK200_BRIDGE_{PRESENT, PARTIAL, NOT_FOUND, OPPOSITE_SIGN_HALT}
  B_WIDENED_V025_TICK200_BRIDGE_{PRESENT, PARTIAL, NOT_FOUND, OPPOSITE_SIGN_HALT}
  C_COMBINED_SE100_BMC010_TICK200_BRIDGE_{PRESENT, PARTIAL, NOT_FOUND, OPPOSITE_SIGN_HALT}
A_null_V0_25 tick-50 sub-verdict computed for the priority-2.b trigger.

Slice rollup (locked, 6 outcomes, priority-ordered — PRIORITY 3 IS
REACHABILITY-GATED, carries forward from v0.53f):
  1. CORPUS_REDERIVE_DRIFT_HALT
  2. ANCHOR_REPLICATION_HALT  (Tier-1 A_null_V0_25 tick-50  +  Tier-2 B_widened
                               categorical reachability  +  A_null tick-50 sub-verdict)
  3. RELAXED_OPPOSITE_SIGN_HALT  (reachability-gated: C tick-200 sub-verdict =
                                  OPPOSITE_SIGN_HALT AND C reach >= 0.25)
  4. WIDENED_BRIDGE_RESCUED_BY_COMBINED_SE100_BMC010
        (C reach >= 0.25 AND C tick-200 sub-verdict = PRESENT)
  ...
  6. WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_COMBINED_SE100_BMC010
        (C reach >= 0.25 AND C tick-200 sub-verdict in {PARTIAL, NOT_FOUND})

When C tick-200 sub-verdict = OPPOSITE_SIGN_HALT AND C reach < 0.25:
priority 3 SKIPS; rollup falls through to priority 5; the wrong-sign cell(s)
are logged descriptively in audit_summary.csv under the new
``wrong_sign_cells_under_reachability_below_threshold`` section.

Conservation framing: zero ``src/`` modifications. v0.53e's
``body_config: BodyConfig | None = None`` seam in ``run_chamber()`` carries
forward and supports v0.53g's BodyConfig override directly.

Usage:
    uv run python scripts/v0_53g_substrate_axis_combined_se100_bmc010_audit.py
    uv run python scripts/v0_53g_substrate_axis_combined_se100_bmc010_audit.py \
        --out-dir runs/v0.53g-substrate-axis-combined-se100-bmc010
"""
...
WINDOWS: tuple[int, ...] = (TICK_50, TICK_100, TICK_200)

# Arms (locked, 3 — asymmetric BodyConfig).
ARM_A_NULL_V025: str = "A_null_V0_25"
ARM_B_WIDENED_V025: str = "B_widened_V0_25"
ARM_C_WIDENED_COMBINED_SE100_BMC010: str = "C_widened_combined_se100_bmc010"
ARMS: tuple[str, ...] = (ARM_A_NULL_V025, ARM_B_WIDENED_V025, ARM_C_WIDENED_COMBINED_SE100_BMC010)

LAYOUT_NAME_BY_ARM: dict[str, str] = {
    ARM_A_NULL_V025: "tight_gradient",
    ARM_B_WIDENED_V025: "widened_gradient",
    ARM_C_WIDENED_COMBINED_SE100_BMC010: "widened_gradient",
}
...
V0_25_AMBIENT_INFLUX_RATE: float = 1.0  # baseline (preserved on ALL arms in v0.53g).
...
STARTING_ENERGY_100_DOSE: float = 100.0  # v0.53g test dose (maximal under max_energy=100.0)
BASE_METABOLIC_COST_010_DOSE: float = 0.10  # v0.53g test dose (60% reduction)

# Per-arm body_starting_energy override (locked).
# A and B use V0_25 baseline (60.0); C is the starting_energy=100 variant.
BODY_STARTING_ENERGY_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010: STARTING_ENERGY_100_DOSE,
}

# Per-arm body_base_metabolic_cost override (locked).
# A and B use V0_25 baseline (0.25); C is the metabolic-cost=0.10 variant.
BODY_BASE_METABOLIC_COST_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010: BASE_METABOLIC_COST_010_DOSE,
}
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    if arm in (ARM_B_WIDENED_V025, ARM_C_WIDENED_COMBINED_SE100_BMC010):
        return widened_gradient_layout()
    msg = f"v0.53g: unknown arm {arm!r}"
    raise V053gReducerError(msg)
...
def _body_config_for_arm(arm: str) -> BodyConfig | None:
    ...
    ``BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)`` — TWO
    knobs overridden simultaneously (v0.53e's starting_energy=100 AND v0.53f's
    base_metabolic_cost=0.10 applied together; the first multi-knob slice in
    the v0.53d→v0.53g substrate-axis stack). All other BodyConfig fields
    preserved at default (max_energy=100.0, starting_health=100.0,
    max_health=100.0, sensor_radius_metabolic_cost=0.05,
    effective_sensor_radius_override=None).
    """
    if arm in (ARM_A_NULL_V025, ARM_B_WIDENED_V025):
        return None
    if arm == ARM_C_WIDENED_COMBINED_SE100_BMC010:
        return BodyConfig(
...
# Primary observables — three windows, three observable types.
...
# v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e / v0.53f published bridge cells
# (label, observable) -> signed_d. Tier-1 anchor at tick-50 only.
...
# Per-arm tick-200 sub-verdicts (locked).
...
SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_PRESENT = (
    "C_COMBINED_SE100_BMC010_TICK200_BRIDGE_PRESENT"
)
SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_PARTIAL = (
    "C_COMBINED_SE100_BMC010_TICK200_BRIDGE_PARTIAL"
)
SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_NOT_FOUND = (
    "C_COMBINED_SE100_BMC010_TICK200_BRIDGE_NOT_FOUND"
)
SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_OPPOSITE = (
    "C_COMBINED_SE100_BMC010_TICK200_OPPOSITE_SIGN_HALT"
)

# A_null_V0_25 tick-50 sub-verdict (priority-2.b trigger).
...
ROLLUP_RELAXED_OPPOSITE_SIGN_HALT = "RELAXED_OPPOSITE_SIGN_HALT"
ROLLUP_WIDENED_BRIDGE_RESCUED_BY_COMBINED_SE100_BMC010 = (
    "WIDENED_BRIDGE_RESCUED_BY_COMBINED_SE100_BMC010"
)
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010 = (
    "WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010"
)
ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_COMBINED_SE100_BMC010 = (
    "WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_COMBINED_SE100_BMC010"
)
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53g's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_ANCHOR_REPLICATION_HALT: (
        "Halt: v0.53g's A_null_V0_25 arm does not reproduce v0.48–v0.53f's "  # noqa: RUF001
        "tick-50 spatial bridge, OR v0.53g's B_widened_V0_25 arm does not "
        "reproduce v0.53c–v0.53f's `0/64` reachability lock at tick-200. "  # noqa: RUF001
        "v0.53g cannot interpret the C_widened_combined_se100_bmc010 cells "
        "without an established baseline on the V0_25 substrate."
    ),
    ROLLUP_RELAXED_OPPOSITE_SIGN_HALT: (
        "Halt: a v0.53g C_widened_combined_se100_bmc010 tick-200 spatial / foraging "
        "primary fires in the WRONG direction under a gating label, AND C "
        "reachability at tick-200 clears the locked 25% threshold (so the bridge "
        "framework is meaningful at this measurement). The combined founder-facing "
        "budget relaxation (`BodyConfig.starting_energy = 100.0` AND "
        "`BodyConfig.base_metabolic_cost = 0.10`) on the `widened_gradient` layout "
        "surfaces a regime where the locked expected signs do not hold under "
        "measurable food access. The reachability-gated trigger preserves v0.53e's "
        "locked sign discipline while excluding the v0.53e-style measurement-edge "
        "case (wrong-sign at reachability=0)."
    ),
    ROLLUP_WIDENED_BRIDGE_RESCUED_BY_COMBINED_SE100_BMC010: (
        ...
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` on "
        "`widened_gradient` (starting_energy raised from V0_25 baseline 60.0 to "
        "the maximal legal 100.0 AND base_metabolic_cost lowered from V0_25 "
        "baseline 0.25 to 0.10, a 60% reduction), the v0.48 sensor_radius spatial "
        "/ foraging bridge fires PRESENT under the locked +0.5 paired_d threshold "
        "at tick-200. The geometry/substrate cell that v0.53c locked as "
        ...
        "locked as `RELAXED_OPPOSITE_SIGN_HALT`, and v0.53f locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010` "
        "admits a measurable bridge under the combined founder-facing budget "
        "envelope: B_widened_V0_25 reachability remains `0/64` at tick-200 "
        "(predecessor lock holds), C_widened_combined_se100_bmc010 reachability "
        "clears the locked 25% threshold, and ≥ 2/3 spatial / foraging primaries "
        "fire PRESENT under both gating labels. Neither single-knob axis alone "
        "was sufficient (v0.53e: `starting_energy=100` extended survival ~9% but "
        "reachability=0; v0.53f: `base_metabolic_cost=0.10` extended survival "
        "~16% but reachability=0); the conjunction lifts the reachability "
        "ceiling. The (V0_25 × `widened_gradient`) cell is bounded by the "  # noqa: RUF001
        "combined founder-facing budget envelope, not by the geometry alone: "
        "`WIDENED_BRIDGE_RESCUED_BY_COMBINED_SE100_BMC010`."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010: (
        "On the modern A_null corpus with the V0_25 substrate held constant except "
        "for the combined founder-facing budget relaxation "
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` on "
        "`widened_gradient`, fewer than 25% of C_widened_combined_se100_bmc010 "
        "runs have any founder lineage with pre200 food events. "
        "C_widened_combined_se100_bmc010's reachability is below the locked 25% "
        "threshold at tick-200; the geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200`, v0.53d locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX`, v0.53e "
        "locked as `RELAXED_OPPOSITE_SIGN_HALT`, and v0.53f locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010` "
        "remains below the reachability threshold under the combined "
        "founder-facing budget envelope. The bridge is not rescued by "
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` under "
        "V0_25 × `widened_gradient` × N_TICKS=200; the (V0_25 × "  # noqa: RUF001
        "`widened_gradient`) reachability ceiling is not lifted by any "
        "conservative single- or two-knob founder-facing budget envelope tested "
        "in the v0.53d→v0.53g stack. v0.53h (or later) candidates shift toward "
        "non-budget axes: geometry calibration (corridor width, food placement, "
        "hazard-band geometry), policy / hazard-avoidance dynamics, or "
        "N_TICKS=200 horizon extension: "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010`."
    ),
    ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_COMBINED_SE100_BMC010: (
        "On the modern A_null corpus with the V0_25 substrate held constant except "
        "for the combined founder-facing budget relaxation "
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` on "
        "`widened_gradient`, C_widened_combined_se100_bmc010's reachability "
        "clears the locked 25% threshold at tick-200 but the v0.48 sensor_radius "
        "spatial / foraging bridge does not fully fire PRESENT — fewer than 2/3 "
        "primaries fire across both gating labels at tick-200 under the strict "
        "NaN rule. The geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200`, v0.53d locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX`, v0.53e "
        "locked as `RELAXED_OPPOSITE_SIGN_HALT`, and v0.53f locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010` is "
        "partially rescued under the combined founder-facing budget envelope: "
        "reachability becomes measurable but the bridge does not fully "
        "replicate. Layout-specific partial-rescue logged in Results: "
        "`WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_COMBINED_SE100_BMC010`."
    ),
}


class V053gReducerError(Exception):
    """Halt condition raised when a v0.53g invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53g audit (read-only).

    Captures founder ``body.energy`` AND the model-level
    ``body_config.starting_energy`` AND ``body_config.base_metabolic_cost``
    at setup_observer time (read-only). Used by Test #5 to verify BOTH
    knobs of the ``body_config`` seam took effect at config-time founder
    construction — B founders observe
    ``body.energy == 60.0`` AND ``body_config.starting_energy == 60.0`` AND
    ``body_config.base_metabolic_cost == 0.25`` (V0_25 baseline); C founders
    observe ``body.energy == 100.0`` (overridden via starting_energy=100.0)
    AND ``body_config.starting_energy == 100.0`` AND
    ``body_config.base_metabolic_cost == 0.10`` (overridden via
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53g: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    Reads ``body.energy`` AND the model-level ``body_config.starting_energy``
    AND ``body_config.base_metabolic_cost`` at tick-0 to verify the v0.53g
    ``body_config`` seam propagated BOTH knobs to founder construction (B arm
    should observe starting_energy=60.0 and base_metabolic_cost=0.25 and
    energy=60.0; C arm should observe starting_energy=100.0 and
    base_metabolic_cost=0.10 and energy=100.0). Strictly read-only on every
    founder body field and on model.body_config.
    """
    founders = sorted(model.agents, key=lambda a: int(a.body.lineage_id))
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53g invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53g layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(
    ...
    Order (mirrors v0.53c / v0.53d / v0.53e / v0.53f merged implementation):
      (a) capture v0.53g founder audit table from live bodies (read-only;
          INCLUDES body.energy AND body_config.starting_energy AND
          body_config.base_metabolic_cost at tick 0 to verify BOTH knobs of
          the body_config seam propagated correctly).
      (b) wire AgentBorn (lineage tracking) / AteFood / HazardDamageApplied
          listeners (sender=model). The AteFood / HazardDamageApplied
          listeners accumulate pre50 / pre100 / pre200 rollups in parallel.
      (c) read back model.world dimensions and assert layout match.
      (d) capture tick 0 snapshot.

    setup_observer is strictly READ-ONLY on every founder body field AND on
    model.body_config — no patching, no clamps, no overrides. The combined
    starting_energy=100.0 + base_metabolic_cost=0.10 change propagates via the
    body_config parameter to run_chamber() at config-time founder
    construction, NOT via any post-construction body patch.
    """
    ...
    def setup(model: HHModel) -> None:
        ...
        def _on_ate_food(_sender: object, *, event: AteFood) -> None:
            ...
            # pre200 always accumulates for tick_now <= TICK_200.
...
def _run_one_arm(  # noqa: PLR0915 — adds body_starting_energy capture vs v0.53f
    arm: str, version: str, seed: int, hazard: int, runs_root: Path
) -> _RunCapture:
    """Execute one (arm, version, seed, hazard) run, returning its capture.

    A and B preserve the V0_25 baseline (default ``BodyConfig()`` with
    ``starting_energy=60.0``, ``base_metabolic_cost=0.25``). C overrides
    ``body_config=BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)``
    — TWO knobs simultaneously; all other BodyConfig fields preserved at
    default. All other WorldConfig / ReproductionConfig knobs are inherited
    byte-identically from the V0_25 baseline arm; ``ambient_influx_rate=1.0``
    (V0_25 baseline) on all three arms.
    """
    if arm not in ARMS:
        msg = f"v0.53g: unknown arm {arm!r}"
        raise V053gReducerError(msg)
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53g: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53g-{arm}-{version}-A_null-hzd{hazard}",
    ...
    # The simulation may terminate early on population extinction
    # (run_chamber breaks the tick loop when no agents are alive). The
    # observation window invariant is therefore: tick records cover a
    # contiguous prefix [0, last_tick] with last_tick <= TICK_200, and the
    # tick-0 record always exists. tick50/100/200_readiness lists may be
    # empty if extinction occurred before the snapshot tick.
    max_expected_ticks = TICK_200 + 1
    if len(capture.tick_records) < 1 or len(capture.tick_records) > max_expected_ticks:
        msg = (
            f"v0.53g invariant: per-tick observer must capture between 1 and "
            f"{max_expected_ticks} ticks for {run_id}; got {len(capture.tick_records)}"
        )
        raise V053gReducerError(msg)
    if 0 not in capture.tick_records:
        msg = f"v0.53g invariant: missing tick-0 record for {run_id}"
        raise V053gReducerError(msg)
    captured_ticks = sorted(capture.tick_records.keys())
    if captured_ticks != list(range(captured_ticks[0], captured_ticks[-1] + 1)):
        msg = (
            f"v0.53g invariant: tick records must be a contiguous prefix for "
            f"{run_id}; got non-contiguous ticks {captured_ticks}"
        )
        raise V053gReducerError(msg)
    if len(capture.founder_records) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53g invariant: expected exactly {EXPECTED_FOUNDERS} founder records for "
            f"{run_id}; got {len(capture.founder_records)}"
        )
        raise V053gReducerError(msg)

    # Compute per-window living-population descriptive flag (any agent
    # alive at the snapshot tick of that window).
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53g: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053gReducerError(msg)
...
_SUBVERDICT_TICK200_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010: (
        SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_PRESENT,
        SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_PARTIAL,
        SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_NOT_FOUND,
        SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_OPPOSITE,
...
# A_null_V0_25 tick-50 sub-verdict for the priority-2.b trigger.
_SUBVERDICT_TICK50_A_NULL: tuple[str, str, str, str] = (
...
def _arm_subverdict_tick200(
    arm: str,
    summaries_a: list[ObservableSummary],
    summaries_b: list[ObservableSummary],
) -> str:
    """Per-arm tick-200 4-way sub-verdict; both labels gate."""
    if arm not in _SUBVERDICT_TICK200_TABLE:
        msg = f"v0.53g: unknown arm {arm!r} in tick-200 subverdict"
        raise V053gReducerError(msg)
    return _classify_subverdict(summaries_a, summaries_b, _SUBVERDICT_TICK200_TABLE[arm])


def _a_null_subverdict_tick50(
    summaries_a: list[ObservableSummary],
    summaries_b: list[ObservableSummary],
) -> str:
    """A_null_V0_25 tick-50 sub-verdict (priority-2.b trigger only)."""
    return _classify_subverdict(summaries_a, summaries_b, _SUBVERDICT_TICK50_A_NULL)


# ---------------------------------------------------------------------------
# Re-anchor (corpus + Tier-1 bridge + Tier-2 categorical)
# ---------------------------------------------------------------------------
...
def _check_bridge_re_anchor_tick50(
    ...
    v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e / v0.53f."""
...
def _compute_arm_reachability_run_share(
    ...
    for (a, _ver, _seed, _haz), rows_in_run in by_arm_run.items():
        if a != arm:
            continue
        n_runs_total += 1
        if any(getattr(r, events_field) > 0 for r in rows_in_run):
            n_runs_with_food += 1
    if n_runs_total == 0:
        return float("nan"), 0
...
def _compute_population_stability(
    ...
    """Per (arm, window) descriptive metrics:
    - living_population_run_share: fraction of arm runs with >= 1 agent alive at tick-N.
    - label_a_n_runs: count of arm runs where Label A produces a non-nan winner.
    - label_b_n_runs: count of arm runs where Label B (window-specific) produces a winner.
    """
    by_arm_run = _index_rows_by_arm_run(all_rows)
        ...
        for window in WINDOWS:
            ...
            for k in run_keys:
                ...
                if any(getattr(r, LABEL_A_FIELD) for r in rows_in_run):
                    n_label_a += 1
                if any(getattr(r, label_b_field_by_window[window]) for r in rows_in_run):
                    n_label_b += 1
...
class WrongSignCellRow:
    """Descriptive log row for a tick-200 wrong-sign cell on the C arm.

    Recorded whenever a C tick-200 paired_d cell fires wrong-sign (signed_d <=
    -0.5). When C reachability_tick_200 < 0.25 (priority 3 SKIPS), these rows
    appear in audit_summary.csv under the
    ``wrong_sign_cells_under_reachability_below_threshold`` section as a
    descriptive non-halting log. When C reachability_tick_200 >= 0.25 (priority
    3 fires), the rows are still recorded for completeness.
    """
...
def _collect_c_tick200_wrong_sign_cells(
    summaries_tick200: list[ObservableSummary],
) -> list[WrongSignCellRow]:
    """Collect every C tick-200 cell with signed_d <= -0.5 (descriptive log)."""
    rows: list[WrongSignCellRow] = []
    for s in summaries_tick200:
        if s.arm != ARM_C_WIDENED_COMBINED_SE100_BMC010:
            continue
...
# ---------------------------------------------------------------------------
# Slice rollup (priority-ordered — PRIORITY 3 IS REACHABILITY-GATED)
# ---------------------------------------------------------------------------
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    a_null_tick200_subverdict: str,
    b_tick200_subverdict: str,
    c_tick200_subverdict: str,
    corpus_re_anchor: list[ReAnchorRow],
    bridge_re_anchor: list[BridgeReAnchorRow],
    b_widened_v025_reachability_tick200: float,
    c_widened_combined_se100_bmc010_reachability_tick200: float,
) -> tuple[str, str]:
    ...
    reachability anchor != 0.0 exactly.

    Priority 3 (RELAXED_OPPOSITE_SIGN_HALT) is REACHABILITY-GATED — fires iff
    BOTH:
      - C_widened_combined_se100_bmc010 tick-200 sub-verdict == OPPOSITE_SIGN_HALT, AND
      - c_widened_combined_se100_bmc010_reachability_run_share_tick_200 >= 0.25.
    If C sub-verdict == OPPOSITE_SIGN_HALT AND C reach < 0.25, priority 3
    SKIPS; rollup falls through to priority 5 (the wrong-sign cell(s) are
    logged descriptively in audit_summary.csv but do NOT halt).

    Priorities 4/5/6 gate on C_widened_combined_se100_bmc010 reachability +
    sub-verdict only.
    """
    ...
    # Priority 2: ANCHOR_REPLICATION_HALT — three legs.
    ...
    # exactly. Any non-zero (i.e., any single B_widened_V0_25 run with at least
    # one founder lineage producing pre200_food_events_count > 0) halts loud.
    ...
    # Fires iff C sub-verdict == OPPOSITE_SIGN_HALT AND C reach >= 0.25.
    # If C sub-verdict == OPPOSITE_SIGN_HALT AND C reach < 0.25: priority 3
    # SKIPS; falls through to priority 5 (wrong-sign cells logged descriptively
    # in audit_summary.csv but no halt).
    if (
        c_tick200_subverdict == SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_OPPOSITE
        and c_widened_combined_se100_bmc010_reachability_tick200 >= B_REACHABILITY_THRESHOLD
    ):
        return (
            ROLLUP_RELAXED_OPPOSITE_SIGN_HALT,
            LOCKED_PHRASES[ROLLUP_RELAXED_OPPOSITE_SIGN_HALT],
        )

    # Priorities 4 / 5 / 6: gate on C reachability + C sub-verdict only.
    # A_null_V0_25 / B_widened_V0_25 tick-200 sub-verdicts are descriptive at
    # this point (their halt conditions are exhausted at priority 2).
    _ = a_null_tick200_subverdict
    _ = b_tick200_subverdict
    if c_widened_combined_se100_bmc010_reachability_tick200 < B_REACHABILITY_THRESHOLD:
        ...
        return (
            ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010,
            LOCKED_PHRASES[ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010],
        )
    if c_tick200_subverdict == SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_PRESENT:
        # Priority 4: rescued.
        return (
            ROLLUP_WIDENED_BRIDGE_RESCUED_BY_COMBINED_SE100_BMC010,
            LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_RESCUED_BY_COMBINED_SE100_BMC010],
        )
    # Priority 6: partially rescued (C sub-verdict in {PARTIAL, NOT_FOUND}).
    return (
        ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_COMBINED_SE100_BMC010,
        LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_COMBINED_SE100_BMC010],
    )
...
def _write_arm_reachability_section(
    ...
    *,
    ...
    for arm, (reach, n) in arm_reach.items():
        ...
        writer.writerow(
            [section, f"{arm}/passes_threshold", _format_value(reach >= B_REACHABILITY_THRESHOLD)]
        )
        ...
        writer.writerow(
            [
                ...
                _format_value(is_gating and arm == ARM_C_WIDENED_COMBINED_SE100_BMC010),
            ]
        )


def _write_audit_summary_csv(
    summaries_tick50: list[ObservableSummary],
    summaries_tick100: list[ObservableSummary],
    summaries_tick200: list[ObservableSummary],
    corpus_re_anchor: list[ReAnchorRow],
    ...
    with path.open("w", newline="") as f:
        ...
        _write_paired_d_section(writer, "paired_d_tick50", summaries_tick50)
        _write_paired_d_section(writer, "paired_d_tick100", summaries_tick100)
        _write_paired_d_section(writer, "paired_d_tick200", summaries_tick200)
        ...
        # NEW v0.53g: wrong_sign_cells_under_reachability_below_threshold.
        # Always written when wrong-sign cells exist; the section header marker
        # `priority3_skipped_under_reachability_below_threshold` distinguishes
        # the case where priority 3 SKIPPED (reach < 0.25) from the case where
        # priority 3 fired (reach >= 0.25).
        writer.writerow(
            [
        ...
        writer.writerow(["sub_verdicts_tick200", ARM_A_NULL_V025, a_null_tick200_subverdict])
        writer.writerow(["sub_verdicts_tick200", ARM_B_WIDENED_V025, b_tick200_subverdict])
        writer.writerow(
            ["sub_verdicts_tick200", ARM_C_WIDENED_COMBINED_SE100_BMC010, c_tick200_subverdict]
        )
...
def _write_audit_log(  # noqa: PLR0912
    ...
    lines: list[str] = []
    lines.append(
        "=== v0.53g substrate-axis disambiguation audit (combined se100 + bmc010 tick-200) ===\n"
    )
    ...
    for r in corpus_re_anchor:
        ...
        lines.append(
            f"  {r.arm:<32} {r.version} h={r.hazard}  n={r.n_runs_contributing:>2}  "
            f"derived={derived_str}  published={published_str}  drift={drift_str}{halt_str}\n"
        )
    lines.append(
        "\nTier-1 bridge re-anchor "
        "(A_null_V0_25 tick-50 vs "
        "v0.48 / v0.53 / v0.53b / v0.53c / v0.53d / v0.53e / v0.53f published):\n"
    )
    for br in bridge_re_anchor_tick50:
    ...
    lines.append(
        f"  {ARM_B_WIDENED_V025:<32} tick-200  n={b_v025_n_t200:>2}  "
        f"derived={b_v025_reach_t200:.4f}  expected=0.0000{tier2_halt_str}\n"
    )

    for arm in ARMS:
        lines.append(f"\nArm {arm} — TICK-50 paired_d per (gating-label, primary observable):\n")
        for s in summaries_tick50:
            if s.arm != arm:
                continue
            lines.append(f"  [{s.label}] {_observable_line(s)}")
    for arm in ARMS:
        lines.append(f"\nArm {arm} — TICK-100 paired_d per (gating-label, primary observable):\n")
        for s in summaries_tick100:
            if s.arm != arm:
                continue
            lines.append(f"  [{s.label}] {_observable_line(s)}")
    for arm in ARMS:
        lines.append(f"\nArm {arm} — TICK-200 paired_d per (gating-label, primary observable):\n")
        for s in summaries_tick200:
            if s.arm != arm:
                continue
            lines.append(f"  [{s.label}] {_observable_line(s)}")
    ...
    for window in WINDOWS:
        for arm in ARMS:
            ...
            elif window == TICK_200 and arm == ARM_C_WIDENED_COMBINED_SE100_BMC010:
                tag = " (verdict-gating)"
            else:
                tag = " (descriptive)"
            passes = reach >= B_REACHABILITY_THRESHOLD
            lines.append(
                f"  tick-{window:>3} {arm:<32}: {reach:.4f} (n={n}, "
                f"threshold={B_REACHABILITY_THRESHOLD:.2f}, passes={passes}){tag}\n"
            )
    lines.append("\nPopulation-stability per (arm, window) (descriptive):\n")
    for ps in population_stability:
        lines.append(
            f"  {ps.arm:<32} tick-{ps.window:>3}  n_runs={ps.n_runs_total:>2}  "
    ...
    if wrong_sign_cells:
        lines.append(
            "\nWrong-sign cells under C_widened_combined_se100_bmc010 tick-200 "
            "(reachability-gated priority-3 trigger; priority-3 SKIPPED="
            f"{priority3_skipped_under_reachability}):\n"
        )
        for wsc in wrong_sign_cells:
            lines.append(
    ...
    lines.append(
        f"\nTick-200 sub-verdicts: {ARM_A_NULL_V025}={a_null_tick200_subverdict}  "
        f"{ARM_B_WIDENED_V025}={b_tick200_subverdict}  "
        f"{ARM_C_WIDENED_COMBINED_SE100_BMC010}={c_tick200_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53g substrate-axis disambiguation audit (combined se100 + bmc010 tick-200) ===")
    ...
    print(
        "Tick-200 primary observables: "
        + ", ".join(f"{n} ({'+' if s > 0 else '-'})" for n, s in PRIMARY_OBSERVABLES_TICK200)
    )
    print()
                    ...
                    for seed in seeds:
                        ...
                        b100_lid = next(
                            (
                                r.lineage_id
                                for r in rows
                                if r.is_high_tick100_readiness_fraction_lineage
                            ),
                            None,
                        )
                        b200_lid = next(
                            (
                                r.lineage_id
                                for r in rows
                                if r.is_high_tick200_readiness_fraction_lineage
                            ),
                            None,
                        )
                        print(
                            f"  {arm:<32} {version} h={hazard:>2} seed={seed:>2}  "
                            f"a_top={a_lid}  b50_top={b50_lid}  b100_top={b100_lid}  "
                            f"b200_top={b200_lid}"
                        )
    ...
    summaries_tick50: list[ObservableSummary] = []
    for arm in ARMS:
        for label_name, label_field in label_pairs_tick50:
            for name, sign in PRIMARY_OBSERVABLES_TICK50:
                summaries_tick50.append(
    ...
    summaries_tick100: list[ObservableSummary] = []
    for arm in ARMS:
        for label_name, label_field in label_pairs_tick100:
            for name, sign in PRIMARY_OBSERVABLES_TICK100:
                summaries_tick100.append(
                    _summarise_observable_for_arm(
                        arm=arm,
                        observable=name,
                        sign=sign,
    ...
    summaries_tick200: list[ObservableSummary] = []
    for arm in ARMS:
        for label_name, label_field in label_pairs_tick200:
            for name, sign in PRIMARY_OBSERVABLES_TICK200:
                summaries_tick200.append(
                    _summarise_observable_for_arm(
                        arm=arm,
                        observable=name,
                        sign=sign,
    ...
    summaries_by_arm_label_tick50: dict[tuple[str, str], list[ObservableSummary]] = {}
    for s in summaries_tick50:
        summaries_by_arm_label_tick50.setdefault((s.arm, s.label), []).append(s)
    summaries_by_arm_label_tick200: dict[tuple[str, str], list[ObservableSummary]] = {}
    for s in summaries_tick200:
        summaries_by_arm_label_tick200.setdefault((s.arm, s.label), []).append(s)

    a_null_tick50_subverdict = _a_null_subverdict_tick50(
        summaries_by_arm_label_tick50.get((ARM_A_NULL_V025, LABEL_A_NAME), []),
        summaries_by_arm_label_tick50.get((ARM_A_NULL_V025, LABEL_B_TICK50_NAME), []),
    )
    a_null_tick200_subverdict = _arm_subverdict_tick200(
        ARM_A_NULL_V025,
        summaries_by_arm_label_tick200.get((ARM_A_NULL_V025, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get((ARM_A_NULL_V025, LABEL_B_TICK200_NAME), []),
    )
    b_tick200_subverdict = _arm_subverdict_tick200(
        ARM_B_WIDENED_V025,
        summaries_by_arm_label_tick200.get((ARM_B_WIDENED_V025, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get((ARM_B_WIDENED_V025, LABEL_B_TICK200_NAME), []),
    )
    c_tick200_subverdict = _arm_subverdict_tick200(
        ARM_C_WIDENED_COMBINED_SE100_BMC010,
        summaries_by_arm_label_tick200.get((ARM_C_WIDENED_COMBINED_SE100_BMC010, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get(
            (ARM_C_WIDENED_COMBINED_SE100_BMC010, LABEL_B_TICK200_NAME), []
        ),
    )

    corpus_re_anchor = _check_corpus_re_anchor(all_rows)
    bridge_re_anchor_tick50 = _check_bridge_re_anchor_tick50(summaries_by_arm_label_tick50)

    # Per-arm reachability at all three windows.
    reachability_by_window: dict[int, dict[str, tuple[float, int]]] = {}
    for window, events_field in (
        (TICK_50, "pre50_food_events_count"),
        (TICK_100, "pre100_food_events_count"),
        (TICK_200, "pre200_food_events_count"),
    ):
        reachability_by_window[window] = {
            arm: _compute_arm_reachability_run_share(all_rows, arm, events_field) for arm in ARMS
        }

    population_stability = _compute_population_stability(all_rows, captures)

    # Always collect C tick-200 wrong-sign cells (descriptive log; relevant
    # whether or not priority 3 fires).
    wrong_sign_cells = _collect_c_tick200_wrong_sign_cells(summaries_tick200)
    c_reach_t200 = reachability_by_window[TICK_200][ARM_C_WIDENED_COMBINED_SE100_BMC010][0]
    priority3_skipped_under_reachability = (
        c_tick200_subverdict == SUBVERDICT_C_COMBINED_SE100_BMC010_TICK200_OPPOSITE
        and c_reach_t200 < B_REACHABILITY_THRESHOLD
    )

    rollup_verdict, rollup_phrase = _evaluate_rollup(
        a_null_tick50_subverdict=a_null_tick50_subverdict,
        a_null_tick200_subverdict=a_null_tick200_subverdict,
        b_tick200_subverdict=b_tick200_subverdict,
        c_tick200_subverdict=c_tick200_subverdict,
        ...
        c_widened_combined_se100_bmc010_reachability_tick200=c_reach_t200,
    )

    per_lineage_path = out_dir / "per_run_per_lineage_v053g.csv"
    ...
    _write_audit_summary_csv(
        ...
        a_null_tick200_subverdict,
        b_tick200_subverdict,
        c_tick200_subverdict,
    ...
    if rollup_verdict in {
        ...
        raise V053gReducerError(rollup_phrase)
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53g-substrate-axis-combined-se100-bmc010"),
        help=(
            "Directory for v0.53g outputs "
            "(default: runs/v0.53g-substrate-axis-combined-se100-bmc010)."
        ),
    )
