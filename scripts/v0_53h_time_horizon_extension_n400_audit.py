"""v0.53h time-horizon extension audit (combined founder starting-energy +
per-tick metabolic-cost rescue probe at ``n_ticks=400`` on the C arm).

Pre-reg: [[docs/experiments/fear_hunger_v0.53h.md]]. Question: with
``BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)`` held identical
to v0.53g C, and ``n_ticks`` doubled from 200 to 400 on the C arm only, does C
reachability cross 0.25 at tick-400?

Corpus (locked, identical shape to v0.53..v0.53g): A_null arm only across
v0.42 / v0.43R / v0.44 / v0.45, hazards {0, 8}, seeds 41..72. 64 runs per arm;
3 arms (asymmetric BodyConfig AND asymmetric ``n_ticks``); 192 runs total.
...
Arms (locked, 3 — asymmetric BodyConfig AND asymmetric ``n_ticks``):
  ...
  C_widened_combined_se100_bmc010_n400:      widened_gradient +
      body_config=BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10) +
      n_ticks=400

Through tick-200 the C arm in v0.53h is byte-identical to v0.53g C under
matched RNG (Tier-3 predecessor-sanity anchor); from tick-200 to tick-400 the
C arm continues running (B has terminated).
...
Tick-50 cells anchor v0.48..v0.53g (Tier-1 priority 2.a). Tick-100 / tick-200
cells are descriptive on A/B and predecessor-sanity on C (tick-200 is the
Tier-3 anchor; tick-200 reachability on C must equal 0.0 exactly per v0.53g).
The tick-400 panel on C is verdict-gating.

Labels:
  Label A:           high_sensor_radius_lineage. Trait-resolved (identical to v0.48..v0.53g).
  ...
  Label B (tick-200): high_tick200_readiness_fraction_lineage — predecessor-sanity.
...
Anchor stack (locked, 4-tier):
  ...
  Tier-3 (priority 2.d): c_widened_combined_se100_bmc010_n400_reachability_run_share_tick_200
                         == 0.0 exactly  (NEW; predecessor-sanity vs v0.53g).

Verdict-gating reachability (locked, pre-data):
  c_widened_combined_se100_bmc010_n400_reachability_run_share_tick_400  vs threshold 0.25.

Per-arm tick-400 sub-verdicts on C (locked, 4-way — verdict-gating):
  C_COMBINED_SE100_BMC010_N400_TICK400_BRIDGE_{PRESENT, PARTIAL, NOT_FOUND, OPPOSITE_SIGN_HALT}
...
REACHABILITY-GATED on C tick-400):
  ...
  3. RELAXED_OPPOSITE_SIGN_HALT  (gates on C tick-400 sub-verdict + reach >= 0.25)
...
Conservation framing: zero ``src/`` modifications. ``n_ticks`` is already a
``run_chamber()`` parameter (default 200); v0.53e's
``body_config: BodyConfig | None = None`` seam carries forward.

Usage:
    uv run python scripts/v0_53h_time_horizon_extension_n400_audit.py
    uv run python scripts/v0_53h_time_horizon_extension_n400_audit.py \
        --out-dir runs/v0.53h-time-horizon-extension-n400
"""
...
N_TICKS_C: int = 400  # C horizon (doubled — v0.53h primary axis)
N_FOUNDERS: int = 5
TICK_50: int = 50
TICK_100: int = 100
TICK_200: int = 200
...
ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: str = "C_widened_combined_se100_bmc010_n400"
ARMS: tuple[str, ...] = (
    ARM_A_NULL_V025,
    ARM_B_WIDENED_V025,
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400,
)

LAYOUT_NAME_BY_ARM: dict[str, str] = {
    ARM_A_NULL_V025: "tight_gradient",
    ARM_B_WIDENED_V025: "widened_gradient",
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: "widened_gradient",
}
...
N_TICKS_BY_ARM: dict[str, int] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: N_TICKS_C,
}
...
WINDOWS_BY_ARM: dict[str, tuple[int, ...]] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: WINDOWS_C,
}
...
V0_25_AMBIENT_INFLUX_RATE: float = 1.0  # baseline (preserved on ALL arms in v0.53h).
...
STARTING_ENERGY_100_DOSE: float = 100.0  # v0.53e/g/h test dose (maximal under max_energy=100.0)
BASE_METABOLIC_COST_010_DOSE: float = 0.10  # v0.53f/g/h test dose (60% reduction)
...
BODY_STARTING_ENERGY_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: STARTING_ENERGY_100_DOSE,
}
...
BODY_BASE_METABOLIC_COST_BY_ARM: dict[str, float] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: BASE_METABOLIC_COST_010_DOSE,
}
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    if arm in (ARM_B_WIDENED_V025, ARM_C_WIDENED_COMBINED_SE100_BMC010_N400):
        return widened_gradient_layout()
    msg = f"v0.53h: unknown arm {arm!r}"
    raise V053hReducerError(msg)
...
def _body_config_for_arm(arm: str) -> BodyConfig | None:
    ...
    to v0.53g C; v0.53h's only delta from v0.53g is ``n_ticks`` (200 -> 400).
    All other BodyConfig fields preserved at default (max_energy=100.0,
    starting_health=100.0, max_health=100.0, sensor_radius_metabolic_cost=0.05,
    effective_sensor_radius_override=None).
    """
    if arm in (ARM_A_NULL_V025, ARM_B_WIDENED_V025):
        return None
    if arm == ARM_C_WIDENED_COMBINED_SE100_BMC010_N400:
        return BodyConfig(
...
# v0.48..v0.53g published bridge cells (label, observable) -> signed_d.
...
# Priority-2.b consumes A_null tick-50; tick-400 on C is verdict-gating.
...
SUBVERDICT_C_COMBINED_TICK50_PRESENT = "C_COMBINED_SE100_BMC010_N400_TICK50_BRIDGE_PRESENT"
SUBVERDICT_C_COMBINED_TICK50_PARTIAL = "C_COMBINED_SE100_BMC010_N400_TICK50_BRIDGE_PARTIAL"
SUBVERDICT_C_COMBINED_TICK50_NOT_FOUND = "C_COMBINED_SE100_BMC010_N400_TICK50_BRIDGE_NOT_FOUND"
SUBVERDICT_C_COMBINED_TICK50_OPPOSITE = "C_COMBINED_SE100_BMC010_N400_TICK50_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_COMBINED_TICK100_PRESENT = "C_COMBINED_SE100_BMC010_N400_TICK100_BRIDGE_PRESENT"
SUBVERDICT_C_COMBINED_TICK100_PARTIAL = "C_COMBINED_SE100_BMC010_N400_TICK100_BRIDGE_PARTIAL"
SUBVERDICT_C_COMBINED_TICK100_NOT_FOUND = "C_COMBINED_SE100_BMC010_N400_TICK100_BRIDGE_NOT_FOUND"
SUBVERDICT_C_COMBINED_TICK100_OPPOSITE = "C_COMBINED_SE100_BMC010_N400_TICK100_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_COMBINED_TICK200_PRESENT = "C_COMBINED_SE100_BMC010_N400_TICK200_BRIDGE_PRESENT"
SUBVERDICT_C_COMBINED_TICK200_PARTIAL = "C_COMBINED_SE100_BMC010_N400_TICK200_BRIDGE_PARTIAL"
SUBVERDICT_C_COMBINED_TICK200_NOT_FOUND = "C_COMBINED_SE100_BMC010_N400_TICK200_BRIDGE_NOT_FOUND"
SUBVERDICT_C_COMBINED_TICK200_OPPOSITE = "C_COMBINED_SE100_BMC010_N400_TICK200_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_COMBINED_TICK400_PRESENT = "C_COMBINED_SE100_BMC010_N400_TICK400_BRIDGE_PRESENT"
SUBVERDICT_C_COMBINED_TICK400_PARTIAL = "C_COMBINED_SE100_BMC010_N400_TICK400_BRIDGE_PARTIAL"
SUBVERDICT_C_COMBINED_TICK400_NOT_FOUND = "C_COMBINED_SE100_BMC010_N400_TICK400_BRIDGE_NOT_FOUND"
SUBVERDICT_C_COMBINED_TICK400_OPPOSITE = "C_COMBINED_SE100_BMC010_N400_TICK400_OPPOSITE_SIGN_HALT"
...
ROLLUP_WIDENED_BRIDGE_RESCUED_BY_TIME_HORIZON_400 = "WIDENED_BRIDGE_RESCUED_BY_TIME_HORIZON_400"
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TIME_HORIZON_400 = (
    "WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TIME_HORIZON_400"
)
ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_TIME_HORIZON_400 = (
    "WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_TIME_HORIZON_400"
)
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53h's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_ANCHOR_REPLICATION_HALT: (
        "Halt: v0.53h's A_null_V0_25 arm does not reproduce v0.48–v0.53g's "  # noqa: RUF001
        "tick-50 spatial bridge, OR v0.53h's B_widened_V0_25 arm does not "
        "reproduce v0.53c–v0.53g's `0/64` reachability lock at tick-200, OR "  # noqa: RUF001
        "v0.53h's C_widened_combined_se100_bmc010_n400 arm does not reproduce "
        "v0.53g's `0/64` reachability lock at tick-200. v0.53h cannot interpret "
        "the C tick-400 cells without an established baseline on the V0_25 "
        "substrate AND on the predecessor combined-budget envelope."
    ),
    ROLLUP_RELAXED_OPPOSITE_SIGN_HALT: (
        "Halt: a v0.53h C_widened_combined_se100_bmc010_n400 tick-400 spatial / "
        ...
        "under doubled simulation horizon (`n_ticks = 400`) surfaces a regime "
        "where the locked expected signs do not hold under measurable food access. "
        "The reachability-gated trigger preserves v0.53e's locked sign discipline "
        "while excluding the v0.53e-style measurement-edge case (wrong-sign at "
        "reachability=0)."
    ),
    ROLLUP_WIDENED_BRIDGE_RESCUED_BY_TIME_HORIZON_400: (
        ...
        "`widened_gradient` AND the simulation horizon doubled from `n_ticks=200` "
        "to `n_ticks=400` on the C arm only, the v0.48 sensor_radius spatial / "
        "foraging bridge fires PRESENT under the locked +0.5 paired_d threshold "
        "at tick-400. The geometry/substrate cell that v0.53c locked as "
        ...
        "and v0.53g locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010` "
        "admits a measurable bridge under the doubled simulation horizon: "
        "B_widened_V0_25 reachability remains `0/64` at tick-200 (predecessor "
        "lock holds), C reachability at tick-200 remains `0/64` (v0.53g "
        "predecessor-sanity lock holds), C reachability at tick-400 clears the "
        "locked 25% threshold, and ≥ 2/3 spatial / foraging primaries fire "
        "PRESENT under both gating labels at the tick-400 panel. Doubling "
        "`n_ticks` from 200 to 400 lifts the reachability ceiling on the v0.53g "
        "combined-budget envelope. The (V0_25 × `widened_gradient` × "  # noqa: RUF001
        "combined-budget) cell is bounded by the `n_ticks=200` simulation horizon "
        "under the tested envelope; doubling the horizon to `n_ticks=400` admits "
        "the bridge: `WIDENED_BRIDGE_RESCUED_BY_TIME_HORIZON_400`."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TIME_HORIZON_400: (
        "On the modern A_null corpus with the V0_25 substrate held constant except "
        "for the combined founder-facing budget relaxation "
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` on "
        "`widened_gradient` AND the simulation horizon doubled from `n_ticks=200` "
        "to `n_ticks=400` on the C arm only, fewer than 25% of C runs have any "
        "founder lineage with pre400 food events. C's reachability is below the "
        "locked 25% threshold at tick-400; the geometry/substrate cell that "
        ...
        "and v0.53g locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010` "
        "remains below the reachability threshold under the doubled simulation "
        "horizon. The bridge is not rescued by extending the horizon to "
        "`n_ticks=400` under V0_25 × `widened_gradient` × combined-budget "  # noqa: RUF001
        "envelope; the (V0_25 × `widened_gradient` × combined-budget) "  # noqa: RUF001
        "reachability ceiling is not lifted by doubling the simulation horizon "
        "to `n_ticks=400`. v0.53i (or later) candidates shift toward axes not "
        "yet tested in the v0.53d→v0.53h stack: geometry calibration (corridor "
        "width, food placement, hazard-band geometry), policy / hazard-avoidance "
        "dynamics, or unmodeled-dynamics probes: "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TIME_HORIZON_400`."
    ),
    ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_TIME_HORIZON_400: (
        "On the modern A_null corpus with the V0_25 substrate held constant except "
        ...
        "`BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)` on "
        "`widened_gradient` AND the simulation horizon doubled from `n_ticks=200` "
        "to `n_ticks=400` on the C arm only, C's reachability clears the locked "
        "25% threshold at tick-400 but the v0.48 sensor_radius spatial / foraging "
        ...
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_BASE_METABOLIC_COST_010`, "
        "and v0.53g locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_COMBINED_SE100_BMC010` is "
        "partially rescued under the doubled simulation horizon: reachability "
...
class V053hReducerError(Exception):
    """Halt condition raised when a v0.53h invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53h audit (read-only).
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53h: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53h invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53h layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(  # noqa: PLR0915 — adds pre400 listeners for asymmetric horizon vs v0.53g
    arm: str,
    capture: _RunCapture,
...
def _run_one_arm(  # noqa: PLR0915 — adds n_ticks-asymmetric horizon vs v0.53g
    arm: str, version: str, seed: int, hazard: int, runs_root: Path
) -> _RunCapture:
    """Execute one (arm, version, seed, hazard) run, returning its capture.

    A and B run to ``n_ticks=200`` with default ``BodyConfig()``. C runs to
    ``n_ticks=400`` with
    ``body_config=BodyConfig(starting_energy=100.0, base_metabolic_cost=0.10)``
    — identical to v0.53g C through tick-200 and continuing through ticks
    201..400.
    """
    if arm not in ARMS:
        msg = f"v0.53h: unknown arm {arm!r}"
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53h: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53h-{arm}-{version}-A_null-hzd{hazard}",
...
def _aggregate_per_lineage(  # noqa: PLR0912, PLR0915 — adds pre400 / tick-400 aggregation on C
    capture: _RunCapture,
) -> list[PerLineageRow]:
    all_lineages = sorted(set(capture.lineage_by_agent.values()))
    is_c_arm = capture.arm == ARM_C_WIDENED_COMBINED_SE100_BMC010_N400
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53h: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053hReducerError(msg)
...
def _per_run_paired_delta(
    ...
    for r in rows_in_run:
        v = getattr(r, label_field)
        if isinstance(v, float) and math.isnan(v):
            # NaN-broadcast row (e.g., A/B for the C-only tick-400 label):
            # treat as non-label for the purpose of the paired delta — but we
            # also can't trust that row's observable as "non-label" data when
            # the label itself is undefined for that row's arm. The orchestrator
            # only computes per-arm deltas, so non-label rows here come from
            # the same arm; we keep them.
            non_label.append(r)
            continue
        if not v:
            non_label.append(r)
...
_SUBVERDICT_TICK50_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: (
        SUBVERDICT_C_COMBINED_TICK50_PRESENT,
        SUBVERDICT_C_COMBINED_TICK50_PARTIAL,
        SUBVERDICT_C_COMBINED_TICK50_NOT_FOUND,
        SUBVERDICT_C_COMBINED_TICK50_OPPOSITE,
    ),
}
...
_SUBVERDICT_TICK100_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: (
        SUBVERDICT_C_COMBINED_TICK100_PRESENT,
        SUBVERDICT_C_COMBINED_TICK100_PARTIAL,
        SUBVERDICT_C_COMBINED_TICK100_NOT_FOUND,
        SUBVERDICT_C_COMBINED_TICK100_OPPOSITE,
    ),
}
...
_SUBVERDICT_TICK200_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_WIDENED_COMBINED_SE100_BMC010_N400: (
        SUBVERDICT_C_COMBINED_TICK200_PRESENT,
        SUBVERDICT_C_COMBINED_TICK200_PARTIAL,
        SUBVERDICT_C_COMBINED_TICK200_NOT_FOUND,
        SUBVERDICT_C_COMBINED_TICK200_OPPOSITE,
    ),
}

# C tick-400 sub-verdict (verdict-gating window; C only).
_SUBVERDICT_TICK400_C: tuple[str, str, str, str] = (
    SUBVERDICT_C_COMBINED_TICK400_PRESENT,
    SUBVERDICT_C_COMBINED_TICK400_PARTIAL,
    SUBVERDICT_C_COMBINED_TICK400_NOT_FOUND,
    SUBVERDICT_C_COMBINED_TICK400_OPPOSITE,
)
...
def _arm_subverdict_at_window(
    ...
    if window == TICK_50:
        ...
        if arm != ARM_C_WIDENED_COMBINED_SE100_BMC010_N400:
            msg = f"v0.53h: tick-400 sub-verdict only defined on C; got arm {arm!r}"
            raise V053hReducerError(msg)
        table = _SUBVERDICT_TICK400_C
    else:
        msg = f"v0.53h: unknown window {window!r}"
        raise V053hReducerError(msg)
    return _classify_subverdict(summaries_a, summaries_b, table)


# ---------------------------------------------------------------------------
# Re-anchor (corpus + Tier-1 bridge + Tier-2 categorical + Tier-3 predecessor)
# ---------------------------------------------------------------------------
...
def _collect_c_tick400_wrong_sign_cells(
    ...
    for s in summaries_tick400:
        if s.arm != ARM_C_WIDENED_COMBINED_SE100_BMC010_N400:
            continue
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    tick-50 sub-verdict != PRESENT, (c) B_widened_V0_25 categorical reachability
    anchor != 0.0 exactly, OR (d) C tick-200 predecessor-sanity reachability
    anchor != 0.0 exactly.
    ...
    # Priority 2: ANCHOR_REPLICATION_HALT — four legs (a/b/c/d).
    ...
    # (d) Tier-3 predecessor-sanity: C tick-200 reachability must == 0.0
    # exactly (NEW in v0.53h; locks v0.53g's `0/64` C result deterministically).
    if c_reachability_tick200 != 0.0:
        return (
            ROLLUP_ANCHOR_REPLICATION_HALT,
            LOCKED_PHRASES[ROLLUP_ANCHOR_REPLICATION_HALT],
        )

    # Priority 3: RELAXED_OPPOSITE_SIGN_HALT — REACHABILITY-GATED on C tick-400.
    if (
        c_tick400_subverdict == SUBVERDICT_C_COMBINED_TICK400_OPPOSITE
    ...
    if c_reachability_tick400 < B_REACHABILITY_THRESHOLD:
        ...
        return (
            ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TIME_HORIZON_400,
            LOCKED_PHRASES[ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TIME_HORIZON_400],
        )
    if c_tick400_subverdict == SUBVERDICT_C_COMBINED_TICK400_PRESENT:
        # Priority 4: rescued.
        return (
            ROLLUP_WIDENED_BRIDGE_RESCUED_BY_TIME_HORIZON_400,
            LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_RESCUED_BY_TIME_HORIZON_400],
        )
    # Priority 6: partially rescued (C tick-400 sub-verdict in {PARTIAL, NOT_FOUND}).
    return (
        ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_TIME_HORIZON_400,
        LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_TIME_HORIZON_400],
    )
...
def compute_slice_rollup_verdict(
    *,
    ...
    return _evaluate_rollup(
        ...
        c_reachability_tick200=c_reachability_tick200,
...
def _write_arm_reachability_section(
    ...
    *,
    ...
    for arm, (reach, n) in arm_reach.items():
        ...
        writer.writerow(
            [section, f"{arm}/tier3_predecessor_sanity_anchor", _format_value(is_arm_tier3)]
        )
        writer.writerow(
            [
                section,
                f"{arm}/is_verdict_gating",
                _format_value(is_gating and arm == ARM_C_WIDENED_COMBINED_SE100_BMC010_N400),
            ]
        )
    ...
    *,
    ...
    with path.open("w", newline="") as f:
        ...
        for window, section in (
            ...
            tier3_arm = ARM_C_WIDENED_COMBINED_SE100_BMC010_N400 if window == TICK_200 else None
        ...
        # Wrong-sign cells under C tick-400 reachability < 0.25 (gating window
        # is now tick-400, not tick-200 as in v0.53f/g).
        writer.writerow(
            [
                "wrong_sign_cells_under_reachability_below_threshold",
...
def _write_audit_log(  # noqa: PLR0912, PLR0915
    ...
    *,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("=== v0.53h time-horizon extension audit (n_ticks=400 on C arm) ===\n")
    ...
    lines.append(
        "\nTier-1 bridge re-anchor (A_null_V0_25 tick-50 vs v0.48–v0.53g published):\n"  # noqa: RUF001
    )
    for br in bridge_re_anchor_tick50:
    ...
    c_reach_t200, c_n_t200 = reachability_by_window[TICK_200][
        ARM_C_WIDENED_COMBINED_SE100_BMC010_N400
    ]
    tier3_halt_str = " HALT" if c_reach_t200 != 0.0 else ""
    lines.append(
        "\nTier-3 predecessor-sanity anchor "
        "(C tick-200 reachability_run_share must == 0.0 exactly per v0.53g):\n"
    )
    lines.append(
        f"  {ARM_C_WIDENED_COMBINED_SE100_BMC010_N400:<40} tick-200  n={c_n_t200:>2}  "
        f"derived={c_reach_t200:.4f}  expected=0.0000{tier3_halt_str}\n"
    )

    for window in WINDOWS_ALL:
        for arm in ARMS:
            if window == TICK_400 and arm != ARM_C_WIDENED_COMBINED_SE100_BMC010_N400:
                continue
            lines.append(
    ...
    for window in WINDOWS_ALL:
        for arm in ARMS:
            ...
            elif window == TICK_200 and arm == ARM_C_WIDENED_COMBINED_SE100_BMC010_N400:
                tag = " (Tier-3 predecessor-sanity anchor)"
            elif window == TICK_400 and arm == ARM_C_WIDENED_COMBINED_SE100_BMC010_N400:
                tag = " (verdict-gating)"
            else:
                tag = " (descriptive)"
            passes = (not math.isnan(reach)) and reach >= B_REACHABILITY_THRESHOLD
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53h time-horizon extension audit (n_ticks=400 on C arm) ===")
    ...
    c_tick400_subverdict = _arm_subverdict_at_window(
        ...
        summaries_by_arm_label[TICK_400].get(
            (ARM_C_WIDENED_COMBINED_SE100_BMC010_N400, LABEL_A_NAME), []
        ),
        summaries_by_arm_label[TICK_400].get(
            (ARM_C_WIDENED_COMBINED_SE100_BMC010_N400, LABEL_B_TICK400_NAME), []
        ),
    )
    ...
    c_reach_t200 = reachability_by_window[TICK_200][ARM_C_WIDENED_COMBINED_SE100_BMC010_N400][0]
    c_reach_t400 = reachability_by_window[TICK_400][ARM_C_WIDENED_COMBINED_SE100_BMC010_N400][0]
    ...
    rollup_verdict, rollup_phrase = _evaluate_rollup(
        ...
        c_reachability_tick200=c_reach_t200,
        c_reachability_tick400=c_reach_t400,
    )

    per_lineage_path = out_dir / "per_run_per_lineage_v053h.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053hReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(
        description="v0.53h time-horizon extension audit (n_ticks=400 on C arm)"
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53h-time-horizon-extension-n400"),
        help=("Directory for v0.53h outputs (default: runs/v0.53h-time-horizon-extension-n400)."),
    )
