"""v0.43R intervention audit: post-intervention top-lineage b50 share under
tick-50 food-density rewrites.

Tests the v0.43R density-necessity question on the 48-run intervention
corpus (3 arms x 2 hazards x 8 seeds):

  Primary:   B_reduce_food_density_50pct at h=8 reduces post_intervention_
             top_lineage_b50_share by >= 0.15 below A_null AND
             C_density_preserving_perturbation stays within +/- 0.10 of
             A_null at h=8.
...
Verdict (locked, mutually exclusive on b_passes / c-direction):
  FOOD_DENSITY_NECESSARY                         iff b_passes AND c_passes.
  SUBSTRATE_PERTURBATION_DISRUPTS_DOMINANCE      iff b_passes AND c_below_a.
  FOOD_DENSITY_NOT_NECESSARY                     iff not b_passes.
  C_ABOVE_A_UNMODELED_SUBSTRATE_ARTEFACT (halt)  iff b_passes AND c_above_a.

Auxiliary (per-hazard, non-exclusive):
  DENSITY_REDUCTION_ABLATES_REPRODUCTION fires at hazard h iff
  B_reduce_food_density_50pct's n_excluded_zero_post50 > 2 (out of 8).

Reads (per-arm-per-seed):
  - runs/fear-hunger-v0.43R-tight_gradient/arms/<arm>/seed-<n>/events.jsonl
    for AgentBorn (post-50 births) and FoodRedistributedByIntervention
    (substrate conservation H2d).

Halt invariants (locked):
  - H2a (run count): 48 runs on disk.
  - H2b (intervention-fire count per arm): A_null=0; B=1 reduce-density
    fire; C=1 perturbation fire.
  ...
  - H2d (substrate conservation):
      * For B: total_food_after ~= 0.5 * total_food_before within
        max(1e-3, 1e-5 * total_food_before); multiset digest must change
        on any run with total_food_before > 0.
      * For C: total_food_after ~= total_food_before within tolerance.
        Multiset digest equality is allowed only when n_cells_changed=0
        (legitimate degenerate fixed-point case — see pre-reg).
  ...
  - H2e (regression byte-identity): NOT enforced here; lives in
    [[tests/test_world_intervention_hook_v0_43R.py]]. The audit assumes
    the sweep was produced by the v0.43R chamber driver.
...
Outputs eight CSVs under ``runs/lineage-v0.43R/``:
  ...
  - substrate_conservation.csv (per-(arm, hazard) totals + digest stats)

Usage:
    uv run python scripts/v0_43R_intervention_audit.py
"""
...
# Substrate conservation tolerances
TOTAL_FOOD_ABS_TOL: float = 1e-3
TOTAL_FOOD_REL_TOL: float = 1e-5

# B-arm density factor (locked at 0.5)
B_DENSITY_FACTOR: float = 0.5

# Arm labels (mirror V0_43R_INTERVENTION_ARMS)
ARM_A_NULL: str = "A_null"
ARM_B_REDUCE: str = "B_reduce_food_density_50pct"
ARM_C_PERTURBATION: str = "C_density_preserving_perturbation"

ALL_ARMS: tuple[str, ...] = (ARM_A_NULL, ARM_B_REDUCE, ARM_C_PERTURBATION)
...
# Verdict labels
VERDICT_DENSITY_NECESSARY = "FOOD_DENSITY_NECESSARY"
VERDICT_PERTURBATION_DISRUPTS = "SUBSTRATE_PERTURBATION_DISRUPTS_DOMINANCE"
VERDICT_DENSITY_NOT_NECESSARY = "FOOD_DENSITY_NOT_NECESSARY"


LOCKED_DENSITY_NECESSARY_PHRASE = (
    "Halving the food density at the tick-50/tick-51 boundary materially "
    "disrupts the post-50 dominance pattern at h=8: when food abundance is "
    "reduced by 50% across every eligible cell, the surviving lineages do "
    "not reconstitute concentration of comparable share. The density-"
    "preserving perturbation control (which redistributes food values "
    "across (x, y)-sorted consecutive pairs at a 25/75 split, preserving "
    "total density and matching per-cell magnitude) does NOT produce "
    "comparable disruption. Under the tested substrate, food density at "
    "tick 50 is necessary for the v0.34..v0.41 dominance pattern; the "
    "specific distribution of food values across the food zone is not. "
    "v0.43R does not declare which property of food density carries the "
    "effect; refinement interventions in v0.44+ are required. Necessity "
    "is established at one hazard level (h=8) on one seed band (49..56); "
    "cross-stream calibration and dose-response sweeps are reserved for "
    "v0.44+. Sufficiency is NOT tested."
)
LOCKED_PERTURBATION_DISRUPTS_PHRASE = (
    "Halving food density disrupts post-50 dominance at h=8, but the "
    "magnitude-matched density-preserving perturbation control produces "
    "comparable disruption. The post-50 dominance pattern is sensitive to "
    "any food-value rewrite of comparable per-cell magnitude, not "
    "specifically to density reduction. v0.34..v0.41's correlational "
    "pattern reflects fragility to substrate-shock rather than a density-"
    "specific causal mechanism. v0.44 candidate: reduce the C-arm's "
    "perturbation magnitude to test whether the pattern is fragile to any "
    "substrate value rewrite or only to large-magnitude rewrites. "
    "Mechanism remains unidentified at the food-density level."
)
LOCKED_DENSITY_NOT_NECESSARY_PHRASE = (
    "Halving the food density at the tick-50/tick-51 boundary does not "
    "materially disrupt the post-50 dominance pattern at h=8. A surviving "
    "lineage reconstitutes concentration-of-share even when food abundance "
    "is reduced by 50% across the food zone. Food density at tick 50 is "
    "not necessary for post-50 dominance under the tested substrate; the "
    "causal source lives outside the food-substrate's value layer at the "
    "anchor moment. v0.42 ruled out the tick-50 leader's identity; v0.43R "
    "rules out tick-50 food density. Remaining substrate-anchored "
    "candidates: hazard topology, chamber geometry (wall-adjacency, "
    "reachability), founder-trait + spatial-position interaction at tick "
    "50, and birth-position constraints. v0.44 candidate: hazard "
    "relocation intervention (h)."
)
LOCKED_DENSITY_REDUCTION_ABLATES_REPRODUCTION_PHRASE = (
    "At hazard h={h}, the B_reduce_food_density_50pct arm produced "
    "{n_excluded} of 8 runs with zero post-50 births: halving food density "
    "ablated reproduction itself rather than relocating dominance. This is "
    "a descriptive substrate finding parallel to but independent of the "
    "share-based primary verdict: the reduced per-cell food intake fell "
    "below the threshold required to sustain post-50 reproduction in this "
    ...
    "alongside but does NOT alter the primary verdict logic. v0.44 "
    "candidate: use a milder density factor (e.g., 0.75) to separate "
    "density-matters-for-dominance from density-matters-for-reproduction-"
    "at-all."
)


RUNS_ROOT: Path = Path("runs")
SWEEP_ROOT: Path = RUNS_ROOT / "fear-hunger-v0.43R-tight_gradient" / "arms"
OUT_DIR: Path = RUNS_ROOT / "lineage-v0.43R"
...
class PrimaryTestRow:
    ...
    c_below_a: bool
    c_above_a: bool
    food_density_necessary: bool
    substrate_perturbation_disrupts: bool
    food_density_not_necessary: bool
...
@dataclass(frozen=True)
class SubstrateConservationRow:
    arm: str
    hazard: int
    n_runs: int
    mean_total_food_before: float
    mean_total_food_after: float
...
_ARM_DIRS: dict[str, dict[int, str]] = {
    ARM_A_NULL: {
        0: "v043R-A_null-hzd0-influx-1.0",
        8: "v043R-A_null-hzd8-influx-1.0",
    },
    ARM_B_REDUCE: {
        0: "v043R-B_reduce_food_density_50pct-hzd0-influx-1.0",
        8: "v043R-B_reduce_food_density_50pct-hzd8-influx-1.0",
    },
    ARM_C_PERTURBATION: {
        0: "v043R-C_density_preserving_perturbation-hzd0-influx-1.0",
        8: "v043R-C_density_preserving_perturbation-hzd8-influx-1.0",
    },
}
...
def assert_sweep_present() -> None:
    ...
    if missing:
        msg = (
            f"v0.43R sweep incomplete: {len(missing)} events.jsonl missing. "
...
def _within_total_tolerance(observed: float, expected: float) -> bool:
    tol = max(TOTAL_FOOD_ABS_TOL, TOTAL_FOOD_REL_TOL * max(abs(observed), abs(expected)))
...
def _reduce_one_run(arm: str, hazard: int, seed: int) -> PerRunRow:  # noqa: PLR0912, PLR0915
    ...
    (b/c/d), and return one PerRunRow.
    """
    ...
    if arm == ARM_A_NULL:
        # H2b: zero food-redistribution events on A_null.
        if summaries:
            msg = (
                f"H2b violation: A_null arm hzd={hazard} seed={seed} emitted "
                f"{len(summaries)} FoodRedistributedByIntervention events; expected 0"
            )
            raise lr.LineageReplayError(msg)
        fired = False
        ...
        n_eligible_cells = 0
        n_cells_changed = 0
        total_food_before = float("nan")
        total_food_after = float("nan")
        ...
        if len(summaries) != 1:
            msg = (
                ...
                f"{len(summaries)} FoodRedistributedByIntervention events; expected 1"
            )
            raise lr.LineageReplayError(msg)
        ...
        expected_kind = (
            ...
            if arm == ARM_B_REDUCE
            else KIND_DENSITY_PRESERVING_PERTURBATION
        )
        ...
        total_food_before = float(ev["total_food_before"])
        total_food_after = float(ev["total_food_after"])
        eligible_cells_digest = ev["eligible_cells_digest"]
        digest_before = ev["food_multiset_digest_before"]
        digest_after = ev["food_multiset_digest_after"]
        if n_cells_changed > n_eligible_cells:
            ...
            raise lr.LineageReplayError(msg)
        if arm == ARM_B_REDUCE:
            expected_after = B_DENSITY_FACTOR * total_food_before
            if not _within_total_tolerance(total_food_after, expected_after):
                msg = (
                    f"H2d B-conservation mismatch: arm={arm} hzd={hazard} seed={seed} "
                    f"total_food_after={total_food_after} expected ~{expected_after} "
                    f"(0.5 * {total_food_before})"
                )
                raise lr.LineageReplayError(msg)
            # B must change the multiset on any non-degenerate run (total > 0).
            if total_food_before > 0 and digest_before == digest_after:
                msg = (
                    f"H2d B-multiset invariant: arm={arm} hzd={hazard} seed={seed} "
                    f"halving total>0 produced identical multiset digest "
                    f"(arithmetic bug suspected). digest={digest_before}"
                )
                raise lr.LineageReplayError(msg)
        else:  # ARM_C_PERTURBATION
            if not _within_total_tolerance(total_food_after, total_food_before):
                msg = (
                    f"H2d C-conservation mismatch: arm={arm} hzd={hazard} seed={seed} "
                    f"total_food_after={total_food_after} expected ~{total_food_before}"
                )
                raise lr.LineageReplayError(msg)
            # C may legitimately preserve the multiset (fixed-point case);
            # halt only if digests match AND n_cells_changed > 0 (would
            # indicate a digest computation bug).
            if digest_before == digest_after and n_cells_changed > 0:
                msg = (
                    f"H2d C-multiset bug: arm={arm} hzd={hazard} seed={seed} "
                    f"digest_before == digest_after but n_cells_changed={n_cells_changed} > 0"
                )
                raise lr.LineageReplayError(msg)
        fired = True
    ...
    # (no exclusion — no lineage is killed by substrate rewrite).
...
def build_substrate_conservation(per_run: list[PerRunRow]) -> list[SubstrateConservationRow]:
    out: list[SubstrateConservationRow] = []
    for arm in ALL_ARMS:
        for hazard in EXPECTED_HAZARDS:
            bucket = [r for r in per_run if r.arm == arm and r.hazard == hazard]
            if arm == ARM_A_NULL:
                # No food event; report defaults.
                out.append(
                    SubstrateConservationRow(
                        arm=arm,
                        hazard=hazard,
                        n_runs=len(bucket),
                        mean_total_food_before=float("nan"),
                        mean_total_food_after=float("nan"),
                        n_runs_with_multiset_changed=0,
                        n_runs_with_eligible_cells_drift=0,
                    )
                )
                continue
            mean_before = statistics.mean([r.total_food_before for r in bucket])
            mean_after = statistics.mean([r.total_food_after for r in bucket])
            n_multiset_changed = sum(
                1 for r in bucket if r.food_multiset_digest_before != r.food_multiset_digest_after
            )
            # Eligible-cells digest must be identical across all runs at the
            # same hazard bucket (chamber geometry seed-independent).
            ...
            if n_drift > 0:
                msg = (
                    ...
                    f"chamber geometry should be seed-independent at tick 50"
                )
                raise lr.LineageReplayError(msg)
            out.append(
                SubstrateConservationRow(
                    ...
                    hazard=hazard,
                    n_runs=len(bucket),
                    mean_total_food_before=mean_before,
                    mean_total_food_after=mean_after,
...
def evaluate_primary_test(per_arm: list[PerArmPerHazardRow]) -> PrimaryTestRow:
    ...
    b8 = _share_at(per_arm, ARM_B_REDUCE, 8)
    c8 = _share_at(per_arm, ARM_C_PERTURBATION, 8)
    delta_b = b8 - a8
    delta_c = c8 - a8
    ...
    primary_fires = food_density_necessary or substrate_perturbation_disrupts
    return PrimaryTestRow(
        ...
        c_below_a=c_below_a,
        c_above_a=c_above_a,
        food_density_necessary=food_density_necessary,
        substrate_perturbation_disrupts=substrate_perturbation_disrupts,
        food_density_not_necessary=food_density_not_necessary,
...
def evaluate_secondary_test(
    ...
    b0 = _share_at(per_arm, ARM_B_REDUCE, 0)
    delta_h0 = b0 - a0
...
def evaluate_verdict(primary: PrimaryTestRow, secondary: SecondaryTestRow) -> VerdictRow:
    ...
    if primary.c_above_a_unmodeled_substrate_artefact:
        msg = (
            ...
            "while b_passes=True; the density-preserving perturbation increased "
            "post-50 dominance above null. This is mechanistically unmotivated "
            "under the v0.43R model and indicates a substrate artefact "
            "unmodeled at pre-reg time. Halt loud."
        )
        raise lr.LineageReplayError(msg)

    if primary.food_density_necessary:
        return VerdictRow(
            verdict=VERDICT_DENSITY_NECESSARY,
            locked_phrase=LOCKED_DENSITY_NECESSARY_PHRASE,
            food_density_necessary=True,
            substrate_perturbation_disrupts=False,
            food_density_not_necessary=False,
            primary_fires=True,
    ...
    if primary.substrate_perturbation_disrupts:
        return VerdictRow(
            ...
            food_density_necessary=False,
            substrate_perturbation_disrupts=True,
            food_density_not_necessary=False,
            primary_fires=True,
            secondary_fires=secondary.secondary_fires,
        )
    # primary did not fire -> b_passes is False -> NOT_NECESSARY.
    return VerdictRow(
        verdict=VERDICT_DENSITY_NOT_NECESSARY,
        locked_phrase=LOCKED_DENSITY_NOT_NECESSARY_PHRASE,
        food_density_necessary=False,
        substrate_perturbation_disrupts=False,
        food_density_not_necessary=True,
        primary_fires=False,
        secondary_fires=secondary.secondary_fires,
    )
...
def evaluate_auxiliary_findings(per_arm: list[PerArmPerHazardRow]) -> list[AuxiliaryFindingRow]:
    ...
    for hazard in EXPECTED_HAZARDS:
        b_row = next(r for r in per_arm if r.arm == ARM_B_REDUCE and r.hazard == hazard)
        n_excl = b_row.n_excluded_zero_post50
        ablation_threshold_exceeded = n_excl > AUXILIARY_DENSITY_REDUCTION_ABLATION_MAX_EXCLUDED
        phrase = (
            LOCKED_DENSITY_REDUCTION_ABLATES_REPRODUCTION_PHRASE.format(h=hazard, n_excluded=n_excl)
...
PER_RUN_FIELDNAMES: list[str] = [
    ...
    "total_food_after",
    "eligible_cells_digest",
    "food_multiset_digest_before",
    "food_multiset_digest_after",
...
SUBSTRATE_CONSERVATION_FIELDNAMES: list[str] = [
    "arm",
    "hazard",
    "n_runs",
    "mean_total_food_before",
    "mean_total_food_after",
...
def write_outputs(
    ...
    substrate_conservation: list[SubstrateConservationRow],
    ...
    paths = {
        ...
        "substrate_conservation": out_dir / "substrate_conservation.csv",
    }
    ...
    _write_dataclass_csv(
        ...
        paths["substrate_conservation"],
        SUBSTRATE_CONSERVATION_FIELDNAMES,
    )
    return paths
...
def main() -> None:
    ...
    print(
        f"v0.43R intervention_audit: {len(per_run)} per-run records "
    ...
    substrate_conservation = build_substrate_conservation(per_run)
    ...
    print(f"  {'arm':>40} {'h':>3} {'n_runs':>6} {'n_used':>6} {'mean':>8} {'median':>8}")
    for r in per_arm:
        m = "nan" if math.isnan(r.mean_share) else f"{r.mean_share:.3f}"
        med = "nan" if math.isnan(r.median_share) else f"{r.median_share:.3f}"
        print(
            f"  {r.arm:>40} {r.hazard:>3d} {r.n_runs:>6d} {r.n_runs_used:>6d} {m:>8} {med:>8}",
            flush=True,
        )

    print()
    print("Substrate conservation:")
    print(
        f"  {'arm':>40} {'h':>3} {'n':>3} "
        f"{'mean_before':>12} {'mean_after':>12} {'n_multiset_changed':>20}"
    )
    for r in substrate_conservation:
        mb = "nan" if math.isnan(r.mean_total_food_before) else f"{r.mean_total_food_before:.1f}"
        ma = "nan" if math.isnan(r.mean_total_food_after) else f"{r.mean_total_food_after:.1f}"
        print(
            f"  {r.arm:>40} {r.hazard:>3d} {r.n_runs:>3d} "
            f"{mb:>12} {ma:>12} {r.n_runs_with_multiset_changed:>20d}",
            flush=True,
        )
    ...
    print(
        ...
        f"B_reduce={primary.b_share_h8:.3f}  "
        f"C_perturb={primary.c_share_h8:.3f}"
    )
    print(
    ...
    print()
    print(f"  v0.43R verdict: {verdict.verdict}")
    print(f'    locked phrase: "{verdict.locked_phrase}"')

    print()
    print("Auxiliary findings (DENSITY_REDUCTION_ABLATES_REPRODUCTION):")
    for r in auxiliary:
        print(
