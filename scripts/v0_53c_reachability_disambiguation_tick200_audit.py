"""v0.53c reachability-disambiguation audit (3-arm tick-200 reducer).

Pre-reg: [[docs/experiments/fear_hunger_v0.53c.md]]. Question: does extending
the observation window from tick-100 (v0.53b) to tick-200 (the full
``N_TICKS=200`` simulation length under V0_25) reveal whether v0.53b's
``WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_100`` is a tick-100-window
artifact (B reachability crosses 25% at tick-200) or persists at the
maximum-window measurement (B reachability still < 25% at tick-200,
formally bounding v0.53's ``BRIDGE_PARTIALLY_GENERALIZES`` to the V0_25
substrate x ``widened_gradient`` x ``N_TICKS=200`` cell)?

Corpus (locked, identical shape to v0.53 / v0.53b): A_null arm only across
v0.42 / v0.43R / v0.44 / v0.45, hazards {0, 8}, seeds 41..72. 64 runs per
arm; 3 arms (one per layout); 192 runs total.

Arms (locked, 3 — same layout selection as v0.53 / v0.53b):
  A_null_V0_25:        ChamberLayout = tight_gradient_layout()
  B_widened_gradient:  ChamberLayout = widened_gradient_layout()
  C_food_ladder:       ChamberLayout = food_ladder_layout()

Each arm constructs HHModel via the normal A_null path (no founder body
patch, no override, no helper RNG, no permutation). The only difference
between v0.53b and v0.53c is the observation window upper bound:
v0.53b stops accumulation at tick-100; v0.53c continues through tick-200.
Three rollups (pre50_*, pre100_*, pre200_*) and three Label B variants
(tick50, tick100, tick200) are computed in parallel from a single per-tick
observer pass.

Primary observables (locked, 3, generalized to tick-200):
  pre200_food_events_count                       (+)  — verdict-gating
  pre200_food_energy_acquired                    (+)  — verdict-gating
  mean_distance_to_nearest_food_cell_tick200     (-)  — verdict-gating

Tick-50 cells anchor v0.48 / v0.53 / v0.53b (Tier-1, gating priority 2).
Tick-100 cells re-anchor v0.53b (SECONDARY sanity check; not gating).

Labels:
  Label A: high_sensor_radius_lineage. Trait-resolved. Identical to v0.48-v0.53b.
...
B_widened reachability metric (locked):
  b_reachability_run_share_tick_200 = fraction of B_widened runs where at
  least one founder lineage has pre200_food_events_count > 0.
  Threshold: B_REACHABILITY_THRESHOLD = 0.25.
  Tick-50 / tick-100 reachability also computed (descriptive; should match
  v0.53 / v0.53b).

Per-arm tick-200 sub-verdicts (locked, 4-way each):
  A_null_V0_25:       A_NULL_V025_TICK200_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      A_NULL_V025_TICK200_OPPOSITE_SIGN_HALT
  B_widened_gradient: B_WIDENED_TICK200_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      B_WIDENED_TICK200_OPPOSITE_SIGN_HALT
  C_food_ladder:      C_LADDER_TICK200_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      C_LADDER_TICK200_OPPOSITE_SIGN_HALT
A_null_V0_25 tick-50 sub-verdict computed for the BRIDGE_REPLICATION_HALT trigger.

Slice rollup (locked, 6 outcomes, priority-ordered):
  1. CORPUS_REDERIVE_DRIFT_HALT
  2. BRIDGE_REPLICATION_HALT (Tier-1 A_null_V0_25 tick-50 anchor)
  3. LAYOUT_OPPOSITE_SIGN_HALT (any tick-200 wrong-sign across A/B/C)
  4. WIDENED_REACHABILITY_RESOLVED_AT_TICK_200 (B reachability_tick200 >= 0.25
     AND B tick-200 = PRESENT)
  5. WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND_AT_TICK_200
     (B reachability_tick200 >= 0.25 AND B tick-200 in {PARTIAL, NOT_FOUND})
  6. WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200 (B reachability_tick200 < 0.25)

Conservation framing: no `src/` modifications; the window extension is
purely script-local in the per-tick observer.

Usage:
    uv run python scripts/v0_53c_reachability_disambiguation_tick200_audit.py
    uv run python scripts/v0_53c_reachability_disambiguation_tick200_audit.py \
        --out-dir runs/v0.53c-reachability-disambiguation-tick200
"""
...
ARM_B_WIDENED: str = "B_widened_gradient"
ARM_C_LADDER: str = "C_food_ladder"
ARMS: tuple[str, ...] = (ARM_A_NULL_V025, ARM_B_WIDENED, ARM_C_LADDER)

LAYOUT_NAME_BY_ARM: dict[str, str] = {
    ARM_A_NULL_V025: "tight_gradient",
    ARM_B_WIDENED: "widened_gradient",
    ARM_C_LADDER: "food_ladder",
}
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    if arm == ARM_B_WIDENED:
        return widened_gradient_layout()
    if arm == ARM_C_LADDER:
        return food_ladder_layout()
    msg = f"v0.53c: unknown arm {arm!r}"
    raise V053cReducerError(msg)


# Hardcoded re-anchor reference values (A_null_V0_25 arm only; Tier-1 only).
...
# v0.48 / v0.53 / v0.53b's published bridge cells (label, observable) -> signed_d.
...
SUBVERDICT_B_WIDENED_TICK200_PRESENT = "B_WIDENED_TICK200_BRIDGE_PRESENT"
SUBVERDICT_B_WIDENED_TICK200_PARTIAL = "B_WIDENED_TICK200_BRIDGE_PARTIAL"
SUBVERDICT_B_WIDENED_TICK200_NOT_FOUND = "B_WIDENED_TICK200_BRIDGE_NOT_FOUND"
SUBVERDICT_B_WIDENED_TICK200_OPPOSITE = "B_WIDENED_TICK200_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_LADDER_TICK200_PRESENT = "C_LADDER_TICK200_BRIDGE_PRESENT"
SUBVERDICT_C_LADDER_TICK200_PARTIAL = "C_LADDER_TICK200_BRIDGE_PARTIAL"
SUBVERDICT_C_LADDER_TICK200_NOT_FOUND = "C_LADDER_TICK200_BRIDGE_NOT_FOUND"
SUBVERDICT_C_LADDER_TICK200_OPPOSITE = "C_LADDER_TICK200_OPPOSITE_SIGN_HALT"

# Per-arm tick-50 sub-verdicts (computed for A_null_V0_25 only, anchor-only).
...
ROLLUP_LAYOUT_OPPOSITE_HALT = "LAYOUT_OPPOSITE_SIGN_HALT"
ROLLUP_WIDENED_REACHABILITY_RESOLVED_AT_TICK_200 = "WIDENED_REACHABILITY_RESOLVED_AT_TICK_200"
ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND_AT_TICK_200 = (
    "WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND_AT_TICK_200"
)
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200 = (
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53c's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_BRIDGE_REPLICATION_HALT: (
        "Halt: v0.53c's A_null_V0_25 arm does not reproduce v0.48 / v0.53 / v0.53b's "
        "tick-50 spatial bridge — either a paired_d cell drifts beyond 1e-3 of the "
        "published value, or the tick-50 sub-verdict does not resolve to PRESENT. "
        "v0.53c cannot interpret the tick-200 cells without an established baseline."
    ),
    ROLLUP_LAYOUT_OPPOSITE_HALT: (
        "Halt: a v0.53c tick-200 spatial / foraging primary fires in the WRONG "
        "direction under a gating label; the maximum-window extension surfaces a "
        "regime incompatible with the locked expected signs."
    ),
    ROLLUP_WIDENED_REACHABILITY_RESOLVED_AT_TICK_200: (
        "On the modern A_null corpus with the V0_25 substrate held constant and the "
        "observation window extended from tick-100 to tick-200 (the full simulation "
        "length), the v0.48 spatial / foraging bridge fires PRESENT on "
        "`widened_gradient` under the locked +0.5 paired_d threshold. v0.53b's "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_100` was a tick-100-window "
        "artifact; the bridge is measurable on `widened_gradient` given the maximum "
        "simulation window on this substrate."
    ),
    ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND_AT_TICK_200: (
        "On the modern A_null corpus with the V0_25 substrate held constant and the "
        "observation window extended from tick-100 to tick-200 (the full simulation "
        "length), the v0.48 spatial / foraging bridge does NOT fire PRESENT on "
        "`widened_gradient` despite measurable reachability (≥ 25% of runs producing "
        "pre200 food access). The NOT_FOUND outcome under maximum observation is not "
        "reducible to window-censoring on this layout × substrate; the bridge fails "  # noqa: RUF001
        "to fire even when food primaries become measurable. Layout-specific failure "
        "logged in Results."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200: (
        "On the modern A_null corpus with the V0_25 substrate held constant and the "
        "observation window extended from tick-100 to tick-200 (the full simulation "
        "length), fewer than 25% of `widened_gradient` runs have any founder lineage "
        "with pre200 food events. B_widened's reachability is below the locked 25% "
        "threshold under maximum observation; the bridge cannot be measured on "
        "`widened_gradient` at any window length the V0_25 substrate's `N_TICKS=200` "
        "simulation can deliver. v0.53's `BRIDGE_PARTIALLY_GENERALIZES` verdict scope "
        "is now formally bounded along the (V0_25 substrate × `widened_gradient`) "  # noqa: RUF001
        "cell; substrate-variation disambiguation (v0.53d candidate) is the natural "
        "follow-up."
    ),
}


class V053cReducerError(Exception):
    """Halt condition raised when a v0.53c invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53c audit (read-only)."""
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53c: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53c invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53c layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(
    ...
    Order (mirrors v0.53b):
      (a) capture v0.53c founder audit table from live bodies (read-only).
...
def _run_one_arm(arm: str, version: str, seed: int, hazard: int, runs_root: Path) -> _RunCapture:
    ...
    capture = _RunCapture(
        arm=arm, layout_name=layout_name, version=version, seed=seed, hazard=hazard
    )
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53c: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53c-{arm}-{version}-A_null-hzd{hazard}",
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53c: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053cReducerError(msg)
...
_SUBVERDICT_TICK200_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_B_WIDENED: (
        SUBVERDICT_B_WIDENED_TICK200_PRESENT,
        SUBVERDICT_B_WIDENED_TICK200_PARTIAL,
        SUBVERDICT_B_WIDENED_TICK200_NOT_FOUND,
        SUBVERDICT_B_WIDENED_TICK200_OPPOSITE,
    ),
    ARM_C_LADDER: (
        SUBVERDICT_C_LADDER_TICK200_PRESENT,
        SUBVERDICT_C_LADDER_TICK200_PARTIAL,
        SUBVERDICT_C_LADDER_TICK200_NOT_FOUND,
        SUBVERDICT_C_LADDER_TICK200_OPPOSITE,
    ),
}

# A_null_V0_25 tick-50 sub-verdict for the BRIDGE_REPLICATION_HALT trigger.
...
def _classify_subverdict(
    ...
    but not the numerator. A label with 3 NaN cells clears 0/3 -> doesn't fire.
    A label with 2 NaN + 1 firing clears 1/3 -> doesn't fire.
    A label with 1 NaN + 2 firing clears 2/3 -> fires.
    """
...
def _arm_subverdict_tick200(
    ...
    if arm not in _SUBVERDICT_TICK200_TABLE:
        msg = f"v0.53c: unknown arm {arm!r} in tick-200 subverdict"
        raise V053cReducerError(msg)
...
def _a_null_subverdict_tick50(
    ...
    """A_null_V0_25 tick-50 sub-verdict (BRIDGE_REPLICATION_HALT trigger only)."""
...
def _check_bridge_re_anchor_tick50(
    ...
    """Tier-1 anchor: A_null_V0_25 tick-50 cells against v0.48 / v0.53 / v0.53b."""
...
# ---------------------------------------------------------------------------
# B_widened reachability + population-stability metrics
# ---------------------------------------------------------------------------


def _compute_b_reachability_run_share(
    all_rows: list[PerLineageRow], events_field: str
) -> tuple[float, int]:
    """Fraction of B_widened runs where >= 1 founder lineage has
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    # Priority 2: bridge replication halt (A_null_V0_25 tick-50 vs v0.48 / v0.53 / v0.53b).
    ...
    # Priority 3: layout opposite-sign halt at tick-200 (any of A/B/C).
    if (
        a_null_tick200_subverdict == SUBVERDICT_A_NULL_V025_TICK200_OPPOSITE
        or b_tick200_subverdict == SUBVERDICT_B_WIDENED_TICK200_OPPOSITE
        or c_tick200_subverdict == SUBVERDICT_C_LADDER_TICK200_OPPOSITE
    ):
        return ROLLUP_LAYOUT_OPPOSITE_HALT, LOCKED_PHRASES[ROLLUP_LAYOUT_OPPOSITE_HALT]

    # Priority 4 / 5 / 6: gating on B_widened only (B reachability_tick200 + B sub-verdict).
    if b_reachability_tick200 >= B_REACHABILITY_THRESHOLD:
        if b_tick200_subverdict == SUBVERDICT_B_WIDENED_TICK200_PRESENT:
            return (
                ROLLUP_WIDENED_REACHABILITY_RESOLVED_AT_TICK_200,
                LOCKED_PHRASES[ROLLUP_WIDENED_REACHABILITY_RESOLVED_AT_TICK_200],
            )
        # PARTIAL or NOT_FOUND
        return (
            ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND_AT_TICK_200,
            LOCKED_PHRASES[ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND_AT_TICK_200],
        )
    return (
        ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200,
        LOCKED_PHRASES[ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200],
    )
...
def _write_b_reachability_section(
    writer: csv.writer,
    section: str,
    reachability: float,
    n_runs: int,
    ...
    writer.writerow([section, "value", _format_value(reachability)])
    writer.writerow([section, "n_runs_b_widened", _format_value(n_runs)])
    writer.writerow([section, "threshold", _format_value(B_REACHABILITY_THRESHOLD)])
    writer.writerow(
        [section, "passes_threshold", _format_value(reachability >= B_REACHABILITY_THRESHOLD)]
    )
    writer.writerow([section, "is_verdict_gating", _format_value(is_gating)])
...
def _write_audit_summary_csv(
    ...
    bridge_re_anchor_tick100: list[BridgeReAnchorRow],
    a_null_tick200_subverdict: str,
    b_tick200_subverdict: str,
    c_tick200_subverdict: str,
    b_reachability_by_window: dict[int, tuple[float, int]],
    ...
    with path.open("w", newline="") as f:
        ...
        for br in bridge_re_anchor_tick100:
            for field_name, value in (
                ("published_signed_d", br.published_signed_d),
                ("derived_signed_d", br.derived_signed_d),
                ("drift_abs", br.drift_abs),
        ...
        for window, section in (
            (TICK_50, "b_reachability_tick50"),
            (TICK_100, "b_reachability_tick100"),
            (TICK_200, "b_reachability_tick200"),
        ):
            reach, n = b_reachability_by_window[window]
            _write_b_reachability_section(writer, section, reach, n, is_gating=(window == TICK_200))
        ...
        writer.writerow(["sub_verdicts_tick200", ARM_B_WIDENED, b_tick200_subverdict])
        writer.writerow(["sub_verdicts_tick200", ARM_C_LADDER, c_tick200_subverdict])
...
def _write_audit_log(  # noqa: PLR0912
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("=== v0.53c reachability disambiguation audit (tick-200) ===\n")
    ...
    lines.append(
        "\nTier-1 bridge re-anchor (A_null_V0_25 tick-50 vs v0.48 / v0.53 / v0.53b published):\n"
    )
    for br in bridge_re_anchor_tick50:
    ...
    lines.append(
        "\nSECONDARY tick-100 bridge re-anchor "
        "(A_null_V0_25 tick-100 within-session sanity check; NOT gating):\n"
    )
    for br in bridge_re_anchor_tick100:
        drift_str = "inf" if math.isinf(br.drift_abs) else f"{br.drift_abs:.4f}"
        within_str = " (within 1e-3)" if br.drift_abs <= RE_ANCHOR_DRIFT_TOLERANCE else ""
        lines.append(
            f"  {br.label}/{br.observable:<46} "
            f"published={br.published_signed_d:+.3f}  derived={br.derived_signed_d:+.3f}  "
            f"drift={drift_str}{within_str}\n"
        )
    for arm in ARMS:
    ...
    for arm in ARMS:
        ...
        for s in summaries_tick200:
            if s.arm != arm:
                continue
            lines.append(f"  [{s.label}] {_observable_line(s)}")
    lines.append("\nB_widened reachability_run_share by window:\n")
    for window in WINDOWS:
        reach, n = b_reachability_by_window[window]
        gating = " (verdict-gating)" if window == TICK_200 else " (descriptive)"
        passes = reach >= B_REACHABILITY_THRESHOLD
        lines.append(
            f"  tick-{window:>3}: {reach:.4f} (n={n}, "
            f"threshold={B_REACHABILITY_THRESHOLD:.2f}, passes={passes}){gating}\n"
        )
    lines.append("\nPopulation-stability per (arm, window) (descriptive):\n")
    for ps in population_stability:
        lines.append(
            f"  {ps.arm:<24} tick-{ps.window:>3}  n_runs={ps.n_runs_total:>2}  "
    ...
    lines.append(
        ...
        f"{ARM_B_WIDENED}={b_tick200_subverdict}  {ARM_C_LADDER}={c_tick200_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53c reachability disambiguation audit (tick-200) ===")
    ...
    summaries_by_arm_label_tick100: dict[tuple[str, str], list[ObservableSummary]] = {}
    for s in summaries_tick100:
        summaries_by_arm_label_tick100.setdefault((s.arm, s.label), []).append(s)
    ...
    b_tick200_subverdict = _arm_subverdict_tick200(
        ARM_B_WIDENED,
        summaries_by_arm_label_tick200.get((ARM_B_WIDENED, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get((ARM_B_WIDENED, LABEL_B_TICK200_NAME), []),
    )
    c_tick200_subverdict = _arm_subverdict_tick200(
        ARM_C_LADDER,
        summaries_by_arm_label_tick200.get((ARM_C_LADDER, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get((ARM_C_LADDER, LABEL_B_TICK200_NAME), []),
    )

    corpus_re_anchor = _check_corpus_re_anchor(all_rows)
    # Tier-1 (gating): tick-50 cells against v0.48 / v0.53 / v0.53b published values.
    bridge_re_anchor_tick50 = _check_bridge_re_anchor_tick50(summaries_by_arm_label_tick50)
    # SECONDARY (NOT gating): tick-100 cells against v0.53b's tick-100 derived (we
    # don't have published reference values, so log v0.53c-derived against v0.53c-derived
    # at TICK_100 for both labels — these are the v0.53b cell positions that should
    # match v0.53b's measurements byte-equivalently. We log derived signed_d directly.
    bridge_re_anchor_tick100 = [
        BridgeReAnchorRow(
            label=label,
            observable=observable,
            published_signed_d=float("nan"),
            derived_signed_d=_lookup_signed_d(
                summaries_by_arm_label_tick100, ARM_A_NULL_V025, label, observable
            ),
            drift_abs=float("nan"),
            halts=False,
        )
        for (label, observable) in (
            (LABEL_A_NAME, "pre100_food_events_count"),
            (LABEL_A_NAME, "pre100_food_energy_acquired"),
            (LABEL_A_NAME, "mean_distance_to_nearest_food_cell_tick100"),
            (LABEL_B_TICK100_NAME, "pre100_food_events_count"),
            (LABEL_B_TICK100_NAME, "pre100_food_energy_acquired"),
            (LABEL_B_TICK100_NAME, "mean_distance_to_nearest_food_cell_tick100"),
        )
    ]

    b_reachability_by_window: dict[int, tuple[float, int]] = {
        TICK_50: _compute_b_reachability_run_share(all_rows, "pre50_food_events_count"),
        TICK_100: _compute_b_reachability_run_share(all_rows, "pre100_food_events_count"),
        TICK_200: _compute_b_reachability_run_share(all_rows, "pre200_food_events_count"),
    }
    ...
    rollup_verdict, rollup_phrase = _evaluate_rollup(
        ...
        b_reachability_tick200=b_reachability_by_window[TICK_200][0],
    )

    per_lineage_path = out_dir / "per_run_per_lineage_v053c.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053cReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(
        description="v0.53c reachability disambiguation audit (tick-200)"
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53c-reachability-disambiguation-tick200"),
        help=(
            "Directory for v0.53c outputs "
            "(default: runs/v0.53c-reachability-disambiguation-tick200)."
        ),
    )
