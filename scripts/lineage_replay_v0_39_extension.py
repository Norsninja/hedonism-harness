"""v0.34 lineage_replay extension over the v0.39 fresh seed stream.

Thin wrapper that imports v0.34's lineage_replay helpers via
``importlib.util`` and runs them on a v0.39-only STREAM_CONFIGS
(seeds 25..32 at ``runs/fear-hunger-v0.39-tight_gradient/arms/``),
writing to a SEPARATE output directory ``runs/lineage-v0.34-fresh/``.

This script does NOT modify [[scripts/lineage_replay.py]] in any way.
It does NOT regenerate the sealed 96-row outputs at
``runs/lineage-v0.34/``. The fresh outputs are anchor sources for
...
Differences from v0.34's main():
  ...
  - cross_check_b50_anchors is NOT called: B_POOL_ANCHORS lock the
    OLD corpus only ({4: 312, 8: 321, 12: 312}); the fresh stream's
  ...
  - Output dir: runs/lineage-v0.34-fresh/.
...
OUT_DIR_FRESH: Path = Path("runs/lineage-v0.34-fresh")


# v0.39-only STREAM_CONFIGS, structurally identical to lr.STREAM_CONFIGS
# entry shape but covering only the fresh stream.
...
def main() -> None:
    ...
    print(
        f"v0.34 lineage_replay (v0.39 fresh extension): {len(runs)} fresh runs discovered",
        flush=True,
    )
    ...
    # B_POOL_ANCHORS lock the old corpus; fresh stream's per-hazard
    ...
    print()
    print("Fresh-stream pool summary (one row per hazard):")
    print(
