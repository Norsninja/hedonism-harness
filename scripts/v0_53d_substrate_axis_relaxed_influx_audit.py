"""v0.53d substrate-axis disambiguation audit (relaxed-influx rescue probe).

Pre-reg: [[docs/experiments/fear_hunger_v0.53d.md]]. Question: is v0.53c's
``WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200`` verdict V0_25-substrate-
specific, or geometry-fundamental? Specifically: does relaxing one substrate
parameter — doubling ``ambient_influx_rate`` from the V0_25 baseline of
``1.0`` / tick to ``2.0`` / tick on ``widened_gradient`` — lift reachability
above the locked 25% threshold at tick-200 AND admit a measurable v0.48
sensor_radius spatial / foraging bridge?

Corpus (locked, identical shape to v0.53 / v0.53b / v0.53c): A_null arm only
across v0.42 / v0.43R / v0.44 / v0.45, hazards {0, 8}, seeds 41..72. 64 runs
per arm; 3 arms (asymmetric substrate); 192 runs total.

Arms (locked, 3 — asymmetric substrate):
  A_null_V0_25:        tight_gradient_layout()   + ambient_influx_rate=1.0
  B_widened_V0_25:     widened_gradient_layout() + ambient_influx_rate=1.0
  C_widened_relaxed:   widened_gradient_layout() + ambient_influx_rate=2.0

A and B preserve V0_25 baseline. C differs from B by exactly one knob
(``ambient_influx_rate``: 1.0 -> 2.0); ``energy_pool_initial=1500.0`` is
preserved on C to satisfy the pool requirement for ambient_influx_rate>0.
...
Tick-50 cells anchor v0.48 / v0.53 / v0.53b / v0.53c (Tier-1 priority 2.a).
Tick-100 cells are descriptive. Tick-200 cells are verdict-gating on the C arm.

Labels:
  Label A: high_sensor_radius_lineage. Trait-resolved (identical to v0.48-v0.53c).
...
Tier-2 categorical anchor (locked, pre-data):
  ...
  (categorical, not float-tolerance — locked from v0.53c's measured 0/64 result.)

Verdict-gating reachability (locked, pre-data):
  c_widened_relaxed_reachability_run_share_tick_200  vs  threshold 0.25.
...
Per-arm tick-200 sub-verdicts (locked, 4-way each):
  ...
  C_RELAXED_TICK200_BRIDGE_{PRESENT, PARTIAL, NOT_FOUND, OPPOSITE_SIGN_HALT}
...
Slice rollup (locked, 6 outcomes, priority-ordered):
  ...
  3. RELAXED_OPPOSITE_SIGN_HALT  (C_widened_relaxed tick-200 wrong-sign)
...
Conservation framing: no `src/` modifications; the substrate variation is
purely chamber-config-local on the C arm.

Usage:
    uv run python scripts/v0_53d_substrate_axis_relaxed_influx_audit.py
    uv run python scripts/v0_53d_substrate_axis_relaxed_influx_audit.py \
        --out-dir runs/v0.53d-substrate-axis-relaxed-influx
"""
...
ARM_C_WIDENED_RELAXED: str = "C_widened_relaxed"
ARMS: tuple[str, ...] = (ARM_A_NULL_V025, ARM_B_WIDENED_V025, ARM_C_WIDENED_RELAXED)

LAYOUT_NAME_BY_ARM: dict[str, str] = {
    ARM_A_NULL_V025: "tight_gradient",
    ARM_B_WIDENED_V025: "widened_gradient",
    ARM_C_WIDENED_RELAXED: "widened_gradient",
}

# Per-arm ambient_influx_rate override (locked).
# A and B use V0_25 baseline (1.0); C is the relaxed variant (2.0).
AMBIENT_INFLUX_RATE_BY_ARM: dict[str, float] = {
    ARM_A_NULL_V025: 1.0,
    ARM_B_WIDENED_V025: 1.0,
    ARM_C_WIDENED_RELAXED: 2.0,
}
...
V0_25_AMBIENT_INFLUX_RATE: float = 1.0  # baseline
RELAXED_AMBIENT_INFLUX_RATE: float = 2.0  # v0.53d test dose


def _layout_for_arm(arm: str) -> ChamberLayout:
    if arm == ARM_A_NULL_V025:
        return tight_gradient_layout()
    if arm in (ARM_B_WIDENED_V025, ARM_C_WIDENED_RELAXED):
        return widened_gradient_layout()
    msg = f"v0.53d: unknown arm {arm!r}"
    raise V053dReducerError(msg)
...
# v0.48 / v0.53 / v0.53b / v0.53c published bridge cells (label, observable) -> signed_d.
...
SUBVERDICT_C_RELAXED_TICK200_PRESENT = "C_RELAXED_TICK200_BRIDGE_PRESENT"
SUBVERDICT_C_RELAXED_TICK200_PARTIAL = "C_RELAXED_TICK200_BRIDGE_PARTIAL"
SUBVERDICT_C_RELAXED_TICK200_NOT_FOUND = "C_RELAXED_TICK200_BRIDGE_NOT_FOUND"
SUBVERDICT_C_RELAXED_TICK200_OPPOSITE = "C_RELAXED_TICK200_OPPOSITE_SIGN_HALT"
...
ROLLUP_WIDENED_BRIDGE_RESCUED_BY_RELAXED_INFLUX = "WIDENED_BRIDGE_RESCUED_BY_RELAXED_INFLUX"
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX = (
    "WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX"
)
ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_RELAXED_INFLUX = (
    "WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_RELAXED_INFLUX"
)
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53d's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_ANCHOR_REPLICATION_HALT: (
        "Halt: v0.53d's A_null_V0_25 arm does not reproduce v0.48 / v0.53 / v0.53b / "
        "v0.53c's tick-50 spatial bridge, OR v0.53d's B_widened_V0_25 arm does not "
        "reproduce v0.53c's `0/64` reachability lock at tick-200. v0.53d cannot "
        "interpret the C_widened_relaxed cells without an established baseline on the "
        "V0_25 substrate."
    ),
    ROLLUP_RELAXED_OPPOSITE_SIGN_HALT: (
        "Halt: a v0.53d C_widened_relaxed tick-200 spatial / foraging primary fires in "
        "the WRONG direction under a gating label; doubling `ambient_influx_rate` from "
        "V0_25 baseline `1.0` to `2.0` / tick on the `widened_gradient` layout surfaces "
        "a regime incompatible with the locked expected signs."
    ),
    ROLLUP_WIDENED_BRIDGE_RESCUED_BY_RELAXED_INFLUX: (
        "On the modern A_null corpus with the V0_25 substrate held constant except for "
        "`ambient_influx_rate = 2.0` / tick (doubled from the V0_25 baseline of `1.0` / "
        "tick) on `widened_gradient`, the v0.48 sensor_radius spatial / foraging bridge "
        "fires PRESENT under the locked +0.5 paired_d threshold at tick-200. The "
        "geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200` admits a measurable bridge "
        "under the relaxed-influx envelope: B_widened_V0_25 reachability remains `0/64` "
        "at tick-200 (predecessor lock holds), C_widened_relaxed reachability clears "
        "the locked 25% threshold, and ≥ 2/3 spatial / foraging primaries fire PRESENT "
        "under both gating labels. The (V0_25 × `widened_gradient`) reachability "  # noqa: RUF001
        "ceiling is bounded by the V0_25 ambient influx, not by the geometry alone: "
        "`WIDENED_BRIDGE_RESCUED_BY_RELAXED_INFLUX`."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX: (
        "On the modern A_null corpus with the V0_25 substrate held constant except for "
        "`ambient_influx_rate = 2.0` / tick (doubled from the V0_25 baseline of `1.0` / "
        "tick) on `widened_gradient`, fewer than 25% of C_widened_relaxed runs have any "
        "founder lineage with pre200 food events. C_widened_relaxed's reachability is "
        "below the locked 25% threshold at tick-200; the geometry/substrate cell that "
        "v0.53c locked as `WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200` remains "
        "below the reachability threshold under the tested relaxed-influx envelope. "
        "The bridge is not rescued by `ambient_influx_rate = 2.0` under V0_25 × "  # noqa: RUF001
        "`widened_gradient` × N_TICKS=200; v0.53e (or later) substrate disambiguation "  # noqa: RUF001
        "along a different knob, a stronger dose, or multi-knob co-variation remains "
    ...
    ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_RELAXED_INFLUX: (
        "On the modern A_null corpus with the V0_25 substrate held constant except for "
        "`ambient_influx_rate = 2.0` / tick (doubled from the V0_25 baseline of `1.0` / "
        "tick) on `widened_gradient`, C_widened_relaxed's reachability clears the "
        "locked 25% threshold at tick-200 but the v0.48 sensor_radius spatial / "
        "foraging bridge does not fully fire PRESENT — fewer than 2/3 primaries fire "
        "across both gating labels at tick-200 under the strict NaN rule. The "
        "geometry/substrate cell that v0.53c locked as "
        "`WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_200` is partially rescued under "
        "the relaxed-influx envelope: reachability becomes measurable but the bridge "
        "does not fully replicate. Layout-specific partial-rescue logged in Results: "
        "`WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_RELAXED_INFLUX`."
    ),
}


class V053dReducerError(Exception):
    """Halt condition raised when a v0.53d invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53d audit (read-only)."""
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53d: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53d invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53d layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(
    ...
    Order (mirrors v0.53c merged implementation):
      (a) capture v0.53d founder audit table from live bodies (read-only).
...
def _run_one_arm(arm: str, version: str, seed: int, hazard: int, runs_root: Path) -> _RunCapture:
    ...
    A and B preserve the V0_25 baseline ``ambient_influx_rate=1.0``.
    C overrides ``ambient_influx_rate=2.0`` (relaxed). All other WorldConfig /
    ReproductionConfig knobs are inherited byte-identically from the V0_25
    baseline arm.
    """
    if arm not in ARMS:
        msg = f"v0.53d: unknown arm {arm!r}"
    ...
    arm_influx = AMBIENT_INFLUX_RATE_BY_ARM[arm]
    ...
    capture = _RunCapture(
        ...
        ambient_influx_rate=arm_influx,
        version=version,
        seed=seed,
        hazard=hazard,
    )
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53d: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53d-{arm}-{version}-A_null-hzd{hazard}",
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53d: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053dReducerError(msg)
    ...
    for lid in all_lineages:
        ...
        rows.append(
            PerLineageRow(
                ...
                ambient_influx_rate=capture.ambient_influx_rate,
...
_SUBVERDICT_TICK200_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_WIDENED_RELAXED: (
        SUBVERDICT_C_RELAXED_TICK200_PRESENT,
        SUBVERDICT_C_RELAXED_TICK200_PARTIAL,
        SUBVERDICT_C_RELAXED_TICK200_NOT_FOUND,
        SUBVERDICT_C_RELAXED_TICK200_OPPOSITE,
    ),
}
...
def _arm_subverdict_tick200(
    ...
    if arm not in _SUBVERDICT_TICK200_TABLE:
        msg = f"v0.53d: unknown arm {arm!r} in tick-200 subverdict"
        raise V053dReducerError(msg)
...
def _check_bridge_re_anchor_tick50(
    ...
    """Tier-1 anchor: A_null_V0_25 tick-50 cells against v0.48 / v0.53 / v0.53b / v0.53c."""
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    c_widened_relaxed_reachability_tick200: float,
) -> tuple[str, str]:
    ...
    Priority 3 fires only on C_widened_relaxed tick-200 wrong-sign (A and B
    OPPOSITE flags do NOT trigger priority 3 in v0.53d — A/B are anchor arms
    and surface as priority-2 halts via sub-verdict / categorical lock).

    Priorities 4/5/6 gate on C_widened_relaxed reachability + sub-verdict only.
    """
    ...
    # Priority 3: RELAXED_OPPOSITE_SIGN_HALT — C_widened_relaxed only.
    if c_tick200_subverdict == SUBVERDICT_C_RELAXED_TICK200_OPPOSITE:
        return (
            ROLLUP_RELAXED_OPPOSITE_SIGN_HALT,
    ...
    if c_widened_relaxed_reachability_tick200 < B_REACHABILITY_THRESHOLD:
        # Priority 5: below threshold, regardless of C sub-verdict.
        return (
            ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX,
            LOCKED_PHRASES[ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD_UNDER_RELAXED_INFLUX],
        )
    if c_tick200_subverdict == SUBVERDICT_C_RELAXED_TICK200_PRESENT:
        # Priority 4: rescued.
        return (
            ROLLUP_WIDENED_BRIDGE_RESCUED_BY_RELAXED_INFLUX,
            LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_RESCUED_BY_RELAXED_INFLUX],
        )
    # Priority 6: partially rescued (C sub-verdict in {PARTIAL, NOT_FOUND}).
    return (
        ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_RELAXED_INFLUX,
        LOCKED_PHRASES[ROLLUP_WIDENED_BRIDGE_PARTIALLY_RESCUED_BY_RELAXED_INFLUX],
    )
    ...
    for arm, (reach, n) in arm_reach.items():
        ...
        writer.writerow(
            [
                ...
                _format_value(is_gating and arm == ARM_C_WIDENED_RELAXED),
            ]
        )
...
def _write_audit_summary_csv(
    ...
    with path.open("w", newline="") as f:
        ...
        writer.writerow(["sub_verdicts_tick200", ARM_C_WIDENED_RELAXED, c_tick200_subverdict])
...
def _write_audit_log(  # noqa: PLR0912
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("=== v0.53d substrate-axis disambiguation audit (relaxed-influx tick-200) ===\n")
    ...
    lines.append(
        ...
        "(A_null_V0_25 tick-50 vs v0.48 / v0.53 / v0.53b / v0.53c published):\n"
    )
    for br in bridge_re_anchor_tick50:
    ...
    lines.append(
        f"  {ARM_B_WIDENED_V025:<24} tick-200  n={b_v025_n_t200:>2}  "
    ...
    for window in WINDOWS:
        for arm in ARMS:
            ...
            elif window == TICK_200 and arm == ARM_C_WIDENED_RELAXED:
                tag = " (verdict-gating)"
            else:
                tag = " (descriptive)"
            passes = reach >= B_REACHABILITY_THRESHOLD
            lines.append(
                f"  tick-{window:>3} {arm:<24}: {reach:.4f} (n={n}, "
    ...
    lines.append(
        ...
        f"{ARM_C_WIDENED_RELAXED}={c_tick200_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53d substrate-axis disambiguation audit (relaxed-influx tick-200) ===")
    print(f"Arms: {list(ARMS)}")
    print(f"Per-arm ambient_influx_rate: {AMBIENT_INFLUX_RATE_BY_ARM}")
    ...
    c_tick200_subverdict = _arm_subverdict_tick200(
        ARM_C_WIDENED_RELAXED,
        summaries_by_arm_label_tick200.get((ARM_C_WIDENED_RELAXED, LABEL_A_NAME), []),
        summaries_by_arm_label_tick200.get((ARM_C_WIDENED_RELAXED, LABEL_B_TICK200_NAME), []),
    )
    ...
    rollup_verdict, rollup_phrase = _evaluate_rollup(
        ...
        c_widened_relaxed_reachability_tick200=(
            reachability_by_window[TICK_200][ARM_C_WIDENED_RELAXED][0]
        ),
    )

    per_lineage_path = out_dir / "per_run_per_lineage_v053d.csv"
    ...
    if rollup_verdict in {
        ...
        raise V053dReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(
        description="v0.53d substrate-axis disambiguation audit (relaxed-influx tick-200)"
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53d-substrate-axis-relaxed-influx"),
        help=("Directory for v0.53d outputs (default: runs/v0.53d-substrate-axis-relaxed-influx)."),
    )
