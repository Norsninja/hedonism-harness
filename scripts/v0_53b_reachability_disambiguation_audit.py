"""v0.53b reachability-disambiguation audit (3-arm tick-100 reducer).

Pre-reg: [[docs/experiments/fear_hunger_v0.53b.md]]. Question: does extending
the observation window from tick-50 to tick-100 reveal whether v0.53's
``B_WIDENED_BRIDGE_NOT_FOUND`` is reachability-censored (B's pre-window food
primaries fire under longer window, sub-verdict resolves PRESENT) or
structurally absent (B's pre-window food primaries remain near-zero even under
longer window, OR sub-verdict remains NOT_FOUND despite measurable food
access)?

Corpus (locked, identical shape to v0.53): A_null arm only across
...
between v0.53 and v0.53b is the observation window: v0.53 captures pre50
primaries; v0.53b captures pre50 (for the v0.48 / v0.53 re-anchor) AND
pre100 primaries (for the verdict). Layout invariant assert at setup
end (V053bReducerError on mismatch).

Primary observables (locked, 3, generalized to tick-100):
  pre100_food_events_count           (+)
  pre100_food_energy_acquired        (+)
  mean_distance_to_nearest_food_cell (-)  (snapshot mean over ticks 0..100)

Tick-50 cells are also computed for the Tier-1 anchor against v0.48 /
v0.53 published cells.

Labels:
  Label A: high_sensor_radius_lineage. Trait-resolved (override is None
           on every founder under v0.53b). Identical to v0.48-v0.53.
  Label B (tick-50): high_tick50_readiness_fraction_lineage — for the
           Tier-1 anchor only.
  Label B (tick-100): high_tick100_readiness_fraction_lineage — gates
           the v0.53b verdict.
...
B_widened reachability metric (locked):
  b_reachability_run_share_tick_100 = fraction of B_widened runs where at
  least one founder lineage has pre100_food_events_count > 0.
  Threshold: B_REACHABILITY_THRESHOLD = 0.25.

Per-arm tick-100 sub-verdicts (locked):
  A_null_V0_25:       A_NULL_V025_TICK100_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      A_NULL_V025_TICK100_OPPOSITE_SIGN_HALT
  B_widened_gradient: B_WIDENED_TICK100_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      B_WIDENED_TICK100_OPPOSITE_SIGN_HALT
  C_food_ladder:      C_LADDER_TICK100_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND /
                      C_LADDER_TICK100_OPPOSITE_SIGN_HALT
Tick-50 sub-verdicts are also computed for A_null_V0_25 (BRIDGE_REPLICATION_HALT
trigger) but are not first-class outcomes in the rollup.
...
Slice rollup (locked, 6 outcomes, priority-ordered):
  ...
  3. LAYOUT_OPPOSITE_SIGN_HALT (any tick-100 wrong-sign across A/B/C)
  4. WIDENED_REACHABILITY_CENSORED_RESOLVED (B reachability >= 0.25 AND
     B tick-100 = PRESENT)
  5. WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND (B reachability >= 0.25
     AND B tick-100 in {PARTIAL, NOT_FOUND})
  6. WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_100 (B reachability < 0.25)

Conservation framing: no `src/` modifications; the window extension is
purely script-local in the per-tick observer.

Usage:
    uv run python scripts/v0_53b_reachability_disambiguation_audit.py
    uv run python scripts/v0_53b_reachability_disambiguation_audit.py \
        --out-dir runs/v0.53b-reachability-disambiguation
"""
...
def _layout_for_arm(arm: str) -> ChamberLayout:
    ...
    msg = f"v0.53b: unknown arm {arm!r}"
    raise V053bReducerError(msg)
...
# Primary observables (locked, generalized to tick-100 for the verdict).
...
# Tier-1 anchor (tick-50) observables: identical to v0.48 / v0.53.
...
# v0.48 / v0.53's published bridge cells (label, observable) -> signed_d.
# Tier-1 anchor at tick-50 only. v0.53b's tick-100 cells have NO published
# reference. Note observable name ``mean_distance_to_nearest_food_cell`` in
# the published reference maps to v0.53b's
# ``mean_distance_to_nearest_food_cell_tick50`` (renamed for tick-50 vs
# tick-100 disambiguation).
...
SUBVERDICT_B_WIDENED_TICK100_PRESENT = "B_WIDENED_TICK100_BRIDGE_PRESENT"
SUBVERDICT_B_WIDENED_TICK100_PARTIAL = "B_WIDENED_TICK100_BRIDGE_PARTIAL"
SUBVERDICT_B_WIDENED_TICK100_NOT_FOUND = "B_WIDENED_TICK100_BRIDGE_NOT_FOUND"
SUBVERDICT_B_WIDENED_TICK100_OPPOSITE = "B_WIDENED_TICK100_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_LADDER_TICK100_PRESENT = "C_LADDER_TICK100_BRIDGE_PRESENT"
SUBVERDICT_C_LADDER_TICK100_PARTIAL = "C_LADDER_TICK100_BRIDGE_PARTIAL"
SUBVERDICT_C_LADDER_TICK100_NOT_FOUND = "C_LADDER_TICK100_BRIDGE_NOT_FOUND"
SUBVERDICT_C_LADDER_TICK100_OPPOSITE = "C_LADDER_TICK100_OPPOSITE_SIGN_HALT"
...
ROLLUP_WIDENED_REACHABILITY_CENSORED_RESOLVED = "WIDENED_REACHABILITY_CENSORED_RESOLVED"
ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND = (
    "WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND"
)
ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD = "WIDENED_BELOW_REACHABILITY_THRESHOLD_AT_TICK_100"
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "{version}; v0.53b's deterministic re-execution of the V0_25 corpus does not "
        "reproduce the published metric within 1e-3."
    ),
    ROLLUP_BRIDGE_REPLICATION_HALT: (
        "Halt: v0.53b's A_null_V0_25 arm does not reproduce v0.48 / v0.53's tick-50 "
        "spatial bridge — either a paired_d cell drifts beyond 1e-3 of the published "
        "value, or the tick-50 sub-verdict does not resolve to PRESENT. v0.53b cannot "
        "interpret the tick-100 cells without an established baseline."
    ),
    ROLLUP_LAYOUT_OPPOSITE_HALT: (
        "Halt: a v0.53b tick-100 spatial / foraging primary fires in the WRONG "
        "direction under a gating label; the window extension surfaces a regime "
        "incompatible with the locked expected signs."
    ),
    ROLLUP_WIDENED_REACHABILITY_CENSORED_RESOLVED: (
        "On the modern A_null corpus with the V0_25 substrate held constant and the "
        "observation window extended from tick-50 to tick-100, the v0.48 spatial / "
        "foraging bridge fires PRESENT on `widened_gradient` under the locked +0.5 "
        "paired_d threshold. v0.53's `B_WIDENED_BRIDGE_NOT_FOUND` was "
        "reachability-censored by the 50-tick window; the bridge is measurable on "
        "`widened_gradient` given longer observation time."
    ),
    ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND: (
        "On the modern A_null corpus with the V0_25 substrate held constant and the "
        "observation window extended from tick-50 to tick-100, the v0.48 spatial / "
        "foraging bridge does NOT fire PRESENT on `widened_gradient` despite "
        "measurable reachability (≥ 25% of runs producing pre100 food access). "
        "The NOT_FOUND outcome is not reducible to reachability censoring on this "
        "layout; the bridge fails to fire under longer observation even when food "
        "primaries become measurable. Layout-specific failure logged in Results."
    ),
    ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD: (
        "On the modern A_null corpus with the V0_25 substrate held constant and the "
        "observation window extended from tick-50 to tick-100, fewer than 25% of "
        "`widened_gradient` runs have any founder lineage with pre100 food events. "
        "B_widened's reachability is below the locked 25% threshold; the bridge "
        "cannot be measured on `widened_gradient` at this window length on this "
        "substrate. v0.53's `B_WIDENED_BRIDGE_NOT_FOUND` is itself reachability-bound "
        "and no claim about bridge presence is established by v0.53b."
    ),
}


class V053bReducerError(Exception):
    """Halt condition raised when a v0.53b invariant is violated."""
...
class _FounderRecord:
    """Per-founder snapshot for the v0.53b audit (read-only)."""
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.53b: failed to find unique A_null arm for {version} h={hazard}; "
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    ...
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53b invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _assert_layout_invariant(model: HHModel, arm: str) -> None:
    ...
    if actual_width != expected.width or actual_height != expected.height:
        msg = (
            f"v0.53b layout invariant violated for arm {arm!r}: "
...
def _make_setup_observer(
    ...
    Order (mirrors v0.53):
      (a) capture v0.53b founder audit table from live bodies (read-only).
      (b) wire AgentBorn (lineage tracking) / AteFood / HazardDamageApplied
          listeners (sender=model). The AteFood / HazardDamageApplied
          listeners accumulate BOTH pre50 and pre100 rollups in parallel.
    ...
    def setup(model: HHModel) -> None:
        ...
        def _on_ate_food(_sender: object, *, event: AteFood) -> None:
            ...
            # pre100 always accumulates for tick_now <= TICK_100 (includes
            # ticks 0..50 and 51..100).
...
def _run_one_arm(arm: str, version: str, seed: int, hazard: int, runs_root: Path) -> _RunCapture:
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.53b: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.53b-{arm}-{version}-A_null-hzd{hazard}",
    ...
    # contiguous prefix [0, last_tick] with last_tick <= TICK_100, and the
    # tick-0 record always exists. tick50_readiness / tick100_readiness lists
    # may be empty if extinction occurred before the snapshot tick.
    max_expected_ticks = TICK_100 + 1
    if len(capture.tick_records) < 1 or len(capture.tick_records) > max_expected_ticks:
        msg = (
            f"v0.53b invariant: per-tick observer must capture between 1 and "
            f"{max_expected_ticks} ticks for {run_id}; got {len(capture.tick_records)}"
        )
        raise V053bReducerError(msg)
    if 0 not in capture.tick_records:
        msg = f"v0.53b invariant: missing tick-0 record for {run_id}"
        raise V053bReducerError(msg)
    captured_ticks = sorted(capture.tick_records.keys())
    if captured_ticks != list(range(captured_ticks[0], captured_ticks[-1] + 1)):
        msg = (
            f"v0.53b invariant: tick records must be a contiguous prefix for "
            f"{run_id}; got non-contiguous ticks {captured_ticks}"
        )
        raise V053bReducerError(msg)
    if len(capture.founder_records) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.53b invariant: expected exactly {EXPECTED_FOUNDERS} founder records for "
...
def _select_sensor_radius_label(
    ...
    v0.53b reads the trait directly — every founder has
    ``effective_sensor_radius_override = None``, so the override-or-trait
    resolver falls through to ``int(traits.sensor_radius)`` identically
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.53b: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V053bReducerError(msg)
...
_SUBVERDICT_TICK100_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_B_WIDENED: (
        SUBVERDICT_B_WIDENED_TICK100_PRESENT,
        SUBVERDICT_B_WIDENED_TICK100_PARTIAL,
        SUBVERDICT_B_WIDENED_TICK100_NOT_FOUND,
        SUBVERDICT_B_WIDENED_TICK100_OPPOSITE,
    ),
    ARM_C_LADDER: (
        SUBVERDICT_C_LADDER_TICK100_PRESENT,
        SUBVERDICT_C_LADDER_TICK100_PARTIAL,
        SUBVERDICT_C_LADDER_TICK100_NOT_FOUND,
        SUBVERDICT_C_LADDER_TICK100_OPPOSITE,
    ),
}
...
def _arm_subverdict_tick100(
    arm: str,
    summaries_a: list[ObservableSummary],
    summaries_b: list[ObservableSummary],
) -> str:
    """Per-arm tick-100 4-way sub-verdict; both labels gate."""
    if arm not in _SUBVERDICT_TICK100_TABLE:
        msg = f"v0.53b: unknown arm {arm!r} in tick-100 subverdict"
        raise V053bReducerError(msg)
    return _classify_subverdict(summaries_a, summaries_b, _SUBVERDICT_TICK100_TABLE[arm])
...
# ---------------------------------------------------------------------------
# B_widened reachability
# ---------------------------------------------------------------------------


def _compute_b_reachability_run_share_tick_100(all_rows: list[PerLineageRow]) -> tuple[float, int]:
    """Fraction of B_widened runs where >= 1 founder lineage has
    pre100_food_events_count > 0. Returns (fraction, n_runs)."""
    by_arm_run = _index_rows_by_arm_run(all_rows)
    n_runs_total = 0
    n_runs_with_food = 0
    for (a, _ver, _seed, _haz), rows_in_run in by_arm_run.items():
        if a != ARM_B_WIDENED:
            continue
        n_runs_total += 1
        if any(r.pre100_food_events_count > 0 for r in rows_in_run):
            n_runs_with_food += 1
    if n_runs_total == 0:
        return float("nan"), 0
...
def _evaluate_rollup(  # noqa: PLR0911 — 6-way priority match is the explicit verdict structure.
    *,
    ...
    a_null_tick100_subverdict: str,
    b_tick100_subverdict: str,
    c_tick100_subverdict: str,
    corpus_re_anchor: list[ReAnchorRow],
    bridge_re_anchor: list[BridgeReAnchorRow],
    b_reachability: float,
) -> tuple[str, str]:
    ...
    # Priority 2: bridge replication halt (A_null_V0_25 tick-50 vs v0.48 / v0.53).
    ...
    # Priority 3: layout opposite-sign halt at tick-100 (any of A/B/C).
    if (
        a_null_tick100_subverdict == SUBVERDICT_A_NULL_V025_TICK100_OPPOSITE
        or b_tick100_subverdict == SUBVERDICT_B_WIDENED_TICK100_OPPOSITE
        or c_tick100_subverdict == SUBVERDICT_C_LADDER_TICK100_OPPOSITE
    ):
        return ROLLUP_LAYOUT_OPPOSITE_HALT, LOCKED_PHRASES[ROLLUP_LAYOUT_OPPOSITE_HALT]

    # Priority 4 / 5 / 6: gating on B_widened only (B reachability + B sub-verdict).
    if b_reachability >= B_REACHABILITY_THRESHOLD:
        if b_tick100_subverdict == SUBVERDICT_B_WIDENED_TICK100_PRESENT:
            return (
                ROLLUP_WIDENED_REACHABILITY_CENSORED_RESOLVED,
                LOCKED_PHRASES[ROLLUP_WIDENED_REACHABILITY_CENSORED_RESOLVED],
            )
        # PARTIAL or NOT_FOUND
        return (
            ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND,
            LOCKED_PHRASES[ROLLUP_WIDENED_REACHABILITY_RESOLVED_BRIDGE_NOT_FOUND],
        )
    return (
        ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD,
        LOCKED_PHRASES[ROLLUP_WIDENED_BELOW_REACHABILITY_THRESHOLD],
    )
    ...
    with path.open("w", newline="") as f:
        ...
        writer.writerow(
            [
                "b_reachability",
                "n_runs_b_widened",
                _format_value(b_reachability_n_runs),
            ]
        )
        writer.writerow(
            [
                "b_reachability",
                "threshold",
                _format_value(B_REACHABILITY_THRESHOLD),
            ]
        )
        writer.writerow(
            [
                "b_reachability",
                "passes_threshold",
                _format_value(b_reachability >= B_REACHABILITY_THRESHOLD),
            ]
        )
        writer.writerow(["sub_verdicts_tick100", ARM_A_NULL_V025, a_null_tick100_subverdict])
        writer.writerow(["sub_verdicts_tick100", ARM_B_WIDENED, b_tick100_subverdict])
        writer.writerow(["sub_verdicts_tick100", ARM_C_LADDER, c_tick100_subverdict])
...
def _write_audit_log(
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("=== v0.53b reachability disambiguation audit ===\n")
    ...
    lines.append("\nTier-1 bridge re-anchor (A_null_V0_25 tick-50 vs v0.48 / v0.53 published):\n")
    for br in bridge_re_anchor:
    ...
    for arm in ARMS:
        ...
        for s in summaries_tick100:
            ...
            lines.append(f"  [{s.label}] {_observable_line(s)}")
    lines.append(
        f"\nB_widened reachability_run_share_tick_100: {b_reachability:.4f} "
        f"(n_runs={b_reachability_n_runs}, threshold={B_REACHABILITY_THRESHOLD:.2f}, "
        f"passes={b_reachability >= B_REACHABILITY_THRESHOLD})\n"
    )
    lines.append(f"\nA_null_V0_25 tick-50 sub-verdict (anchor): {a_null_tick50_subverdict}\n")
    lines.append(
        f"\nTick-100 sub-verdicts: {ARM_A_NULL_V025}={a_null_tick100_subverdict}  "
        f"{ARM_B_WIDENED}={b_tick100_subverdict}  {ARM_C_LADDER}={c_tick100_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915
    ...
    print("=== v0.53b reachability disambiguation audit ===")
    ...
    print(
        "Tick-100 primary observables: "
        + ", ".join(f"{n} ({'+' if s > 0 else '-'})" for n, s in PRIMARY_OBSERVABLES_TICK100)
    )
    print()
    ...
    # Tick-50 cells (anchor + descriptive).
    summaries_tick50: list[ObservableSummary] = []
    label_pairs_tick50 = (
    ...
    a_null_tick100_subverdict = _arm_subverdict_tick100(
        ARM_A_NULL_V025,
        summaries_by_arm_label_tick100.get((ARM_A_NULL_V025, LABEL_A_NAME), []),
        summaries_by_arm_label_tick100.get((ARM_A_NULL_V025, LABEL_B_TICK100_NAME), []),
    )
    b_tick100_subverdict = _arm_subverdict_tick100(
        ARM_B_WIDENED,
        summaries_by_arm_label_tick100.get((ARM_B_WIDENED, LABEL_A_NAME), []),
        summaries_by_arm_label_tick100.get((ARM_B_WIDENED, LABEL_B_TICK100_NAME), []),
    )
    c_tick100_subverdict = _arm_subverdict_tick100(
        ARM_C_LADDER,
        summaries_by_arm_label_tick100.get((ARM_C_LADDER, LABEL_A_NAME), []),
        summaries_by_arm_label_tick100.get((ARM_C_LADDER, LABEL_B_TICK100_NAME), []),
    )

    corpus_re_anchor = _check_corpus_re_anchor(all_rows)
    # Bridge re-anchor uses tick-50 cells against the v0.48 / v0.53 published values.
    bridge_re_anchor = _check_bridge_re_anchor(summaries_by_arm_label_tick50)

    b_reachability, b_reachability_n_runs = _compute_b_reachability_run_share_tick_100(all_rows)

    rollup_verdict, rollup_phrase = _evaluate_rollup(
        a_null_tick50_subverdict=a_null_tick50_subverdict,
        a_null_tick100_subverdict=a_null_tick100_subverdict,
        b_tick100_subverdict=b_tick100_subverdict,
        c_tick100_subverdict=c_tick100_subverdict,
        corpus_re_anchor=corpus_re_anchor,
        bridge_re_anchor=bridge_re_anchor,
        b_reachability=b_reachability,
    )

    per_lineage_path = out_dir / "per_run_per_lineage_v053b.csv"
    ...
    _write_audit_summary_csv(
        ...
        corpus_re_anchor,
        bridge_re_anchor,
        a_null_tick100_subverdict,
        b_tick100_subverdict,
        c_tick100_subverdict,
        b_reachability,
        b_reachability_n_runs,
        rollup_verdict,
        rollup_phrase,
        audit_summary_path,
    )
    ...
    if rollup_verdict in {
        ...
        raise V053bReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(description="v0.53b reachability disambiguation audit")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.53b-reachability-disambiguation"),
        help=("Directory for v0.53b outputs (default: runs/v0.53b-reachability-disambiguation)."),
    )
