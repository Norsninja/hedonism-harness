"""v0.30 tight w*=0.75 small-margin robustness audit sweep driver.

Runs ``V0_30_TIGHT_W_ARMS`` (3 arms — w in {0.5, 0.75, 1.0},
substrate-byte-identical to V0_27_ARMS at the matched labels)
x tight_gradient x seeds 9..16 = 24 runs, persisting per-arm
comparison.csv into ``runs/fear-hunger-v0.30-tight_gradient/``.

Pre-reg: [[docs/experiments/fear_hunger_v0.30.md]].

Central question: does the v0.27 tight w*=0.75 +2-birth interior-
optimum claim reproduce on a fresh seed stream (9..16) under the
v0.30 4-tier robustness partition (ROBUST / WEAK / FAILURE / REVERSAL)?

The v0.30 audit (`scripts/v0.30_audit.py`) consumes these artifacts
and produces the per-seed paired-comparison report + outcome verdict.
...
from hedonism_harness.experiments.comparison_grid import V0_30_TIGHT_W_ARMS, run_comparison_grid
...
def main() -> None:
    ...
    aggregates = run_comparison_grid(
        ...
        arms=V0_30_TIGHT_W_ARMS,
    )
    elapsed = time.time() - chamber_start
    print(f"  done in {elapsed:.1f}s", flush=True)
    for arm, agg in zip(V0_30_TIGHT_W_ARMS, aggregates, strict=True):
