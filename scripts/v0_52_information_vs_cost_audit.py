"""v0.52 sensor_radius information vs metabolic-cost decoupling (first-class intervention).

Pre-reg: [[docs/experiments/fear_hunger_v0.52.md]]. Question: does the
v0.48/v0.49 sensor_radius bridge depend on information radius, metabolic
cost, or their coupling?

Corpus (locked, same as v0.46-v0.51): A_null arm only across v0.42 /
v0.43R / v0.44 / v0.45, hazards {0, 8}, seeds 41..72. 64 runs per arm;
3 arms; 192 runs total.

Arms (locked):
  A_null:                  default V0_25 BodyConfig (no override; cost=0.05).
  B_zero_sensor_cost:      model.body_config replaced inside setup_observer
                           with sensor_radius_metabolic_cost=0.0; the
                           information-radius channel is preserved
                           (effective_sensor_radius_override=None).
  C_uniform_effective_radius_6:
                           model.body_config replaced inside setup_observer
                           with effective_sensor_radius_override=6; every
                           agent senses with radius 6 regardless of trait,
                           but apply_metabolism still reads the per-agent
                           trait sensor_radius (cost=0.05 default
                           preserved, so the metabolic-cost channel is
                           preserved).

Per pre-reg "Pre-implementation correction (2026-05-08)": all arms use
``FounderSpec(traits_override=None)`` and rely on the model's normal
trait-draw + agent-RNG path. The intervention is applied as a model.body_config
replacement inside the reducer's setup_observer (no agent body trait
mutation; no descendant-time hook). v0.52 makes ONE minimal src/ change:
adds an optional ``effective_sensor_radius_override`` field to BodyConfig
(default None) and threads the resolver into ``sensors.observe`` (line 123)
and ``_read_memory_directional``'s ValenceMemory branch (line 196).
``apply_metabolism`` is NOT modified.
...
Labels (both gate for ALL three arms; founder sensor_radius variation
is preserved in every arm so Label A is NOT degenerate):
  Label A: argmax(founder_sensor_radius), min(lineage_id) tiebreak.
  Label B: argmax(tick50_above_threshold_fraction), 3-tier tiebreak.

Per-arm sub-verdicts (locked, 4-way each):
  A_null:  A_NULL_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND / _OPPOSITE_SIGN_HALT
  B:       B_ZERO_COST_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND / _OPPOSITE_SIGN_HALT
  C:       C_UNIFORM_RADIUS_BRIDGE_PRESENT / _PARTIAL / _NOT_FOUND / _OPPOSITE_SIGN_HALT

Slice rollup (locked, 8 outcomes, priority-ordered):
  1. CORPUS_REDERIVE_DRIFT_HALT
  2. INTERVENTION_OPPOSITE_SIGN_HALT
  3. BRIDGE_REPLICATION_HALT          (A_null vs v0.48; only Tier-1)
  4. INFORMATION_RADIUS_LOAD_BEARING        (B PRESENT, C NOT_FOUND)
  5. METABOLIC_COST_LOAD_BEARING            (B NOT_FOUND, C PRESENT)
  6. BOTH_CHANNELS_INDEPENDENTLY_SUFFICIENT (B PRESENT, C PRESENT)
  7. CHANNELS_COUPLED_OR_SHARED_CONFOUND    (B NOT_FOUND, C NOT_FOUND)
  8. CHANNELS_MIXED                          (any cell containing PARTIAL)

Conservation framing (interventional, with one minimal src/ change):
default override=None preserves byte-identity; no traits_override;
no agent body trait mutation; no descendant-time hook; no Mesa cell
modification; no modifications to prior reducer or audit scripts.

Usage:
    uv run python scripts/v0_52_information_vs_cost_audit.py
    uv run python scripts/v0_52_information_vs_cost_audit.py \
        --out-dir runs/v0.52-information-vs-cost
"""
...
ARM_B_ZERO_COST: str = "B_zero_sensor_cost"
ARM_C_UNIFORM_RADIUS: str = "C_uniform_effective_radius_6"
ARMS: tuple[str, ...] = (ARM_A_NULL, ARM_B_ZERO_COST, ARM_C_UNIFORM_RADIUS)

# Override constants per pre-reg.
B_ZERO_COST_VALUE: float = 0.0
C_UNIFORM_RADIUS_VALUE: int = 6  # V0_25 max draw — equalizing uplift, not nerf
...
# Primary observables (identical to v0.48-v0.51).
...
SUBVERDICT_B_PRESENT = "B_ZERO_COST_BRIDGE_PRESENT"
SUBVERDICT_B_PARTIAL = "B_ZERO_COST_BRIDGE_PARTIAL"
SUBVERDICT_B_NOT_FOUND = "B_ZERO_COST_BRIDGE_NOT_FOUND"
SUBVERDICT_B_OPPOSITE = "B_ZERO_COST_OPPOSITE_SIGN_HALT"

SUBVERDICT_C_PRESENT = "C_UNIFORM_RADIUS_BRIDGE_PRESENT"
SUBVERDICT_C_PARTIAL = "C_UNIFORM_RADIUS_BRIDGE_PARTIAL"
SUBVERDICT_C_NOT_FOUND = "C_UNIFORM_RADIUS_BRIDGE_NOT_FOUND"
SUBVERDICT_C_OPPOSITE = "C_UNIFORM_RADIUS_OPPOSITE_SIGN_HALT"
...
ROLLUP_INFORMATION_RADIUS_LOAD_BEARING = "INFORMATION_RADIUS_LOAD_BEARING"
ROLLUP_METABOLIC_COST_LOAD_BEARING = "METABOLIC_COST_LOAD_BEARING"
ROLLUP_BOTH_INDEPENDENTLY_SUFFICIENT = "BOTH_CHANNELS_INDEPENDENTLY_SUFFICIENT"
ROLLUP_CHANNELS_COUPLED = "CHANNELS_COUPLED_OR_SHARED_CONFOUND"
ROLLUP_CHANNELS_MIXED = "CHANNELS_MIXED"
...
LOCKED_PHRASES: dict[str, str] = {
    ROLLUP_CORPUS_DRIFT_HALT: (
        ...
        "v0.52's deterministic re-execution does not reproduce the published metric within 1e-3."
    ),
    ROLLUP_INTERVENTION_OPPOSITE_HALT: (
        "Halt: a v0.52 spatial / foraging primary fires in the WRONG direction under a "
        "gating label; the channel-decoupling intervention is incompatible with the "
        "locked expected signs."
    ),
    ROLLUP_BRIDGE_REPLICATION_HALT: (
        "Halt: v0.52's A_null arm does not reproduce v0.48's spatial bridge — either a "
        "paired_d cell drifts beyond 1e-3 of the published value, or the A_null sub-verdict "
        "does not resolve to PRESENT. v0.52 cannot interpret the B / C arms without an "
        "established baseline."
    ),
    ROLLUP_INFORMATION_RADIUS_LOAD_BEARING: (
        "On the modern A_null corpus, the v0.48 spatial / foraging bridge survives "
        "flattening of the metabolic-cost channel and disappears under flattening of "
        "the information-radius channel; the information-radius channel is the "
        "load-bearing component of the founder `sensor_radius` causal contribution."
    ),
    ROLLUP_METABOLIC_COST_LOAD_BEARING: (
        "On the modern A_null corpus, the v0.48 spatial / foraging bridge survives "
        "flattening of the information-radius channel and disappears under flattening "
        "of the metabolic-cost channel; the metabolic-cost channel is the load-bearing "
        "component of the founder `sensor_radius` causal contribution."
    ),
    ROLLUP_BOTH_INDEPENDENTLY_SUFFICIENT: (
        "On the modern A_null corpus, the v0.48 spatial / foraging bridge survives "
        "independent flattening of either the metabolic-cost channel or the "
        "information-radius channel; both channels can independently support the "
        "bridge under the locked V0_25 anchor."
    ),
    ROLLUP_CHANNELS_COUPLED: (
        "On the modern A_null corpus, the v0.48 spatial / foraging bridge does not "
        "survive independent flattening of either the metabolic-cost channel or the "
        "information-radius channel; the bridge requires the coupled `sensor_radius` "
        "trait, OR a shared confound v0.52 does not probe (e.g., trait covariance, "
        "founder-position interaction, or a non-linear interaction between the two "
        "channels)."
    ),
    ROLLUP_CHANNELS_MIXED: (
        "On the modern A_null corpus, v0.52's intervention arms do not resolve to a "
        "single load-bearing channel under the locked categorical criteria; at least "
        "one arm is in PARTIAL state and the channel decomposition is therefore not "
        "definitive."
    ),
}


class V052ReducerError(Exception):
    """Halt condition raised when a v0.52 invariant is violated."""
...
class _FounderRecord:
    """Per-founder original trait snapshot (no per-arm assignment changes)."""
...
class _RunCapture:
    ...
    seed: int
    hazard: int
    body_config_sensor_radius_metabolic_cost: float = 0.05
    body_config_effective_sensor_radius_override: int | None = None
...
def _select_a_null_arm(version: str, hazard: int) -> Arm:
    ...
    if len(candidates) != 1:
        msg = (
            f"v0.52: failed to find unique A_null arm for {version} h={hazard}; "
            f"got {len(candidates)} candidates"
        )
        raise V052ReducerError(msg)
    return candidates[0]


# ---------------------------------------------------------------------------
# body_config swap (B/C arms)
# ---------------------------------------------------------------------------


def _apply_body_config_swap(arm: str, model: HHModel, capture: _RunCapture) -> None:
    """Replace ``model.body_config`` for B / C arms inside setup_observer.

    A_null applies no swap. B replaces with sensor_radius_metabolic_cost=0.0
    (zero-cost; information channel preserved). C replaces with
    effective_sensor_radius_override=6 (information channel flattened to
    V0_25 max; metabolic-cost channel preserved).

    No agent body trait is mutated. ``streams.mutation`` is NOT consumed.
    """
    if arm == ARM_A_NULL:
        # No swap. Default BodyConfig() with override=None, cost=0.05.
        capture.body_config_sensor_radius_metabolic_cost = float(
            model.body_config.sensor_radius_metabolic_cost
        )
        capture.body_config_effective_sensor_radius_override = (
            model.body_config.effective_sensor_radius_override
        )
        return
    if arm == ARM_B_ZERO_COST:
        model.body_config = model.body_config.model_copy(
            update={"sensor_radius_metabolic_cost": B_ZERO_COST_VALUE}
        )
    elif arm == ARM_C_UNIFORM_RADIUS:
        model.body_config = model.body_config.model_copy(
            update={"effective_sensor_radius_override": C_UNIFORM_RADIUS_VALUE}
        )
    else:
        msg = f"v0.52: unknown arm {arm!r}"
        raise V052ReducerError(msg)
    capture.body_config_sensor_radius_metabolic_cost = float(
        model.body_config.sensor_radius_metabolic_cost
    )
...
def _capture_founder_audit(model: HHModel, capture: _RunCapture) -> None:
    """Capture per-founder trait snapshot from live model.agents (post body_config swap).

    v0.52 does NOT mutate any agent body, so this also equals the pre-swap
    state. Captured for cross-arm byte-identity verification (test #9).
    """
    founders = sorted(model.agents, key=lambda a: int(a.body.lineage_id))
    if len(founders) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.52 invariant: expected exactly {EXPECTED_FOUNDERS} founders "
...
def _make_setup_observer(
    ...
    """setup_observer factory: applies body_config swap, captures tick 0.

    Order (locked by pre-reg):
      (a) apply body_config swap (B/C) or no-op (A_null).
      (b) capture v0.52 founder audit table from live model.agents.
      (c) seed lineage_by_agent + birth_tick_by_agent for the founders.
      (d) wire AgentBorn / AteFood / HazardDamageApplied listeners
          (sender=model).
      (e) capture tick 0 snapshot.
    """

    def setup(model: HHModel) -> None:
        _apply_body_config_swap(arm, model, capture)
...
def _run_one_arm(arm: str, version: str, seed: int, hazard: int, runs_root: Path) -> _RunCapture:
    ...
    if optional_intervention is not None and optional_intervention.kind != KIND_NULL:
        msg = (
            f"v0.52: base arm must be A_null only; got "
    ...
    try:
        run_chamber(
            ...
            condition=f"v0.52-{arm}-{version}-A_null-hzd{hazard}",
    ...
    if len(capture.tick_records) != expected_ticks:
        msg = (
            f"v0.52 invariant: per-tick observer must capture {expected_ticks} ticks for "
            f"{run_id}; got {len(capture.tick_records)}"
        )
        raise V052ReducerError(msg)
    if len(capture.founder_records) != EXPECTED_FOUNDERS:
        msg = (
            f"v0.52 invariant: expected exactly {EXPECTED_FOUNDERS} founder records for "
...
class PerLineageRow:
    ...
    body_config_sensor_radius_metabolic_cost: float
    body_config_effective_sensor_radius_override: int | None
...
def _select_sensor_radius_label(
    founder_sensor_radius_by_lineage: dict[int, int],
) -> int | None:
    """argmax(founder_sensor_radius), tiebreak min(lineage_id)."""
    if not founder_sensor_radius_by_lineage:
        return None
    best_lid: int | None = None
    best_value = float("-inf")
    for lid in sorted(founder_sensor_radius_by_lineage):
        v = float(founder_sensor_radius_by_lineage[lid])
        if v > best_value:
            best_value = v
            best_lid = lid
    return best_lid
...
def _aggregate_per_lineage(capture: _RunCapture) -> list[PerLineageRow]:
    ...
    for aid, btick in capture.birth_tick_by_agent.items():
        if btick > TICK_50:
            ...
            if lid is None:
                msg = f"v0.52: agent_id {aid} has birth_tick {btick} but no lineage_id"
                raise V052ReducerError(msg)
    ...
    founder_sr_by_lineage: dict[int, int] = {
        lid: founder_by_lineage[lid].founder_sensor_radius for lid in all_lineages
    }
    sensor_label = _select_sensor_radius_label(founder_sr_by_lineage)
    ...
    for lid in all_lineages:
        ...
        rows.append(
            PerLineageRow(
                ...
                body_config_sensor_radius_metabolic_cost=(
                    capture.body_config_sensor_radius_metabolic_cost
                ),
                body_config_effective_sensor_radius_override=(
                    capture.body_config_effective_sensor_radius_override
                ),
...
_SUBVERDICT_TABLE: dict[str, tuple[str, str, str, str]] = {
    ...
    ARM_C_UNIFORM_RADIUS: (
        SUBVERDICT_C_PRESENT,
        SUBVERDICT_C_PARTIAL,
        SUBVERDICT_C_NOT_FOUND,
        SUBVERDICT_C_OPPOSITE,
    ),
}
...
def _arm_subverdict(
    ...
    """Per-arm 4-way sub-verdict; both labels gate for all three arms in v0.52."""
    if arm not in _SUBVERDICT_TABLE:
        msg = f"v0.52: unknown arm {arm!r} in subverdict"
        raise V052ReducerError(msg)
...
_PRESENT_BC = (SUBVERDICT_B_PRESENT, SUBVERDICT_C_PRESENT)
_NOT_FOUND_BC = (SUBVERDICT_B_NOT_FOUND, SUBVERDICT_C_NOT_FOUND)
_PARTIAL_BC = (SUBVERDICT_B_PARTIAL, SUBVERDICT_C_PARTIAL)


def _evaluate_rollup(  # noqa: PLR0911 — 8-way priority match is the explicit verdict structure.
    *,
    ...
    """8-outcome priority-ordered rollup. Returns (verdict, locked_phrase)."""
    drift_halt = next(
    ...
    # MIXED catch-all for any arm in PARTIAL.
    if b_subverdict in _PARTIAL_BC or c_subverdict in _PARTIAL_BC:
        return ROLLUP_CHANNELS_MIXED, LOCKED_PHRASES[ROLLUP_CHANNELS_MIXED]

    b_pres = b_subverdict == SUBVERDICT_B_PRESENT
    c_pres = c_subverdict == SUBVERDICT_C_PRESENT
    if b_pres and not c_pres:
        return (
            ROLLUP_INFORMATION_RADIUS_LOAD_BEARING,
            LOCKED_PHRASES[ROLLUP_INFORMATION_RADIUS_LOAD_BEARING],
        )
    if not b_pres and c_pres:
        return (
            ROLLUP_METABOLIC_COST_LOAD_BEARING,
            LOCKED_PHRASES[ROLLUP_METABOLIC_COST_LOAD_BEARING],
        )
    if b_pres and c_pres:
        return (
            ROLLUP_BOTH_INDEPENDENTLY_SUFFICIENT,
            LOCKED_PHRASES[ROLLUP_BOTH_INDEPENDENTLY_SUFFICIENT],
        )
    return ROLLUP_CHANNELS_COUPLED, LOCKED_PHRASES[ROLLUP_CHANNELS_COUPLED]
...
def _write_audit_summary_csv(
    ...
    with path.open("w", newline="") as f:
        ...
        writer.writerow(["sub_verdicts", "B_zero_sensor_cost", b_subverdict])
        writer.writerow(["sub_verdicts", "C_uniform_effective_radius_6", c_subverdict])
...
def _write_audit_log(
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("=== v0.52 sensor_radius information vs metabolic-cost decoupling ===\n")
    ...
    lines.append(
        ...
        f"B_zero_sensor_cost={b_subverdict}  C_uniform_effective_radius_6={c_subverdict}\n"
    )
...
def run_audit(out_dir: Path) -> tuple[str, str]:  # noqa: PLR0912, PLR0915 — orchestrator threads sweep + aggregation + paired_d (3 arms x 2 labels x 3 obs) + Tier-1 re-anchor + verdicts + I/O.
    """Run the full v0.52 audit, writing outputs under ``out_dir``."""
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=== v0.52 sensor_radius information vs metabolic-cost decoupling ===")
    ...
    print(
        f"B override: sensor_radius_metabolic_cost = {B_ZERO_COST_VALUE}\n"
        f"C override: effective_sensor_radius_override = {C_UNIFORM_RADIUS_VALUE}"
    )
    print()
            ...
            for version, seeds in SEEDS_BY_VERSION.items():
                for hazard in HAZARDS:
                    for seed in seeds:
                        ...
                        cost = capture.body_config_sensor_radius_metabolic_cost
                        ovr = capture.body_config_effective_sensor_radius_override
                        ovr_str = "None" if ovr is None else str(ovr)
                        print(
                            f"  {arm:<32} {version} h={hazard:>2} seed={seed:>2}  "
                            f"a_top={a_lid}  b_top={b_lid}  cost={cost:.2f}  override={ovr_str}"
                        )
    ...
    b_subverdict = _arm_subverdict(
        ARM_B_ZERO_COST,
        summaries_by_arm_label.get((ARM_B_ZERO_COST, LABEL_A_NAME), []),
        summaries_by_arm_label.get((ARM_B_ZERO_COST, LABEL_B_NAME), []),
    )
    c_subverdict = _arm_subverdict(
        ARM_C_UNIFORM_RADIUS,
        summaries_by_arm_label.get((ARM_C_UNIFORM_RADIUS, LABEL_A_NAME), []),
        summaries_by_arm_label.get((ARM_C_UNIFORM_RADIUS, LABEL_B_NAME), []),
    )
    ...
    per_lineage_path = out_dir / "per_run_per_lineage_v052.csv"
    ...
    if rollup_verdict in {
        ...
        raise V052ReducerError(rollup_phrase)
    return rollup_verdict, rollup_phrase


def main() -> None:
    parser = argparse.ArgumentParser(description="v0.52 information vs metabolic-cost audit")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("runs/v0.52-information-vs-cost"),
        help="Directory for v0.52 outputs (default: runs/v0.52-information-vs-cost).",
    )
