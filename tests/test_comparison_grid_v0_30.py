"""Tests for v0.30 ``V0_30_TIGHT_W_ARMS`` (3-arm subset of V0_27_ARMS at
w in {0.5, 0.75, 1.0} on the tight_gradient w*=0.75 audit cell, used
with seeds 9..16 for the small-margin robustness audit).

Covers:
  - V0_30_TIGHT_W_ARMS shape: exactly 3 arms with weights {0.5, 0.75, 1.0}.
  - V0_30_TIGHT_W_ARMS literal-subset identity to V0_27_ARMS at the
    matched labels (H1 substrate-identity-by-construction).
  - V0_30_TIGHT_W_ARMS arm-object identity to V0_29_ARMS at the matched
    labels (H1b cross-version arm-object equivalence; chamber differs at
    run time, arm objects do not).
  - Substrate-byte-identity to V0_27_ARMS by field-equality fallback.
  - Prior arm tuples (ARMS, V0_15..V0_27_ARMS, V0_29_ARMS) untouched.
"""
...
def test_v0_30_tight_w_arms_have_exactly_three_arms() -> None:
    assert len(V0_30_TIGHT_W_ARMS) == 3


def test_v0_30_tight_w_arms_have_expected_labels() -> None:
    labels = tuple(arm.label for arm in V0_30_TIGHT_W_ARMS)
    assert labels == EXPECTED_LABELS


def test_v0_30_tight_w_arms_have_expected_avoidance_weights() -> None:
    weights = tuple(arm.hazard_avoidance_weight for arm in V0_30_TIGHT_W_ARMS)
    assert weights == EXPECTED_WEIGHTS


def test_v0_30_tight_w_arms_pinned_to_hazard_eight() -> None:
    for arm in V0_30_TIGHT_W_ARMS:
        assert arm.hazard_damage == 8.0


def test_v0_30_tight_w_arms_share_substrate() -> None:
    """Every v0.30 arm holds the v0.21 substrate constant (only
...
# ---------------------------------------------------------------------------
# H1 — V0_30_TIGHT_W_ARMS is a literal subset of V0_27_ARMS
# ---------------------------------------------------------------------------
...
def test_v0_30_tight_w_arms_are_literal_v0_27_subset() -> None:
    """Every V0_30_TIGHT_W_ARMS member is the SAME Arm instance as the
    ...
    for arm in V0_30_TIGHT_W_ARMS:
        ...
        assert arm is v27_arm, (
            f"V0_30_TIGHT_W_ARMS[{arm.label}] is not the same Arm instance "
            f"as V0_27_ARMS[{arm.label}]; substrate-identity-by-construction "
            f"broken — likely an accidental copy / mutation."
        )


def test_v0_30_substrate_byte_identical_to_v0_27_at_matched_labels() -> None:
    """Equality fallback for the substrate-identity guard. Even if
    V0_30_TIGHT_W_ARMS is rewritten not to be a literal subset, every
    field must still match V0_27_ARMS at the matched labels."""
    for arm in V0_30_TIGHT_W_ARMS:
...
# ---------------------------------------------------------------------------
# H1b — arm-object identity to V0_29_ARMS (chamber differs at run time)
# ---------------------------------------------------------------------------


def test_v0_30_tight_w_arms_share_arm_objects_with_v0_29_arms() -> None:
    """H1b: V0_29_ARMS (food_ladder) and V0_30_TIGHT_W_ARMS
    (tight_gradient) reference the same underlying V0_27_ARMS instances
    for the shared avoidance-weight labels. Chamber differs at run time;
    the substrate arm objects do not. Tripwire if either tuple drifts."""
    for arm in V0_30_TIGHT_W_ARMS:
...
def test_pre_v0_26_arms_still_have_no_avoidance_weight_override() -> None:
    """No v0.30 mutation of pre-v0.26 arm tuples."""
    for tup in (
        ARMS,
        V0_15_ARMS,
        V0_16_ARMS,
...
def test_v0_26_arms_unchanged_by_v0_30() -> None:
...
def test_v0_27_arms_unchanged_by_v0_30() -> None:
...
def test_v0_29_arms_unchanged_by_v0_30() -> None:
    """V0_29_ARMS is exactly 3 arms with weights (0.5, 0.75, 1.0);
    v0.30 must not mutate it."""
