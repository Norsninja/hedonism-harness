"""v0.45 sweep driver tests.

Validates V0_45_INTERVENTION_ARMS shape, locked constants, intervention_kind
...
def _load_sweep_module():
    path = Path(__file__).resolve().parents[1] / "scripts" / "v0.45_sweep.py"
    spec = importlib.util.spec_from_file_location("v0_45_sweep", path)
    assert spec is not None
    assert spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["v0_45_sweep"] = mod
    spec.loader.exec_module(mod)
    return mod


def test_v0_45_arms_count_and_kinds():
    assert len(V0_45_INTERVENTION_ARMS) == 6
    kinds = {a.intervention_kind for a in V0_45_INTERVENTION_ARMS}
    assert kinds == {
        KIND_NULL,
        KIND_UNIFORM_VALID_REGION_BIRTH,
        KIND_UNIFORM_NEIGHBOR_BIRTH,
...
def test_v0_45_arms_two_hazards_only():
    hazards = sorted({a.hazard_damage for a in V0_45_INTERVENTION_ARMS})
    assert hazards == [0.0, 8.0]


def test_v0_45_arms_each_kind_has_both_hazards():
    by_kind: dict[str, set[float]] = {}
    for a in V0_45_INTERVENTION_ARMS:
...
def test_v0_45_arms_labels_unique():
    labels = [a.label for a in V0_45_INTERVENTION_ARMS]
    assert len(set(labels)) == len(labels)


def test_v0_45_arms_labels_have_v045_prefix():
    for a in V0_45_INTERVENTION_ARMS:
        assert a.label.startswith("v045-")


def test_v0_45_arms_substrate_matches_v0_25_anchors():
    ...
    for arm in V0_45_INTERVENTION_ARMS:
...
def test_v0_45_arms_intervention_kind_set_explicitly():
    for arm in V0_45_INTERVENTION_ARMS:
...
def test_sweep_locked_constants():
    ...
    assert tuple(range(65, 73)) == mod.SEEDS
    assert mod.N_TICKS == 200
    assert mod.N_FOUNDERS == 5
    assert mod.BATCH_ID == "fear-hunger-v0.45-tight_gradient"


def test_sweep_seeds_disjoint_from_prior_streams():
    """Seeds 65..72 must not overlap any prior stream."""
    mod = _load_sweep_module()
    fresh = set(mod.SEEDS)
    for prior in (
        range(1, 9),
...
def test_sweep_arms_constant_matches_module_export():
    mod = _load_sweep_module()
    assert mod.V0_45_INTERVENTION_ARMS is V0_45_INTERVENTION_ARMS
