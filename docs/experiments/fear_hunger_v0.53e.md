...
3. `test_arm_a_null_v025_uses_tight_gradient_layout_and_default_body_config` — chamber driver receives `tight_gradient_layout()` AND `body_config=None`; founder `body.energy` at tick-0 equals `60.0` (default `starting_energy`); layout invariant assert.
4. `test_arm_b_widened_v025_uses_widened_gradient_layout_and_default_body_config` — chamber driver receives `widened_gradient_layout()` AND `body_config=None`; founder `body.energy` at tick-0 equals `60.0`; layout invariant assert.
5. **`test_arm_c_widened_starting_energy_100_uses_widened_gradient_and_starting_energy_100`** — chamber driver receives `widened_gradient_layout()` AND `body_config=BodyConfig(starting_energy=100.0)`; founder `body.energy` at tick-0 equals **`100.0`** (verifies the seam took effect AND the V0_25 founder construction respects the override); layout invariant assert; explicit assertion that no other BodyConfig field differs from default (max_energy=100.0, starting_health=100.0, max_health=100.0, base_metabolic_cost=0.25, sensor_radius_metabolic_cost=0.05, effective_sensor_radius_override=None) AND no other WorldConfig knob differs from V0_25 baseline.
