# fear_hunger v0.53j — food-distance dose-response probe on `widened_gradient`-shape × V0_25 × combined founder-budget envelope at `n_ticks=400`: minimal rescue boundary at `FOOD_NEAR1`

**Slice:** v0.53j
**Type:** **first-class observational sweep** with 4-arm asymmetric-horizon reducer (no `src/` changes — both novel C and D layouts constructed script-local; v0.53e's `body_config` seam carries forward; `n_ticks` already exposed; no intervention; **single geometry knob with two doses** on the C and D arms — v0.53j is a dose-response slice probing the minimal rescue boundary established by v0.53i; D arm is the in-slice positive anchor).
