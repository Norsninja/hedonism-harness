...
11. **`test_pre200_window_strictly_extends_pre100_and_pre50_windows`** *(window-extension test)* — synthetic per-tick AteFood events at ticks 10, 30, 60, 90, 130, 180; assert `pre50_food_events_count = 2`, `pre100_food_events_count = 4`, `pre200_food_events_count = 6`. Confirms three-window accumulation.
