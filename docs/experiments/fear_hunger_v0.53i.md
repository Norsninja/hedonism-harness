...
5. **`test_arm_c_widened_food_near2_combined_N400_seam_differentiates_b_and_c_layout_food_x_min_shift_body_config_two_knobs_and_horizon_at_tick_400`** *(cross-arm contrast — verifies the script-local layout, BOTH BodyConfig knobs, AND C's horizon)* — for the same (version, seed, hazard) tuple under matched RNG streams, assert:
   ...
   - **B vs C layout cross-arm contrast**: `B.food_x_min == 10` AND `C.food_x_min == 8` (specifically asserts the food band shift); `B.food_x_max == 14` AND `C.food_x_max == 12`; `B.world_width == 15` AND `C.world_width == 13`; `B.hazard_x_min == C.hazard_x_min == 5` AND `B.hazard_x_max == C.hazard_x_max == 7` (hazard preserved); `B.spawn_x == C.spawn_x == 1` AND `B.safe_x_min == C.safe_x_min == 0` AND `B.safe_x_max == C.safe_x_max == 4` (spawn AND safe band preserved); `B.height == C.height == 6` (height preserved).
