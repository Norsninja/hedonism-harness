...
### Visual snapshot — seed 42, 5 founders

Glyphs: `_` SAFE, `.` EMPTY, `#` HAZARD, `*` FOOD, lowercase letters are
living agents (id mod 26). Generated by
`scripts/render_chamber_snapshot.py --seed 42 --ticks 100 --n-founders 5`.
...
By tick 100, two founders (`c`, `f`) have starved out and been removed; the
three survivors (`b`, `d`, `e`) are still milling in the safe zone, never
having crossed (or even attempted to cross) the hazard band visible to the
right. The HAZARD column at `x ∈ [8, 11]` and the FOOD column at
`x ∈ [14, 19]` remain perfectly intact — no `*` was ever consumed.
