...
| H10 | invisible-food_ladder b>50 within 10% of coupled | **HOLDS.** 88 vs 92, a 4.3% drop. Food_ladder's *b>50 metric* is buffered by population dynamics (births rise +7, partly offsetting the +24 extra deaths), so the productivity headline barely moves. **This is not geometric protection** — it is a downstream observable smoothing the upstream cull-tax. The cull-tax itself (`total_injury_deaths`, `pool_in_death_residual`) increases sharply at weight=0; H9's failure is the substantive finding here, not H10's holding. |
