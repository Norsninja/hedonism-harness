...
This compounds v0.53g's central finding: not only is reachability decoupled from survival in the early window (tick-200), but **extending the horizon does not extend survival proportionally either** under the tested envelope. The combined-budget runway estimate does not translate into effective survival; descriptively, this is **a survival drain not explained by initial energy or base metabolic cost alone**. Provisionally labeled as a "non-budget depletion sink" — plausibly hazard-band damage on attempted traversals, sensor-radius metabolic cost (0.05 per radius unit) on exploratory movement, reproduction overhead with no offspring funding return, or unmodeled per-tick dynamics — but the empirical evidence in v0.53h establishes only the descriptive observation, not the mechanism. The descriptive log records the trajectory (87.5% → 0% in 200 ticks); no mechanism is formally established by v0.53h.
